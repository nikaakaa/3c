...
这一步只安装通用能力，不迁移 Corin 资产，不更新 Agent schema、MCP bridge 或 `btsmtl-agent-authoring` 技能，也不修改任何具体网络模型。后续 Corin Sawblade 迁移只消费这里安装的正式 authoring/runtime API；网络模型后续只消费正式 EquipmentState 与表现投影，不得反向拥有装备规则。
...
- 本change不修改Agent工具、MCP bridge、Corin资产或具体Network Model。
