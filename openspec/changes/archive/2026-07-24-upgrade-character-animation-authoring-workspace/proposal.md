...
本change交付通用Workspace、Navigator、Details、Preview、Pose Watch和Live Debug能力，不把“当前没有Animation State Machine作者入口”定义为永久限制。后续`refactor-animation-control-boundaries`在同一Pose Graph Workspace增加正式PoseStateMachine、SequencePlayer与AnimationSlot，并把Locomotion source导航从BaseLocomotion Timeline迁到Profile Pose source。若本change晚于该后续change归档，归档时 MUST按已安装current model保留这些新节点和术语，不得恢复“不存在Animation State Machine”的旧口径。
