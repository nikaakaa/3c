...
### Requirement: Simulation、Command、Snapshot与Interpolation频率必须独立

`SimulationTickRate`、`CommandPacketRate`、`SnapshotPacketRate`、`CommandSlackTicks`、`RemoteInterpolationDelayTicks`和`MaxGameplayDatagramBytes` MUST是独立、进入模型configuration identity的策略。系统 MUST删除统一`ObservationCadenceTicks`，MUST不让任一传输频率改变Program或WorldSolver固定步进。
...
- **AND** remote presentation MUST按独立interpolation delay消费snapshot

#### Scenario: Remote Presentation建立初始缓冲

- **WHEN** 首个remote body位于Authority Tick 1且`RemoteInterpolationDelayTicks=6`
- **THEN** Remote Presentation MUST保持最早可用Body，直到缓冲覆盖至少6个Authority Tick
- **AND** 缓冲形成后 MUST从`LastTick - 6`启动表现时钟并按presentation delta逐帧插值
- **AND** MUST不因首个snapshot到达就从Tick 1与最新snapshot同步推进
...
Routine snapshot MUST使用ProgramHash/LayoutHash锁定且覆盖全部committed Character state slot的Network Checkpoint Layout，以已确认base snapshot为基准发送changed-slot bitset、changed values、owner body/world correction、remote body/producer、state/body hash、input ack和event horizon。它 MUST不携带逐slot codec字符串或完整`character-state/float32/v3`bytes，也 MUST不按复制policy省略Action、Timeline、Blackboard或GameplayEffect state。Client MUST重建并校验完整checkpoint后才向Correction Pipeline提交baseline。
...
Network Test Player MUST通过显式构建入口固定Bootstrap、Client和Authority Worker场景顺序，且Network Test Bootstrap MUST是第一场景。四进程启动入口 MUST启动Fantasy Server、Authority Worker、Client A和Client B，并在报告成功前验证三个Unity角色均存活且已建立网络endpoint。旧Player、漏启角色或未进入网络场景 MUST fail-fast，不能作为成功的双客户端测试环境。

#### Scenario: Client B启动语句未执行

- **WHEN** 启动入口未能创建Client B进程或Client B在检查前退出
- **THEN** 启动入口 MUST报告Client B缺失并终止本次新进程
- **AND** MUST不输出四进程已经成功启动
