...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class AssetPathUtility
    {
        ...
        public static string GetMcpServerPackageSource()
        {
            ...
            string siblingServerPath = GetSiblingServerPath();
            if (!string.IsNullOrEmpty(siblingServerPath))
            {
        ...
        private static string GetSiblingServerPath()
        {
            try
            {
                var packageInfo = PackageInfo.FindForAssembly(typeof(AssetPathUtility).Assembly);
                ...
                DirectoryInfo parent = Directory.GetParent(packageInfo.resolvedPath);
                if (parent == null)
                {
                    return null;
                }

                string serverPath = Path.Combine(parent.FullName, "Server");
                return File.Exists(Path.Combine(serverPath, "pyproject.toml")) ? serverPath : null;
            }
            catch
            {
                return null;
