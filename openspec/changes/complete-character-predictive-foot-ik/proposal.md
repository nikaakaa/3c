...
- 支撑脚拥有 `LastLanding` 且不是当前 Swing 时，先用非负 plantHeight 接地，再进入 Locked / Sliding / Unlocked。Locked / Sliding 即使垂直和水平误差为零也保持动画位置权重；Unlocked 使用正式 `UnlockBlendSeconds` 平滑回到原生动画。
- 支撑脚只在 Locked / Sliding 时发布有限 Pitch / Roll：上坡更趋水平，下坡更趋坡面，跑步达到阈值时关闭。摆动脚旋转权重保持为零。
- 有效可见 yaw 先启动 Foot Placement 唯一 trajectory revision，以支撑 `LastLanding` 为 Pivot 输入，重新做当前唯一落点 SphereCast、Capsule、Reachability、Hull 和 Envelope；之后才判定步伐、骨盆和脚 Goal。不刚体旋转旧 Foot Route、Surface 或 Ground Envelope，不改 KCC、Gameplay Body 或 VisualRoot。
...
- `add-character-foot-placement-stride-hips` 并入本 change，不再独立 apply 或按旧四步验收
...
- `openspec/project.md` 仍有“误差软阈值降权、硬阈值释放”的旧表述，归档本 change 时必须改成同事件同踏面滑动、SurfaceIdentity 或高度超阈值才换级、有效转向建立唯一 trajectory revision。
- `add-character-foot-placement-stride-hips` 的 delta 尚未进入 current spec，本 change 覆盖并扩展它，归档时只合并本 change。
...
- GDC 第 21–28 页：转向枢轴靠近接触脚。项目用唯一 trajectory revision 表达，不转胶囊、不旋转旧 Route。
