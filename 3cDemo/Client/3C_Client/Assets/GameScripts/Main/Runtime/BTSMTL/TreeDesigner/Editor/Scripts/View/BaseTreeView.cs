...
namespace TreeDesigner.Editor
{
    public class BaseTreeView : GraphView
    {
        ...
        protected string m_SelectionKey;
        ...
        public virtual void Init(BaseTreeWindow treeWindow)
        {
            ...
            m_SelectionWatcher = schedule.Execute(CheckSelectionChanged).Every(100);
        }
        public virtual void PopulateView(BaseTree tree)
        {
            ...
            m_SelectionKey = string.Empty;
            m_TreeWindow?.PopulateSelectionInspector(selection);
        ...
        protected virtual void CheckSelectionChanged()
        {
            if (m_TreeWindow == null)
                return;

            string key = BuildSelectionKey();
        ...
        protected virtual string BuildSelectionKey()
        {
            if (selection == null || selection.Count == 0)
                return string.Empty;

            return string.Join("|", selection.Select(GetSelectionItemKey));
        }

        protected virtual string GetSelectionItemKey(GraphSelectable selectable)
        {
            if (selectable is BaseEdgeView edgeView && edgeView.Edge != null)
            {
                int ruleGraphId = edgeView.Edge.ConditionRuleGraph ? edgeView.Edge.ConditionRuleGraph.GetHashCode() : 0;
                return $"edge:{edgeView.Edge.GUID}:{edgeView.Edge.TransitionPriority}:{edgeView.Edge.AbortPolicy}:{ruleGraphId}";
            }

            if (selectable is BaseNodeView nodeView && nodeView.Node != null)
                return $"node:{nodeView.Node.GUID}";

            if (selectable is StackNodeView stackNodeView && stackNodeView.StackNode != null)
                return $"stack:{stackNodeView.StackNode.GUID}";

            if (selectable is NodeGroupView nodeGroupView && nodeGroupView.NodeGroup != null)
                return $"group:{nodeGroupView.NodeGroup.GetHashCode()}";

            return selectable?.GetHashCode().ToString() ?? string.Empty;
        }
        ...
        {
            if (!confirmed && !ConfirmRemoveConditionEdgesForNode(node))
                return false;
            DeleteOwnedConditionRuleGraphsForNode(node);
        ...
        {
            if (!confirmed && !BaseEdgeView.ConfirmConditionEdgeDeletion(edge))
                return false;
        ...
        protected virtual GraphViewChange OnGraphViewChanged(GraphViewChange graphViewChange)
        {
            ...
            {
                if (!ConfirmRemoveGraphElements(graphViewChange.elementsToRemove))
                {
                    graphViewChange.elementsToRemove = new List<GraphElement>();
                    return graphViewChange;
                }
        ...
        bool ConfirmRemoveGraphElements(IEnumerable<GraphElement> elements)
        {
            foreach (BaseEdge edge in CollectConditionEdgesToRemove(elements))
            {
        ...
        bool ConfirmRemoveConditionEdgesForNode(BaseNode node)
        {
            foreach (BaseEdge edge in CollectConditionEdgesForNode(node))
            {
                if (!BaseEdgeView.ConfirmConditionEdgeDeletion(edge))
                    return false;
            }
        ...
        void DeleteOwnedConditionRuleGraphsForNode(BaseNode node)
        {
            foreach (BaseEdge edge in CollectConditionEdgesForNode(node))
                BaseEdgeView.DeleteOwnedConditionRuleGraphForEdgeDelete(edge);
        }
        ...
        {
            if (node == null || m_Tree == null)
                yield break;

            foreach (BaseEdge edge in m_Tree.Edges)
            {
                if (edge == null || !(BaseEdgeView.IsStateMachineTransition(edge) || BaseEdgeView.IsBTConditionFlowEdge(edge)))
                    continue;
        ...
        {
            if (m_Tree == null || m_RuntimeReadOnly) return;
            ...
            m_Tree.ApplyModify(operationName, () =>
            {
                ...
                foreach (var copiedNode in data.copiedNodes)
                {
                    ...
                    if (acceptableNodePaths.Contains(startPath) && m_Tree.CanCreateNodeType(newNode.GetType()))
                    {
                        ...
                        newNode.RegenerateOwnedAuthoringIdentities();
