...
- [x] 10.3 使用 Unity Test Runner 定向运行统一状态机 EditMode 测试。
- [x] 10.4 使用 Unity Test Runner 定向运行 Locomotion/TurnBack/root motion EditMode 测试。
- [x] 10.5 读取 Unity Console，确认相关 error 为 0。
- [x] 10.6 不运行 Unity batchmode。

## 11. 文档和收尾
- [x] 11.1 更新 `docs/agents/turnback-rootmotion-debug-log.md`，记录统一管线后的 TurnBack 触发链路。
- [x] 11.2 如 Path 文档涉及管线顺序，按 `update-path-docs` 规则检查是否需要更新。
- [x] 11.3 运行 `openspec validate refactor-locomotion-decision-pipeline --strict --no-interactive`。
- [x] 11.4 确认全部任务完成后再将 checklist 改为 `- [x]`。
