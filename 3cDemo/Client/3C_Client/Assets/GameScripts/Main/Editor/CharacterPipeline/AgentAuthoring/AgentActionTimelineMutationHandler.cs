...
namespace ThirdPersonCharacter.Pipeline.Editor.AgentAuthoring
{
    public sealed class AgentActionEligibilityMutationHandler : IAgentMutationHandler
    {
            ...
            switch (command)
            {
                ...
                case AgentEnsureAnimationClipSegmentMutation value:
                    ...
                    session.AddPlanned(value, null, value.Target.ClipAuthoringId,
                        $"ensure AnimationClip segment {value.StartFrame}..{value.EndFrame}");
                    return true;
        ...
        static void ApplyEnsureAnimationClipSegment(
            AgentMutationSession session,
            AgentEnsureAnimationClipSegmentMutation command)
        {
            if (!TryResolveAnimationTrack(session, command.Target, command.Path, out TimelineData timeline, out AnimationTrack track) ||
                !TryResolveAnimationClip(session, command.Clip, command.Path, out UnityEngine.AnimationClip sourceClip))
                return;
            ...
            track.UpdateMix();
            timeline.Init();
            session.AddAppliedAuthoring(command, timeline.SerializedOwner, clip, clip.AuthoringId, "ensure AnimationClip segment");
        }

        static bool ValidateAnimationClipSegment(
            AgentMutationSession session,
            AgentEnsureAnimationClipSegmentMutation command)
        {
            ...
            {
                session.Report.Error(command.Path, "animation_clip_segment_frames_invalid", "AnimationClip Segment frame范围非法。");
                return false;
            }
            if (sourceClip && command.ClipInFrame > Mathf.RoundToInt(
                    CharacterAnimationClipRegisteredCurveCatalog.ResolveSourceDurationSeconds(sourceClip) * TimelineUtility.FrameRate))
            {
                session.Report.Error(command.Path, "animation_clip_segment_clip_in_invalid", "AnimationClip Segment ClipIn超出素材时长。");
                return false;
            }
            ...
            if (existing is BTSMTL.Timeline.AnimationClip)
                return true;
            session.Report.Error(command.Path, "animation_clip_segment_not_found", $"AnimationClip Segment identity无法解析：{command.Target.ClipAuthoringId}");
            return false;
        }
            ...
            out UnityEngine.AnimationClip clip)
        {
            ...
            {
                session.Report.Error(path + ".animationClip", "animation_clip_local_identity_forbidden", "Document不能创建AnimationClip。");
                return false;
            }
            else if (reference != null)
            {
                clip = AssetDatabase.LoadAssetAtPath<UnityEngine.AnimationClip>(reference.assetPath);
                if (clip &&
                    AssetDatabase.TryGetGUIDAndLocalFileIdentifier(clip, out string guid, out long localFileId) &&
            ...
            if (!clip)
            {
                session.Report.Error(path + ".animationClip", "animation_clip_reference_unresolved", "AnimationClip Segment无法解析精确Clip引用。");
                return false;
            }
