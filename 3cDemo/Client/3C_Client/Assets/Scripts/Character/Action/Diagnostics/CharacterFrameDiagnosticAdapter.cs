...
{
    public sealed class CharacterFrameDiagnosticAdapter
    {
        readonly ICharacterDiagnosticSink sink;

        public CharacterFrameDiagnosticAdapter(ICharacterDiagnosticSink sink)
        {
        ...
        {
            Submit(CharacterFrameDiagnosticEventFormatter.PipelineSnapshot(activePath, step, diagnosticSummary));
        }

        public void LogStatePathChanged(
            in CharacterStateMachineSnapshot previousSnapshot,
            in CharacterStateMachineSnapshot snapshot,
            int step)
        {
            Submit(CharacterFrameDiagnosticEventFormatter.StatePathChanged(in previousSnapshot, in snapshot, step));
        }

        public void LogPendingTransitionChanged(
            in CharacterStateMachineSnapshot previousSnapshot,
            in CharacterStateMachineSnapshot snapshot,
            int step)
        {
            Submit(CharacterFrameDiagnosticEventFormatter.PendingTransitionChanged(in previousSnapshot, in snapshot, step));
        }
        ...
        public void LogLocomotionPhaseChanged(
            ...
            int step)
        {
            Submit(CharacterFrameDiagnosticEventFormatter.LocomotionPhaseChanged(
                locomotionPath,
        ...
        public void LogActionAccepted(
            ...
            int step)
        {
            Submit(CharacterFrameDiagnosticEventFormatter.ActionAccepted(in previousSnapshot, in snapshot, in frame, step));
        }

        public void LogStateTickSnapshot(
            in CharacterStateMachineSnapshot snapshot,
            int step,
            string context)
        {
            Submit(CharacterFrameDiagnosticEventFormatter.StateTickSnapshot(in snapshot, step, context));
        }
        ...
        public void LogAnimationTickSnapshot(
            ...
            string context)
        {
            Submit(CharacterFrameDiagnosticEventFormatter.AnimationTickSnapshot(activePath, step, context));
        }

        public void LogStateMachineDefinitionInvalid(string message)
        {
            Submit(CharacterFrameDiagnosticEventFormatter.StateMachineDefinitionInvalid(message));
        }
        ...
        public void LogTimelineFactsTrace(StateTimelineFactsTrace trace)
        {
            ...
            Submit(CharacterFrameDiagnosticEventFormatter.TimelineFactsTrace(trace));
        }
        ...
        public void LogTransitionConditionTraces(IReadOnlyList<CharacterStateTransitionConditionTrace> traces)
        {
            ...
            for (int i = 0; i < traces.Count; i++)
            {
                ...
                Submit(CharacterFrameDiagnosticEventFormatter.TransitionConditionTrace(trace));
            }
        }

    }

    public static class CharacterFrameDiagnosticEventFormatter
    {
        ...
        public static CharacterFrameDiagnosticTrace StatePathChanged(
            in CharacterStateMachineSnapshot previousSnapshot,
            in CharacterStateMachineSnapshot snapshot,
            int step)
        {
        ...
        public static CharacterFrameDiagnosticTrace PendingTransitionChanged(
            in CharacterStateMachineSnapshot previousSnapshot,
            in CharacterStateMachineSnapshot snapshot,
            int step)
        {
            ...
            return new CharacterFrameDiagnosticTrace(
                RuntimeDiagnosticLogCategory.CharacterFrame,
                RuntimeDiagnosticLogLevel.Trace,
                "character-frame-pending-transition-changed",
        ...
        public static CharacterFrameDiagnosticTrace StateTickSnapshot(
            in CharacterStateMachineSnapshot snapshot,
            int step,
            string context)
        {
