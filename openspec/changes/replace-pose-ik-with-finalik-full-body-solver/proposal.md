...
- 每脚事件以“上一次同姿态Landing -> 下一次同姿态Landing”定义稳定身份与时间域。每只脚只允许在当前权威事件已经进入PreSwing后创建唯一计划，不再为尚未成为当前动作的incoming事件冻结世界路线或晋升候选。计划以创建帧Native Sole、未来Body轨迹和同相位root-local脚姿态共同重基；`Locked / Sliding`期间仍由同一Stance owner持有脚部输出，Predictive只在真实LiftOff后通过既有Anchor Blend连续接管。
...
- A/D转弯使用创建前连续两帧正式平面速度方向得到的有符号`Trajectory Curvature`。KCC未来Body位移和root-local Foot/Hip/Ankle几何共同消费该曲率形成同一圆弧。Movement节点的`Maximum Yaw Velocity`只作为方向突变合法性上限，不得作为路线曲率。
...
Authoritative Action Step
  -> Frozen Collision-resolved KCC Body Trajectory and Trajectory Curvature
...
QueryRoute = Polyline(
    ...
    OpposingLanding(KccTrajectory, TrajectoryCurvature),
    OwnLanding(KccTrajectory, TrajectoryCurvature))
...
- Simulation Locomotion必须保持Corin正式Constant Speed；Foot Prediction必须报告创建帧冻结的committed Body Target Velocity、Simulation Continuation、最大转向能力、实际轨迹曲率、计划剩余时间和预测Root位移；Foot Analysis不得发布Action Root或第二速度真相；最终脚XZ保持当前原动画Pose；
