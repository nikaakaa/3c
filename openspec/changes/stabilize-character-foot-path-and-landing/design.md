...
| `0x171D7910`先读取上一`arr130`与本次Direction，超过实例`+0x70`角度阈值时限制本次Direction变化，再把结果写回`arr130`；随后为每脚修正标量保存独立`arr128`历史并由`0x199`选择`0x2A0/0x2A4`执行`±rate×dt`限制。当前可琳实例`+0x70=10°`，最新Raw双脚精确满足`arr230 = arr228 + arr130 × arr128` | 在完整Vector World Residual负责正式Position Target换代连续后，先保存独立`CorrectionResponseDirectionHistory`并把Requested Support Normal每次最多朝目标转10度，再以过滤后的Direction计算Desired Response；标量`CorrectionResponseHistory`继续按项目typed增减方向选择Profile两档速率。Direction变化不得把上一世界输出投影成新法线scalar；Target Height仍单独沿Component Up处理 | Interpolation Runtime Direction History、Correction Response Stage |
| `0x171D8A00`把修正结果按`k=clamp01((1-weight)×globalWeight)`与基准混合，同时让缓存量回归；运行Trace中`0x278`有12个目标窗口、`0x274`有10个响应窗口，138/138次变化精确匹配`5×dt`，扩展字段`0xFC`全部为1 | 项目让`SelectedWorldTarget + PlantWorldResidual`先形成Desired Output，再由Support Normal方向的Correction Response形成唯一Response Output，最后通过现有typed Foot Goal Position/Rotation Weight Owner与动画基线混合一次；可参考Target/Current Weight有界追踪数学，但不新增匿名模式或Goal后处理器 | Interpolation Runtime、Resolved Foot、Goal Contribution |
...
| Foot与Toe世界姿态生成脚掌多点查询，先解析唯一`FootTargetPosition + SupportNormal`；只有FootL/FootR与Pelvis writer，没有Toe writer | 使用Final Animation Pose与Rig Calibration的Heel/Toe接触几何，在现有World Query Backend内生成固定容量五点Current Support Observation，解析唯一Position+Normal并只写现有Foot Goal；采用已经公式闭合的前五点布局，不复制第六条件查询、匿名外部重载或无稳定tie-break的候选顺序 | Current Support Observation、State Target、Resolved Foot |
...
-> 每脚Correction Response标量沿过滤Direction双向限速
-> 既有Foot Goal权重混回动画基线
-> 唯一Goal Assembler与FBBIK
```

当前正式实现已经删除raw Contact累计PlantBlend并接入Heel/Toe Current Support、独立Correction Response与Target Height显式采用模式，但`20260830-022607`证明两Probe的`纯Component Up最大位移 + 同一selected raw Normal`不是ZZZ的24B几何等价物；单帧斜Normal又被项目自创`BasisTransferred`投影放大为24厘米水平反切、1.218腿伸长比与12.5厘米FBBIK残差。Direction History与标量Correction Response已经按上述P0/P1顺序闭合，`BasisTransferred`整条删除。最新公式级静态闭环又确认前五个查询点由Foot基点、单侧局部轴点、对称局部轴点对与Toe派生点组成，活体Corin尺度为`0.03m / 0.04m / 0.01m`，统一从世界Up抬升`0.5m`并使用`1.0m`查询标量；匿名第六查询、外部查询重载和对象Delta仍未闭合。项目只迁移前五点几何关系，把查询距离继续交给现有Profile，不复制匿名第六分支。正式Owner收敛为`TargetHeightHistory / PlantWorldResidual / CorrectionResponseDirectionHistory / CorrectionResponseHistory / PlantTarget`。Prediction、Pelvis进阶字段、Knee、Moving Platform和PIK到tbik/通用Solver的连接仍不属于已确认主链；除本change已有项目证据支持的能力外，不用未闭合推断扩大范围。
...
| 可琳Target Height可直接换代；`0x130`先按实例10度阈值限制Direction变化，`0x128`再以双档速率连续追赶，楼梯与攻击共用；Raw闭合`arr230=arr228+arr130×arr128` | P1静态顺序 + 活体实例参数 + P1运行时Trace + Raw双脚同帧关系 | 可以开始；Corin显式Direct模式、每次10度Direction上限、两档`1.8/1.5m/s`，全部位于同一Interpolation内且不得把上一世界输出投影为新Direction scalar |
...
| 前五个Foot/Toe查询点与尺度 | 4BA0前五次调用P1公式闭合，活体Corin尺度为单侧纵向`0.03m`、对称侧向`0.04m`、Toe延展`0.01m`；第六查询和外部typed重载未知 | 在Rig Calibration正式声明项目语义的Rear、Lateral与Toe Extension，使用现有Profile查询形状、距离和稳定identity；不复制第六分支或把查询距离当脚掌尺度 |
...
每个表现帧从同一`FinalAnimationPoseFrame`和Rig Calibration取得Heel、Toe接触点、Foot Rotation、Sole Forward、Sole Right、Component Up与Foot尺寸。Rig Calibration为每脚显式保存正数`RearProbeExtension`、`LateralProbeExtent`与`ToeProbeExtension`并纳入Calibration Revision；Corin首个正式值分别为`0.03m / 0.04m / 0.01m`。Current Support Builder在现有World Query Backend内按固定顺序生成五个Probe：`Base = Heel`、`Rear = Heel - SoleForward × RearProbeExtension`、`PositiveLateral = Heel + SoleRight × LateralProbeExtent`、`NegativeLateral = Heel - SoleRight × LateralProbeExtent`、`Toe = Toe + SoleForward × ToeProbeExtension`。这五点是项目对ZZZ已经公式闭合前五点的typed翻译；匿名条件第六点不实施。所有Probe继续使用同一Profile坡度、查询距离、半径、Layer、自身Collider排除、有限值与World Revision合同；不得从Foot Motion Toe曲线、另一Pose Source或LateUpdate骨骼读取建立第二输入，也不得把Profile查询距离或Sphere Radius复用为Foot尺寸。
...
每个Probe分别保留Accepted/Rejected/NotExecuted、命中位置、法线、距离、Surface identity、World Revision和拒绝原因。Base与Toe是必需记录；任一必需记录无效、任一执行过的Probe容量溢出或World Revision不一致时整只脚typed unavailable。Rear与两个Lateral是始终执行的正式候选，NoHit只表示该候选缺席，不改名成成功或复用旧记录。至少Base与Toe合法后，每个Accepted记录先生成自己的完整三维候选：`N = normalize(HitNormal)`、`Translation = N × dot(HitPoint - ProbePosition, N)`、`CandidateSole = OriginalSole + Translation`。Runtime按`dot(CandidateSole, ComponentUp)`选择最高安全候选，等价高度按Surface identity和固定Probe顺序打破平局；最终Position与Requested Direction必须来自同一Selected记录。该公式只使用HitPoint建立沿命中平面的法向位移，不把带Sphere半径切向偏移的HitPoint直接当Foot Position，也不再拼接`OriginalSole + ComponentUp × max displacement`与另一条raw Normal。不得调低Slope阈值、偏好Up、平均多法线或用旧Support、Animated Up、单点降级与默认地面冒充成功。

Current Support Target进入State Target选择，但当前Final Component Pose已经完成Pose Graph混合，Foot Placement不再补造第二状态权重。Position Source与Requested Direction Source必须分别发布Event、Path、Frame、Completion和World lineage，不能伪装成同一Observation。Verified/Retained Anchor同时拥有冻结Position与Direction，实时Current Support拒绝不得反向释放Anchor。Requested Direction归一化后进入唯一Direction History，Applied Direction同时供Correction Response与Rig Sole Forward生成Foot Rotation；Target Height仍独立沿Component Up计算。位置、旋转、Applied Direction、分型lineage、Goal Weight和Writer汇入同一Resolved Foot，Toe不生成第二Goal。Rotation与Ankle Position必须按FBBIK本帧实际Position/Rotation Weight联合反解，零Rotation Weight不得改变Swing动画XZ。Direction变化只由Interpolation内每次10度历史推进，不执行世界投影、不在FBBIK或Final Pose之后增加Rotation低通。Pelvis仍只消费Resolved Foot Pair，不读取原始多点记录。
...
9. 保留已经完成的旧`MaximumVerticalCorrectionSpeed`单档后置链删除，在同一Interpolation中分离Plant Target Height、完整Vector Plant World Residual与新的标量Correction Response History；Current/Target Position与Support Normal按同一状态权重混合并归一化，Correction Response沿该方向使用双档速率，Target Height仍沿Component Up独立处理；安装初始化/重置和Action/攻击/同Event不清零政策，使`Desired Output -> Response Output -> Existing Goal Baseline Mix`只有一条正式链。
