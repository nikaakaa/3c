...
namespace TreeDesigner.Editor
{
    public static partial class TreeDesignerUtility
    {
        ...
        static Dictionary<UnityEngine.Object, SerializedObject> s_SerializedTreeMap = new Dictionary<UnityEngine.Object, SerializedObject>();
        ...
        {
            string[] guids = AssetDatabase.FindAssets("t:script", new[] { "Assets/GameScripts/Main/Runtime/BTSMTL/TreeDesigner/Scripts/Node" });
        ...
        {
            UnityEngine.Object owner = tree.SerializedOwner;
            if (!owner)
            {
                action?.Invoke();
            ...
            Undo.RegisterCompleteObjectUndo(owner, $"Tree ({name})");
        ...
        public static SerializedObject GetNewSerializedTree(this BaseGraph tree)
        {
            ...
            if (!s_SerializedTreeMap.ContainsKey(owner))
                s_SerializedTreeMap.Add(owner, new SerializedObject(owner));
            else
                s_SerializedTreeMap[owner] = new SerializedObject(owner);

            return s_SerializedTreeMap[owner];
        }
        ...
        {
            SerializedObject serializedTree = node.GetSerializedTree();
            string path = node.Owner.GetNodeSerializedPropertyPath(node);
            return serializedTree?.FindProperty(path);
        }
        public static SerializedProperty GetSerializedExposedProperty(this BaseExposedProperty exposedProperty)
        {
            SerializedObject serializedTree = exposedProperty.GetSerializedTree();
            string path = exposedProperty.Owner.GetExposedPropertySerializedPropertyPath(exposedProperty);
            return serializedTree?.FindProperty(path);
        }
