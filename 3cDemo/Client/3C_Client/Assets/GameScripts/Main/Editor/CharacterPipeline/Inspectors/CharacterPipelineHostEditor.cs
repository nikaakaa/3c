    ...
    static class CharacterRuntimeDiagnosticsInspector
    {
        ...
        static void AppendFootIkLegHeader(List<string> values, string prefix)
        {
            string[] beforePath =
            {
                ...
                "current_event_foot_pose_weight", "plan_prediction_blend", "pose_synchronized_prediction_blend",
                "has_plan_revision", "revision_plan_sequence", "plan_revision_blend_weight", "plan_fading_out", "plan_retention_weight",
        ...
        internal static void AppendFootIkLegValues(List<string> values, RuntimeFootIkLegTraceSnapshot leg)
        {
            values.AddRange(new[]
            {
                ...
                Number(leg.CurrentEventFootPoseWeight), Number(leg.PlanPredictionBlend), Number(leg.PoseSynchronizedPredictionBlend),
                Bool(leg.HasPlanRevision), Number(leg.RevisionPlanSequence), Number(leg.PlanRevisionBlendWeight), Bool(leg.PlanFadingOut), Number(leg.PlanRetentionWeight),
