...
BTSMTL 编辑器 MUST 支持从 `InputActionReference` 或 `InputActionAsset` 拖拽创建输入节点。拖拽创建 MUST 复用 `BaseTreeView.CreateNode()` 和当前图 `CanCreateNodeType()`。InputAction Bool 输入节点 MUST 能在 `TransitionRuleGraph` 中作为 Transition 条件输入来源，MUST NOT 直接作为 `StateMachineGraph` 同层 Transition 条件字段。
