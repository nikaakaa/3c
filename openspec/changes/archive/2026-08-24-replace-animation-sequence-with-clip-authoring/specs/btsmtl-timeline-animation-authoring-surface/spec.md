...
该Requirement随Timeline Analysis provider删除。Timeline不再承载Foot Analysis生成内容，Core中也不保留对应扩展位。

#### Scenario: 旧Analysis provider尝试注册

- **WHEN** provider声明Foot Analysis metric lane
- **THEN** Timeline tool catalog MUST拒绝该能力
- **AND** Track高度与scroll range MUST保持不变
