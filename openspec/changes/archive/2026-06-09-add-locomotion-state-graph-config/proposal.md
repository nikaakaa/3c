...
本变更只推进基础移动四阶段的状态图配置化，不扩展完整角色状态机。目标是让当前 `PlayerLocomotionController -> BasicLocomotionPipeline -> UnityHFSM -> motion executor -> animation presenter` 主链保持不变，同时把状态图从硬编码迁移到 ScriptableObject 配置资产。动画资源映射继续使用 Animancer TransitionLibrary 的 alias 表，不再新增项目侧动画映射资产。
...
- 动画 presenter 根据 `MovementAnimationContext` 生成标准 Animancer alias key，并由 Animancer TransitionLibrary 解析实际 transition。
