...
- [x] 16.1 将路由决策与completion结果拆成独立合同。
- [x] 16.2 从路由决策枚举删除capture与release完成值。
- [x] 16.3 定义独立的capture与release completion outcome。
- [x] 16.4 保证同帧后续路由处理不会覆盖completion outcome。
- [x] 16.5 拒绝同帧同时提交capture与release completion。
- [x] 16.6 保持capture成功后的release permission原子输出。
- [x] 16.7 在Fixture帧时间线分别显示路由决策与completion outcome。
- [x] 16.8 修正Prepared、AwaitingCaptureCompletion与Committed的文档语义。
- [x] 16.9 使用模块正式源码执行Fixture序列闭环。
- [x] 16.10 严格校验OpenSpec change。
