...
### Requirement: Timeline 视口导航不得与内容编辑冲突

Timeline Editor MUST让左侧Track Handle与右侧Track内容共享同一纵向滚动位置，并 MUST把左侧工具栏与右侧时间标尺固定在可见视口顶部。普通滚轮 MUST纵向浏览Track，`Shift+Wheel` MUST横向平移Timeline，`Ctrl+Wheel` MUST缩放Timeline横轴，Middle Drag MUST只横向平移Timeline。内联unbounded Curve的纵向视图 MUST只由`Alt+Wheel`缩放、`Alt+Shift+Wheel`平移；Curve Lane MUST不再抢占Middle Drag或普通滚轮。Point Marker MUST提供独立于细绘制线的稳定命中区域，并 MUST按父Track Timeline坐标拖动。

#### Scenario: 展开多条曲线后浏览顶部Track

- **WHEN** 展开的Marker与Curve组合行总高度超过Timeline视口
- **THEN** 作者 MUST能用普通滚轮纵向浏览全部Track并重新到达最上方Track
- **AND** 左侧Track Handle与右侧Clip、Marker、Curve组合行 MUST保持对齐
- **AND** 工具栏和时间标尺 MUST保持可见

#### Scenario: 在曲线区域导航Timeline

- **WHEN** 指针位于内联Curve Lane且作者使用Middle Drag、普通Wheel、Shift+Wheel或Ctrl+Wheel
- **THEN** 输入 MUST只执行对应Timeline导航，不得同时改变Curve纵向视图
- **AND** 只有带Alt的Wheel手势 MAY改变当前unbounded Curve纵向视图

#### Scenario: 编辑细Marker点

- **WHEN** 作者点击或拖动Point Marker绘制线附近的命中区域
- **THEN** Editor MUST选择同一个Marker并按父Track Timeline坐标更新整数frame草稿
- **AND** `SyncMode=None`时 MUST先引导作者在Inspector配置MarkerGroup，不得执行必然失败的Marker新增mutation
