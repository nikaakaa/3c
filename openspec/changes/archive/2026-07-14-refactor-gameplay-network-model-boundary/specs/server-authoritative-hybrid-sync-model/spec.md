...
系统 MUST 使用 model-owned Character adapter 将 Character facts 映射为 ServerAuthoritative outgoing packets，并将 incoming packets 映射为 Character 语义输入。Adapter MUST 复用 model policy resolver，MUST NOT 回读 Graph、Timeline 或 Animation 结构补齐策略。

#### Scenario: 构造 MotionCommand

- **WHEN** adapter 收到 resolved motion fact 且当前 policy 要求发送
- **THEN** adapter MUST 构造 ServerAuthoritative MotionCommand
