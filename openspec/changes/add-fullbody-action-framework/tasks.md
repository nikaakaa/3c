...
## 9. 手动验证
- [ ] 9.1 Play Mode 中普通 WASD 仍能 Idle、MoveStart、MoveLoop、MoveStop。
- [ ] 9.2 Play Mode 中普通 WASD 仍只通过统一 motion executor 移动。
- [ ] 9.3 Play Mode 中按方向再按 Shift 能进入 Directional。
- [ ] 9.4 Directional active 时基础移动不额外叠加平面位移。
- [ ] 9.5 Directional active 时基础 Locomotion base layer 动画不同时播放。
- [ ] 9.6 Directional 结束后继续按方向能进入 Run 档位。
- [ ] 9.7 松开移动回 Idle 后再次普通移动回 Walk。
- [ ] 9.8 无方向按 Shift 能进入 Backstep。
- [ ] 9.9 Backstep 结束后普通移动不强制 Run。
- [ ] 9.10 动作结束后松开并再次按 Shift 能重新触发。
- [ ] 9.11 动作 active 期间 Look 输入仍响应。
