...
## Decision 3: 接触事实来自最终姿势的脚速与表面距离，不复制Gait Phase

每只脚在VisualRoot局部空间计算：

```text
RelativeAnkleVelocity
RelativeToeVelocity
VerticalVelocity
PlanarVelocity
...
- 收益：这是TLOU公开采用的成熟后处理思路，能够直接处理crossfade和非locomotion动画，并避免两份foot phase权威。
- 代价：它不能像离线完整脚轨迹那样提前知道很远的精确落脚时刻。第一版通过有限look-ahead和地面路径预测补足，不建立尚未批准的动画bake系统。
...
预测不直接把脚拉向角色前方。每帧先计算当前动画脚在VisualRoot局部空间的位置和速度，再计算有限预测：

```text
PredictionTime = clamp(estimated time to support, MinLookAhead, MaxLookAhead)
PredictedRootPose = extrapolate visible body pose by visible linear/yaw velocity
PredictedFootLocal = CurrentFootLocal + RelativeFootVelocity * PredictionTime
...
每只脚拥有固定容量query workspace。每帧针对当前sole、heel、toe、路径中点和predicted footprint执行NonAlloc Sphere/Capsule查询，并按路径fraction稳定排序候选。
...
通过过滤的候选形成从当前foot到predicted footprint的分段support envelope。目标脚底不得低于该envelope；前方envelope抬升时，Free脚按PredictionWeight增加有限swing clearance。最终plant选择路径末端最近、可达且法向合法的surface。
...
Plant/release使用连续solve weight和half-life，不增加Planting/Releasing伪状态。状态变化保存结构化reason用于diagnostics。
...
骨盆offset使用presentation delta和显式half-life推进临界阻尼，分别限制最大上移、最大下移和每秒变化。第一版只修改pelvis局部垂直位置，不旋转pelvis、spine或VisualRoot。
...
1. 应用planner给出的pelvis局部垂直offset。
