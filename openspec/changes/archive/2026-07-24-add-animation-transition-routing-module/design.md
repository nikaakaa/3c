...
`TransitionRouteDecisionKind`只表达本帧要求调用方执行的路由动作。CompletionOutcome独立表达调用方回报事实经过校验后的结果。Capture completion必须先产生release permission，release completion只能在后续Frame回报；同一Frame同时提交两种completion属于无效输入。后续规则处理不得覆盖已经产生的completion outcome。
...
3. 后续Frame尚未收到capture completion时进入`AwaitingCaptureCompletion`。
4. 下一份Frame已经携带匹配request generation的capture completion时允许从`Prepared`直接进入`Committed`，不得为了观察等待状态强制增加一帧延迟。
5. 从`Prepared`或`AwaitingCaptureCompletion`收到匹配completion后进入`Committed`并允许调用方释放旧source。
6. completion identity不匹配时进入`Invalid`，不得发出release许可。
