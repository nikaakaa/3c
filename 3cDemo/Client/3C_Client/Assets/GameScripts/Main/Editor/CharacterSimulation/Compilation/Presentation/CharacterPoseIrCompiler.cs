...
namespace ThirdPersonCharacter.Editor.CharacterSimulation
{
    ...
    internal sealed class
        CharacterSequencePlayerPoseCompilerHandler :
            CharacterPoseCompilerHandler<
                CharacterSequencePlayerPosePayload>
    {
        public override CharacterPoseNodeKind Kind =>
            CharacterPoseNodeKind.SequencePlayer;
        public override CharacterPoseOperationCode Code =>
            CharacterPoseOperationCode.SequencePlayer;
        public override bool Player => true;
        public override bool SequencePlayer => true;
        protected override CharacterPresentationPoseSourceSlot GetSource(
        ...
        public override CharacterPoseNodePayload CreatePayload(
            CharacterPoseAuthoringPayloadInput input) =>
            new CharacterSequencePlayerPosePayload(
                input.Require<CharacterSequencePoseSourceSlot>(
                    "pose-source-slot"),
                input.Require<bool>("loop"),
                input.Require<float>("play-rate"),
                input.Require<float>("initial-time"),
                Enum.Parse<CharacterSequencePlayerClockSource>(
            ...
            string sourcePath)
        {
            CharacterPoseCompilerHandlerValidation.Require(
                payload.SourceSlot,
                sourcePath,
                "Sequence source identity is missing.");
            CharacterPoseCompilerHandlerValidation.Require(
                float.IsFinite(payload.PlayRate) &&
                payload.PlayRate > 0f,
                sourcePath,
                "Sequence play rate must be finite and positive.");
            CharacterPoseCompilerHandlerValidation.Require(
                float.IsFinite(payload.InitialTime) &&
                payload.InitialTime >= 0f,
                sourcePath,
                "Sequence initial time must be finite and non-negative.");
            CharacterPoseCompilerHandlerValidation.Require(
                Enum.IsDefined(typeof(CharacterSequencePlayerClockSource), payload.ClockSource),
                sourcePath,
                "Sequence clock source is invalid.");
        }
    }
