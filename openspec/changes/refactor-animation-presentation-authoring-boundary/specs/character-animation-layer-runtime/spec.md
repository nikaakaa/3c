...
**Reason**: 该合同把多个候选 contribution 和 Priority 暴露给动画层，导致表现层重新做业务选择。新合同只接受已解析 selection 与 producer sample。

#### Scenario: 删除候选贡献

- **WHEN** 迁移完成
- **THEN** Animation 模块 MUST不再接收跨 producer Priority contribution 列表

### Requirement: 动画层运行时负责仲裁

**Reason**: producer 胜负属于逻辑层；Animation 模块不再拥有 Arbitrator、authority 或 DesiredCandidate。

#### Scenario: Action 覆盖 Locomotion

- **WHEN** Action 获得动作所有权
- **THEN** 逻辑层 MUST直接选择 Action playback
- **AND** Animation 模块 MUST不比较 Action 与 Locomotion Priority

### Requirement: Animancer 只是最终播放 adapter

**Reason**: Animancer 应拥有实际 state/layer/fade 混合，而不是被当作项目自算权重的输出端。

#### Scenario: 删除项目自算权重

- **WHEN** target 开始播放
- **THEN** 项目 MUST调用 Animancer 正式 Play/Fade
- **AND** MUST不提交自算 LayerPlan

### Requirement: 状态切换混合必须使用 owner handoff 的 Registry 真相

**Reason**: owner/Driver/Registry handoff 是逻辑与动画耦合路径，改由逻辑 selection 和播放生命周期替代。

#### Scenario: 状态切换

- **WHEN** State 发生切换
- **THEN** Animation 模块 MUST只观察最终 selection 与 sample

### Requirement: 统一动画贡献注册表必须拥有播放实例生命周期

**Reason**: 候选 Registry 与播放生命周期职责混合。新 AnimationPlaybackLifecycle 只维护已选 producer。

#### Scenario: 删除 Registry 仲裁语义

- **WHEN** producer release
- **THEN** lifecycle MUST按 Current/Outgoing/Retired 处理
- **AND** MUST不维护候选 membership authority

### Requirement: 动画仲裁必须先归并连续因果链

**Reason**: Animation 模块不再解释 Tree 因果链或 Driver。

#### Scenario: 快速逻辑切换

- **WHEN** 一个表现帧前发生多个状态切换
- **THEN** 逻辑层 MUST输出该层最终 selection
- **AND** Animation 模块 MUST不构建 causal component

### Requirement: 每个动画层最多拥有一个 Active Handoff

**Reason**: ActiveHandoff 是项目自制 transition 状态机；重入由 Animancer 当前视觉图处理。

#### Scenario: fade 重入

- **WHEN** fade 中再次选择新 target
- **THEN** Adapter MUST调用 Animancer 播放新 target
- **AND** MUST不创建 handoff stack

### Requirement: 最终动画输出必须区分 LayerWeight 与 StateWeight

**Reason**: layer/state weight 属于 Animancer 内部播放图，不再由项目输出合同重复表达。

#### Scenario: 应用权重

- **WHEN** Animancer 更新 layer
- **THEN** 最终权重 MUST由 Animancer state/layer graph 计算

### Requirement: 动画仲裁内部必须按状态所有权分层并保持单一权威

**Reason**: 项目不再拥有动画 Arbitrator、Topology Ledger、CandidateAllocator 或 HandoffResolver。

#### Scenario: 删除仲裁模块

- **WHEN** PresentationFrame 处理动画
- **THEN** 调用链 MUST不经过 Arbitration 目录

### Requirement: 动画播放执行必须区分多层协调与单层播放状态

**Reason**: 自制多层协调和单层播放状态都由 Animancer layer/state graph 与轻量 producer lifecycle 替代。

#### Scenario: 多层播放

- **WHEN** 角色存在多个正式 layer
- **THEN** 每层 selection MUST独立提交
- **AND** Animancer MUST负责 layer 合成
