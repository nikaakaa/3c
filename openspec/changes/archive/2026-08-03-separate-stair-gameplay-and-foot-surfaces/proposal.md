...
因此本change建立一条正式且唯一的楼梯表面所有权：每段单调连续梯段使用持久化Gameplay Traversal Ramp，Foot Placement使用持久化真实踏面查询Collider；二者由同一梯段作者绑定显式关联，但不得被同一个运行系统同时消费。现有Low、High与OverLimit路线都包含上行、顶平台和下行，因此每条路线拆成上行与下行两个作者绑定，共六个稳定Ramp身份；顶平台继续作为普通`Ground`。

## What Changes

- 新增`StairTraversalSurfaceAuthoring`作为连续楼梯的唯一作者绑定：
  - 一个绑定只表达从Lower到Upper的单调上升梯段；包含上行和下行的往返路线必须拆成两个绑定。
  - 保存稳定Stair identity。
...
- 迁移Gameplay Lab连续楼梯：
  - LowStairs、HighStairs与连续OverLimitStairs的上行、下行Gameplay碰撞分别改为正式Traversal Ramp，共六条Ramp。
