...
上一Committed Surface、Frame、Authority Tick、Trajectory Generation、Future Translation Source、Foot State、Residual与查询输出不得进入Observation Key。上一Surface不得传入World Query或改变候选选择；历史只在Observation之后通过独立`LandingAcceptanceDistance`决定是否替换NextSwingLanding。1毫米Key量化定义世界查询输入是否相同，Landing接受距离定义新Observation是否改变正式Landing与Ground Path，两者不得合并。`LandingAcceptanceDistance`不得同时控制Path Revision、Residual截止或Release完成。
