    ...
    static class CharacterRuntimeDiagnosticsInspector
    {
        ...
        static void AppendFootIkLegHeader(List<string> values, string prefix)
        {
            string[] beforePath =
            {
                ...
                "has_plan_revision", "revision_plan_sequence", "plan_revision_blend_weight", "plan_transition_kind", "plan_fading_out", "plan_retention_weight",
        ...
        internal static void AppendFootIkLegValues(List<string> values, RuntimeFootIkLegTraceSnapshot leg)
        {
            values.AddRange(new[]
            {
                ...
                Bool(leg.HasPlanRevision), Number(leg.RevisionPlanSequence), Number(leg.PlanRevisionBlendWeight), leg.PlanTransitionKind, Bool(leg.PlanFadingOut), Number(leg.PlanRetentionWeight),
