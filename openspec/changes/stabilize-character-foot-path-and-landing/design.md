...
| `0x171D6920`为每脚目标高度保存历史并执行`delta=clamp(target-history, ±rate×dt)` | 每脚唯一Target Height历史从`Ground Envelope + Formal Foot Height`的Swing目标连续进入Approach与Plant；新增独立`MaximumVerticalTargetSpeed`，同Event目标换代、Contact Verification与Reentry继续该历史 | Interpolation Runtime Target Height Stage |
