...
用户批准从112611开始对齐输入、筛选ZZZ共同候选，并在候选有实际价值时进入独立实现与同输入回放。c3166ff／676fd5c／9a2db28完成132307回放及根任务／独立Diagnostics核对后不接纳：目标层有收益，但移动平地更低且后段既有翻侧峰值加大。1b10e81／ccb14e1已撤销，141106已动态确认恢复。112611只是撤销对照，不表示它是最好版本。
...
独立Diagnostics复核与根任务无矛盾：硬Clamp行173→146，但向上超过50毫米9→11；993–998窗口峰值228.396→272.241毫米、累计膝点路程502.650→590.173毫米。14.4验证已完成，不等于质量通过；本轮不叠加新半径、响应或Bend变量。
...
## 最终处置：撤销这一适配，不否定ZZZ选择结构

按同输入回放工作流，将“减少一部分突降”与“没有引入新坏窗”分开裁决。平地更深及R995／996的集中放大属于本次真实代价，不能用37项评分不变或全局反转次数下降抵消。本候选不作为下一步已经通过的基线。

- Runtime仅恢复c3166ff的Builder、Module、Profile及Corin配置四路径；0.2米新配置和公开候选API随本次删除，不保留兼容分支。原Foot修复、3Hz、20毫米及速度门均保留。
- 三个生成产物精确恢复9a2db28前的正式版本；这七个代码／配置／产品路径对d51e247无内容diff。676fd5c由原Diagnostics owner精确撤销为ccb14e1，回到66／35。
- 活跃proposal／design／delta恢复当前原公式，失败模型、候选提交、132307及原字节Proof继续保存。代码相同不代替恢复证明，仍需完整Editor编译、显式加载及同Record恢复Replay。
- 新证据只排除“当前Ankle位移适配＋完整几何选择就足以解决所有骨盆下陷”。它没有否定2F40／97B0的已证数学，也没有授权删掉所有Reach或把ZZZ的g／k混进Pelvis。

并行窗口补充了用户指定的旧代码参照91758ff7da8bc18f8716218a1ea13ec9c000061c：旧Primary-only硬区间及Release无Reach，与当前无Primary仍受双Foot Request限制确实不同。该版本的同步采样尚未定位，不能替代本轮112611的同输入A/B，也不把更早Lyra CSV冒充旧包。后续应先处理这条准入／交接差异，不重复本轮已失败的目标适配；本记录不新增该方向的实施授权。

## 141106恢复闭环

恢复编译由同一Diagnostics owner完成一次完整Editor依赖构建：57个既有／依赖警告、0错误、107.08秒，finally立即shutdown。Root随后显式Refresh，域重载恢复后Console零错误，通过原Record完成20260831-141106-877-437b93da1dd04804b2191935bdda390b；后台分析与正式发布结束后退出本任务Play。

与112611逐行对账：1043帧／2086脚行、1224列，1200个非身份列逐值相同，24个差异列都是本次实例／Surface／Path身份；Foot、骨盆、Solved Knee及状态回到原值。37项规则／计数／score相同，1044个完整Proof帧、Runtime Identity、输入／Body／起点／时钟均相同。

当前官方Proof选择132307为前驱，依然报告7个产品身份差异、Frame分歧0；它没有官方对112611的matched=true。独立恢复证据与官方结果并列保存，不放行比较器。恢复不等于骨盆问题已解决：420仍下降80.210毫米，世界大步仍33，平地和既有反弯仍在。

恢复包另存pelvis-proximity-restored-141106，14原文件和原字节Proof逐流核SHA，原目录和132307候选都不删除。samples SHA256=340b543b0254f3ccf30bab0a8c3ebdc97040af50695510a976eada3e5cd9d587；run ZIP=da37090de09688ac50ec462be67c6095aaae371135245e765018d46e59659082；原Proof=fc102def8cd2c3c8024f00a4c87efe65e3dbd29cec2893fe8a56ac0a754b4523；原Proof ZIP=96a1c85b78b0dd2adc7c9e7192548f4515fb6d143b8cd12b5e470aedb47b6cf5。
