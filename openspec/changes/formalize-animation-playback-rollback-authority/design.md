...
这条路径要满足预测回滚，关键输入必须在 restore/replay 时完全一致。F6 失败日志显示，restore 后第一次 replay 分叉点不是 position/yaw，而是 `animationNormalizedTime` 和 `blackboard.animation.locomotionNormalizedTime` 从历史中段变成 `0`。这说明状态机恢复到了 TurnBack，但动画播放进度权威仍可能被表现层的播放/重启逻辑覆盖。

## Goals

- 明确影响 simulation 输出的动画播放进度属于可回滚 simulation state。
- 区分首次进入 state 的播放归零和 rollback restore 的中段 resume。
- 保证 `TickSampledMotion` 的 profile sampling window 在 restore 后恢复，而不是从 0 重新采样。
...
- 不把视觉 blend 权重全部纳入 simulation；只有影响 profile delta/yaw 或 facts 的数据才进入回滚状态。
- 不把所有 Action/Attack 一次性迁移；本设计要求能力可复用，TurnBack 是第一验收对象。
- 不重写状态机或 FullBody 主线。

## Decisions

### Decision: Animation playback clock 由 simulation restore state 掌权

对 `TickSampledMotion` 状态，normalized time 不是纯表现层细节。它必须由 snapshot 或可确定性 runtime state 表达，并在 replay 中作为 profile sampling window 的权威输入。

Presenter 可以提供只读进度，也可以在 restore 时被要求 seek 到某个进度，但它不能在 restore 后把同一 alias 当作首次进入重播并归零。
...
rollback restore 到 TurnBack 中段：
...
这不是 F6 特例，而是所有 profile-driven 状态的通用规则。
...
### Decision: Presenter restart 只响应真实新播放

基础移动 Presenter 可以继续避免重复播放同一 alias。对于 TurnBack 这种 one-shot，restart 只能发生在 phase/alias 的真实新进入，或明确检测到播放段不连续且不是 rollback restore resume。
...
实现时应先用可控 fake playback source / fake presenter 建立确定性测试，再补真实 `BasicLocomotionAnimancerPresenter` 的 restore 行为测试。测试必须覆盖：
...
- 风险：只修 TurnBack 会留下 Action root-motion profile 的同类问题。
  - Mitigation: 规格使用通用动画播放权威和采样窗口术语，TurnBack 只是第一验收状态。
- 风险：自动测试使用 fake presenter 掩盖真实 Animancer 行为。
  - Mitigation: 必须增加真实 `BasicLocomotionAnimancerPresenter` 层级的恢复语义测试或静态验证，并保留 F6 手动验收。

## Migration Plan

1. 审计当前 snapshot、runtime blackboard、locomotion runtime state 和 presenter playback restore 的字段。
2. 明确 current playback progress 与 previous motion playback progress 的 capture/restore 顺序。
...
- Action/Attack 的未来 profile-driven motion 是否直接复用 Locomotion playback progress 结构，还是抽象出通用 animation motion playback runtime state；本变更只要求 TurnBack 路径不能写成专用旁路。
