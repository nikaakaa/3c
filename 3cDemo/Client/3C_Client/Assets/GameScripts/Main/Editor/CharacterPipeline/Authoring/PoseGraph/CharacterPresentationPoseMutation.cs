...
namespace ThirdPersonCharacter.Pipeline.Editor
{
    ...
    static class CharacterPosePayloadFieldMutation
    {
            ...
            IReadOnlyList<SetPoseNodeFieldMutation> fields)
        {
            ...
            if (payload is CharacterSequencePlayerPosePayload sequence)
                return SetSequence(sequence, fields);
        ...
        static CharacterPoseNodePayload SetSequence(CharacterSequencePlayerPosePayload current, string field, object value)
        {
            CharacterSequencePlayerClockSource clockSource = field == "clock-source"
                ? EnumValue<CharacterSequencePlayerClockSource>(value)
                : current.ClockSource;
            return new CharacterSequencePlayerPosePayload(
                field == "pose-source-slot" ? Require<CharacterSequencePoseSourceSlot>(value, field) : current.SourceSlot,
                field == "loop" ? Convert.ToBoolean(value) : current.Loop,
                field == "play-rate" ? Convert.ToSingle(value) : current.PlayRate,
                field == "initial-time" ? Convert.ToSingle(value) : current.InitialTime,
                clockSource);
        }

        static CharacterPoseNodePayload SetSequence(
            CharacterSequencePlayerPosePayload current,
            IReadOnlyList<SetPoseNodeFieldMutation> fields)
        {
            var values = fields.ToDictionary(value => value.FieldId, value => value.Value, StringComparer.Ordinal);
            CharacterSequencePlayerClockSource clockSource = values.TryGetValue("clock-source", out object clockSourceValue)
                ? EnumValue<CharacterSequencePlayerClockSource>(clockSourceValue)
                : current.ClockSource;
            return new CharacterSequencePlayerPosePayload(
                values.TryGetValue("pose-source-slot", out object source)
                    ? Require<CharacterSequencePoseSourceSlot>(source, "pose-source-slot")
                    : current.SourceSlot,
                values.TryGetValue("loop", out object loop) ? Convert.ToBoolean(loop) : current.Loop,
