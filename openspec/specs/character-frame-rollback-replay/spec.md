# character-frame-rollback-replay Specification

## Purpose
定义 Character frame 回滚重放主线、输入帧回灌、状态 Capture/Restore、runtime facts 收敛和 Fantasy 接入前的边界。
## Requirements
### Requirement: Character frame 回滚重放主入口
系统 MUST 提供 Character frame 回滚重放能力，使本地 synctest 的 replay 能通过当前 Character frame pipeline 主线推进动作、移动、动作事实和动画事实。该能力 MUST 复用现有 `CharacterRuntimeCore`、`CharacterFrameRuntimeController`、InputRequestBuffer、Locomotion graph 和 Action lifecycle，不得新增第二套角色控制器、第二套 gameplay tick 入口或默认 mixed 状态图。
...
- **AND** MUST 调用 Character frame pipeline 或等价当前正式 Character frame 主线
- **AND** MUST NOT 只调用 `PlayerLocomotionController.Tick(...)` 作为 Character frame replay 的最终路径
...
- **THEN** 系统 MAY 继续只通过 Movement/Locomotion runtime module 重放
- **AND** 该 adapter MUST 明确标识为 locomotion-only，不得作为 Sandbox 动作 demo 的完整回滚验收

#### Scenario: 不创建分裂控制路径
- **WHEN** Character frame replay 推进角色
...
- **WHEN** Character frame replay 推进 tick N
...
### Requirement: Character frame 状态 Capture/Restore
系统 MUST 定义 Character frame 运行时状态的纯数据 capture/restore 边界，使 replay 从历史 tick 恢复时，Locomotion graph restore state、Action lifecycle restore state、input buffer restore state、pending frame facts 和影响下一 tick 输出的事实能回到快照时刻。restore state MUST NOT 保存 Unity Object、Animancer runtime object、Animator、AnimationClip、InputAction 或场景实例引用。
...
- **GIVEN** 系统持有 tick N 的 Character frame restore state
...
#### Scenario: 不污染输入历史
- **WHEN** replay 恢复 Character frame 状态
- **THEN** 系统 MUST NOT 将动作消费结果写回 `PredictionInputHistory`
- **AND** 后续 replay MUST 仍从原始输入帧重新推导动作请求

### Requirement: Character frame Runtime Facts 收敛
系统 MUST 在 Character frame replay 中重新写入 action facts、locomotion facts、animation facts 和 runtime blackboard facts，使同一输入序列重放后的最终快照能与原始快照在定义容差内比较。Action facts MUST 从 Action lifecycle 或 action output 派生；Locomotion facts MUST 从 Movement module 派生；若 animation facts 仍受表现层播放进度影响，系统 MUST 输出字段级 differences 以定位缺口。

#### Scenario: Action facts 重放收敛
- **GIVEN** 原始运行中 Dodge 请求被 Character frame 主线接受
...
### Requirement: Debug Runner Character frame 验证
系统 MUST 让 Play Mode debug runner 可用 Character frame replay adapter 执行本地 synctest，并保持安全探针和可见 correction 两种调试语义。
...
#### Scenario: Character frame differences 可读
- **WHEN** Character frame synctest 失败
...
系统 MUST 将本变更限制为本地 Character frame replay 一致性，不得在本变更中接入真实 Fantasy 网络、修改协议文件或实现高延迟模拟器。该变更完成后，后续 MAY 单独规划本地 latency/reconciliation simulator，再将 transport 替换为 Fantasy。

#### Scenario: 不修改 Fantasy 协议
- **WHEN** 实施 Character frame replay
...
- **WHEN** 规划本地高延迟预测回滚模拟器
- **THEN** 该规划 MUST 以 Character frame replay 在 Move/Run/Dodge 上可诊断收敛为前置条件

### Requirement: Character frame 可回滚状态完整性审计
系统 MUST 审计并覆盖 Character frame replay 中所有会影响下一 tick 输出的纯数据状态。至少包括 Character frame active state、state time、pending transition、input buffer restore state、locomotion runtime state、runtime blackboard、animation facts、profile sampling window、motion root pose 和 camera basis override。

#### Scenario: 捕获影响下一 Tick 的状态
- **WHEN** tick N 的 `CharacterSimulationSnapshot` 被创建
- **THEN** 快照 MUST 包含或可确定性重建 tick N+1 推进所需的 Character frame、Locomotion、action、animation 和 motion facts
...
系统 MUST 能用严格 synctest 验证 profile 驱动的 Locomotion 动画状态。TurnBack EntryLocal 或等价 profile 驱动状态 MUST 通过正式 Character frame replay 主线恢复、重放和比较，不得在测试中直接绕过主线采样器。
...
- **THEN** 测试 MUST 通过 `CharacterFrameRollbackSimulation` 或等价 Character frame replay adapter 推进
...
系统 MUST 将 Unity Animator runtime delta 视为非确定性表现来源，不能作为预测回滚严格验收的唯一基准。需要回滚的动画位移 MUST 通过 tick 驱动 profile、纯数据曲线或等价确定性 motion source 验收。
...
#### Scenario: AnimatorDirect 只作为本地表现预览
- **GIVEN** 某动画只能暂时通过 AnimatorDirect 播放
- **WHEN** 它不参与预测回滚验收
- **THEN** 系统 MAY 将该模式限制为 presentation-only 或调试预览能力
- **AND** 文档 MUST 标明它不提供本地预测确定性保证
- **AND** 它 MUST NOT 成为 gameplay motion、profile-driven motion 或 rollback scope 的正式运动来源

### Requirement: Replay 复用 Character Frame Pipeline
系统 MUST 让 Character frame replay、synctest 和本地高延迟校正复用 live gameplay 使用的 `CharacterFramePipelineHost -> CharacterFramePipeline` 主线。Replay adapter MAY 从 `PredictionInputFrame` 构造 pipeline 输入，但 MUST NOT 通过另一套手工顺序直接拼接 input buffer、controller Tick、状态恢复和动画播放事实。Replay adapter MUST NOT 直接创建 `CharacterFramePipeline`、legacy FullBody submitter、第二 runner、第二 motion executor 或第二 animation presenter。
...
- **AND** replay MUST NOT 直接调用 `BasicLocomotionPipeline` 作为 Character frame 最终路径

#### Scenario: Replay 快照来自 Pipeline 结果
- **WHEN** replay 推进 tick N 后捕获快照
- **THEN** 快照 MUST 来自同一 `CharacterFramePipeline` 写入的 Character frame 状态、runtime blackboard、input buffer restore state 和 motion executor restore state
- **AND** 快照 recorder MUST 不需要额外 enrich 一条独立 Character frame gameplay truth

#### Scenario: Replay 不创建分裂持有者
- **WHEN** Character frame replay 或 synctest 构造推进入口
- **THEN** replay MUST 使用角色正式 `CharacterFramePipelineHost`
- **AND** MUST NOT 为 replay 单独创建第二个 `CharacterFramePipeline`
- **AND** MUST NOT 直接调用 legacy FullBody submitter 具体实现来绕过 host
...
- **WHEN** Character frame synctest 发现原始运行和 replay 不一致
...
- **WHEN** 检查 Character frame rollback snapshot
...
系统 MUST 使用 `RollbackCameraBasisState` 作为 replay 中 camera-relative 输入解算的唯一快照事实，并 MUST 删除或内联独立 `CameraYaw` 字段。真实 camera yaw/pitch、FreeLook 轴和 Main Camera transform MUST 保持 local-only。
...
#### Scenario: 删除 CameraYaw 独立语义
- **WHEN** 测试和日志已经改为读取 `CameraBasisState.Yaw`
- **THEN** 系统 MUST 删除 `CharacterSimulationSnapshot.CameraYaw` 或将其内联为 `CameraBasisState.Yaw` 的只读派生视图
- **AND** MUST 删除 `WithCameraYaw()` 并改用 `WithCameraBasis(...)`

#### Scenario: 静态验证无真实相机 rollback
- **WHEN** 运行静态边界验证
- **THEN** 系统 MUST 证明 Character frame rollback capture/restore 不调用真实 camera capture/restore
- **AND** MUST 证明不存在 `ThirdPersonCameraRollbackState` 或等价真实相机 rollback 状态

### Requirement: Character frame Restore 状态去诊断污染
系统 MUST 区分影响 replay 输出的 Character frame gameplay restore state 与只影响日志去重或调试显示的 diagnostic restore state。默认 snapshot comparison MUST 关注 gameplay facts，诊断字段不得导致本地 synctest 误判为 simulation mismatch。

#### Scenario: Gameplay restore 保留下一 tick 所需事实
- **WHEN** Committed Action 状态被 capture
...
### Requirement: Character frame replay 使用权威域比较
Character frame rollback replay MUST 使用预测回滚权威域和比较域判断 replay 结果。Character frame 状态机、Action gameplay facts、Locomotion gameplay facts、motion executor state 和 profile-driven motion facts MUST 属于 strict gameplay；纯视觉动画播放漂移 MUST 可诊断但 MUST NOT 单独导致 Character frame replay 失败。

#### Scenario: Character frame gameplay mismatch 仍失败
- **GIVEN** Character frame replay 后 action state 或 locomotion state 不一致
- **WHEN** 本地 synctest 比较快照
- **THEN** 结果 MUST 包含 strict differences
- **AND** Character frame replay MUST 失败

#### Scenario: 视觉 animation drift 不阻塞 Character frame replay
- **GIVEN** Character frame replay 后只有 Action animation normalized time 或 MoveLoop visual playback time 不一致
- **WHEN** 本地 synctest 比较快照
- **THEN** 结果 MUST 保留 presentation differences
- **AND** Character frame replay MAY 成功

#### Scenario: Profile-driven motion 仍严格
- **GIVEN** Character frame replay 期间处于 TurnBack 或等价 profile-driven motion 状态
- **WHEN** playback window、profile delta、root position 或 yaw 不一致
- **THEN** 结果 MUST 包含 strict differences
- **AND** Character frame replay MUST 失败

### Requirement: Character frame 不写业务类型特判
Character frame replay MUST NOT 因项目暂时偏向 MOBA/MMO 或格斗而写死一套业务类型分支。业务差异 MUST 通过状态 policy、timeline facts、motion source 或 compare scope 声明表达。
...
- **WHEN** 正式 Corin 角色 prefab 或正式场景实例未挂载 Character frame replay adapter
...
- **WHEN** Character frame replay adapter 执行 capture、restore 或 advance
...
- **WHEN** EditMode 测试需要验证 Character frame replay
- **THEN** 测试 MAY 在 fixture 中临时创建 Character frame replay adapter
...
系统 MUST 在 Character frame replay 和本地回滚中保持 Directional Dodge 完成后的 Run latch 行为确定。Run latch MUST 作为 Locomotion runtime state capture/restore 的一部分参与比较；Action lifecycle restore 只恢复动作状态，不得用默认 graph active `Action.Dodge` 或 Action facts 代替 Run latch。
...
- **WHEN** 系统 capture tick N 的 Character frame restore state
...
Character frame rollback replay MUST 将 Action motion resolver result 视为 strict gameplay facts 的一部分。预测路径和正式路径 MUST 使用同一 action motion spec 与 resolver 输入，产出一致的 movement command、completed 和 run latch 派生。
...
### Requirement: 声明为 Profile-driven 的播放窗口回滚收敛
Character frame rollback replay MUST 保证声明使用 profile-driven motion、`TickSampledMotion`、root motion profile 或等价 sampled motion 的动画状态从历史中段恢复后，playback window 和 profile sampling window 与原始运行逐 tick 收敛。恢复同一 tick 后，replay MUST 不把该 sampled motion active state 的动画当作新进入重新归零；最终 snapshot 比较 MUST 覆盖 sampled motion animation progress、runtime blackboard animation facts、motion executor root pose、position 和 yaw。

#### Scenario: TurnBack 中段恢复不重启动画
- **GIVEN** 原始运行在 tick N 处处于 `Locomotion.TurnBack`
- **AND** `Locomotion.TurnBack` 声明使用 `TickSampledMotion`
...
#### Scenario: 纯表现动画不参与收敛断言
- **GIVEN** 某动画未声明为 sampled motion、profile-driven motion、warp window 或 window facts 的输入
- **WHEN** 本地 synctest 从历史 tick 恢复并重放
- **THEN** replay MAY 不要求该动画 visual normalized time 与历史逐 tick 一致
- **AND** snapshot 比较 MUST NOT 因纯表现 normalized time 差异而判定 simulation 分叉
- **AND** 如果该动画进度影响了 motion facts、runtime blackboard 权威事实或角色 root pose，测试 MUST 失败

### Requirement: 回滚修复不得产生分裂路径
实现 profile playback rollback 收敛时，系统 MUST 继续通过 `CharacterFrameRollbackSimulation` 或批准的 rollback adapter、`CharacterFrameRuntimeController`、`CharacterRuntimeCore`、runtime blackboard 和正式 motion executor 主线推进。实现 MUST NOT 为 TurnBack 创建 F6 专用重放路径、Presenter 专用运动路径、直接 sampler 测试通道或 Animator root delta fallback。

#### Scenario: Replay 走角色帧主线
- **WHEN** 自动测试验证 TurnBack 中段恢复
- **THEN** 测试 MUST 通过 `CharacterFrameRollbackSimulation.Advance` 或等价 rollback adapter 推进正式角色帧 runtime
