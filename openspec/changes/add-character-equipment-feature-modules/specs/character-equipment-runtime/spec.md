...
系统 MUST使用`PendingEquipmentChange`表达Begin、Committed、Cancelled状态，并把ChangeId、Slot、from/to Equipment、source ActionInstance、begin/commit tick保存进typed state。Begin MUST只创建候选pending记录；Timeline TreeClip MAY在明确帧提交Commit operation；Cancel/Action abort MUST显式撤销未提交记录。系统 MUST不按动画normalized time、Renderer状态或Timeline结束猜测commit。

#### Scenario: 换装前摇被打断

- **WHEN** Equip Action在commit TreeClip之前被Dodge取消
- **THEN** Pending change MUST变为Cancelled并从state移除
