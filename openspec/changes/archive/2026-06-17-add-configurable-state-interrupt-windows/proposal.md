...
本变更只收口数据模型和配置边界：窗口负责定义 timing 并产出纯数据 timeline facts；请求策略负责描述 from/target、priority、resistance、force 和 required fact id。单帧 facts 权威、transition evaluator 拆分和 Action motion output 已由当前 specs 的稳定合同表达，不在这里继续膨胀。
...
- Current spec boundaries:
  - 后续 Attack、Skill 或 HitReact 实现必须复用本变更的 window/request 数据边界。
  - `animation-phase-timeline-facts` 提供 current/projected/target facts 的稳定语义。
  - `unified-character-state-machine` 提供 evaluator extension 边界。
  - `fullbody-action-framework` 与 `character-runtime-blackboard` 提供 action motion output 和 facts 消费边界。
