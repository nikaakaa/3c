...
恢复提交a6eceac已通过正常Unity编译及`20260831-233436-894-d1564c7fa0b442f6aef02bb470ca0b1b`回放。恢复包为facts71/diagnosis40，1043输出帧、2086脚行、1215列、67186几何行；官方Proof对前次232702匹配1044输入帧，持久副本已保留。独立对205014逐行比较：只有24列采样/实例/Surface/Path身份变化，映射均无冲突，其余1191列逐值一致，包括实际Foot、Solved Knee和Pelvis。42项规则、计数、覆盖全部恢复，穿透19/84、接触间隙13/60、分数61.9；不再保留本轮Runtime或Diagnostics候选。

恢复编译期间唯一Import Code4来自并行GM文件写入时间变动，Editor随后重新导入成功；恢复Replay无错误并完整封口，未改GM代码。两轮Unity编译各留下的单个闲置Roslyn已核实身份并清理，没有额外.NET build，最后复查.NET/MSBuild残留为0。失败包232702、恢复包233436及205014基线均原地保留。
