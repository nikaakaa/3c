...
## Why
当前源码和 proposal 语言里同时存在 `CharacterGraphDefinition`、`CharacterExecutionNodeTree`、旧 submitter graph/chain、`ActionBranch` 等名称。它们职责不同，但都容易被理解成“正式行为图”。这会让后续目标混淆：behavior submission runner 不是 authoring graph，authoring graph 不是 runtime runner，ActionBranch 也不是顶层行为二分。
...
旧 `CharacterFrameSubmitterGraph` / `CharacterFrameSubmitterChain` 退役归属 `refactor-character-submitter-chain-boundary`，本变更只消费其结果。

## Acceptance Criteria
- 生产代码中不再把 behavior submission runner 命名为 graph 或 chain。
...
## Dependencies
- SHOULD 在 `refactor-character-submitter-chain-boundary` 后实施，避免重复处理旧 submitter graph/chain 退役。
