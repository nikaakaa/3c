...
namespace ThirdPersonCharacter.Equipment
{
	...
	public sealed class CharacterEquipmentRigBindingCatalog : MonoBehaviour
	{
	    ...
	    {
	        bool valid = true;
	        var rigIds = new HashSet<string>(StringComparer.Ordinal);
	        var catalogRenderers = new HashSet<Renderer>();
	        for (int i = 0; i < RigObjects.Count; i++)
	        {
	            EquipmentRigObjectBinding rig = RigObjects[i];
	            if (rig == null || string.IsNullOrEmpty(rig.RigBindingId) || !rigIds.Add(rig.RigBindingId))
	            {
	                errors?.Add($"{name}: Equipment Rig Binding #{i} is missing or duplicated.");
	                valid = false;
	                continue;
	            }
	            if (rig.RendererBindingIds.Count == 0 || rig.RendererBindingIds.Count != rig.Renderers.Count)
	            {
	                errors?.Add($"{name}: Equipment Rig Binding '{rig.RigBindingId}' Renderer identities and references do not align.");
	                valid = false;
	                continue;
	            }
	            var rendererIds = new HashSet<string>(StringComparer.Ordinal);
	            var renderers = new HashSet<Renderer>();
	            for (int rendererIndex = 0; rendererIndex < rig.RendererBindingIds.Count; rendererIndex++)
	            {
	                string rendererId = EquipmentSlotDefinition.Normalize(rig.RendererBindingIds[rendererIndex]);
	                Renderer renderer = rig.Renderers[rendererIndex];
	                if (string.IsNullOrEmpty(rendererId) || !rendererIds.Add(rendererId) || !renderer ||
	                    !renderers.Add(renderer) || !catalogRenderers.Add(renderer))
	                {
	                    errors?.Add($"{name}: Equipment Rig Binding '{rig.RigBindingId}' Renderer #{rendererIndex} is missing or duplicated.");
	                    valid = false;
	                }
	            }
	        }
	        ...
	        {
	            EquipmentSocketBinding socket = Sockets[i];
	            if (socket == null || string.IsNullOrEmpty(socket.SocketBindingId) || !socketIds.Add(socket.SocketBindingId) ||
	                !socket.Socket || !sockets.Add(socket.Socket))
	            {
	                errors?.Add($"{name}: Equipment Socket Binding #{i} is missing or duplicated.");
	                valid = false;
	            }
	        }
	        return valid;
