...
namespace ThirdPersonCharacter.Pipeline.Presentation
{
    ...
    public sealed class ActionPresentationSamplingFrameTransaction
    {
        ...
        readonly AnimationPlaybackId[] m_ResolvedMarkers;
        int m_LifecycleCount;
        int m_SampleCount;
        int m_ResolvedMarkerCount;
        ...
        internal ActionPresentationSamplingFrameTransaction(int capacity)
        {
            ...
            m_ResolvedMarkers = new AnimationPlaybackId[capacity];
            Closed = true;
        }
        ...
        internal ActionMarkerMutationLease MarkerLease { get; private set; }
        ...
        internal void Begin(
            ...
            ActionMarkerMutationLease markerLease,
        ...
        internal bool AddResolvedMarker(AnimationPlaybackId playbackId)
        {
            for (int i = 0; i < m_ResolvedMarkerCount; i++)
            {
                if (m_ResolvedMarkers[i].Equals(playbackId))
                    return false;
            }
            if (m_ResolvedMarkerCount == m_ResolvedMarkers.Length)
            {
                throw new InvalidOperationException(
                    "Action sampling Marker resolution capacity was exceeded.");
            }
            m_ResolvedMarkers[m_ResolvedMarkerCount++] = playbackId;
            return true;
        }
        ...
        internal void Close()
        {
            ...
            if (m_ResolvedMarkerCount > 0)
            {
                Array.Clear(
                    m_ResolvedMarkers,
                    0,
                    m_ResolvedMarkerCount);
            }
            Identity = 0;
            PresentationFrame = 0;
    ...
    public sealed class ActionPresentationSamplingRuntime
    {
        ...
        readonly ActionMarkerEffectiveSampleState m_MarkerState;
            ...
            ActionAnimationBindingIndex bindings)
        {
            ...
            int relationCapacity = playbackCapacity;
            int clipStride = 0;
            ...
            m_MarkerState =
                new ActionMarkerEffectiveSampleState(
                    playbackCapacity,
                    relationCapacity);
            m_Transaction =
            ...
            bool captureDiagnostics)
        {
            ...
            {
                if (markerLease.IsValid)
                    m_MarkerState.Discard(markerLease);
        ...
        public ActionMarkerEffectiveSample ResolveIndependentMarker(
            ActionPresentationSamplingFrameTransaction transaction,
            AnimationPlaybackId playbackId)
        {
            RequireTransaction(transaction);
            ...
            return m_MarkerState.ResolveIndependent(
                transaction.MarkerLease,
                playbackId,
                binding.Animation.MarkerSync,
                in projectedRawSample);
        }

        public ActionMarkerEffectiveSample ResolveRelatedMarker(
            ActionPresentationSamplingFrameTransaction transaction,
        ...
        {
            RequireTransaction(transaction);
            ResolvedActionAnimationBinding source =
                RequireProjectedBinding(
                    transaction,
                    sourcePlaybackId,
                    out _);
            ResolvedActionAnimationBinding target =
                RequireProjectedBinding(
                    transaction,
                    targetPlaybackId,
        ...
        public void ResolveSlotMarkers(
            ActionPresentationSamplingFrameTransaction transaction,
            IReadOnlyList<AnimationSlotFramePlan> slotPlans)
        {
            RequireTransaction(transaction);
            if (slotPlans == null)
                throw new ArgumentNullException(nameof(slotPlans));
            for (int i = 0; i < slotPlans.Count; i++)
            {
                AnimationSlotFramePlan plan = slotPlans[i];
                if (!plan.IsValid ||
                    plan.TargetsSourcePose ||
                    !plan.HasMarkerSource)
                {
                    continue;
                }
                ResolvedActionAnimationBinding source =
                    RequireProjectedBinding(
                        transaction,
                        plan.MarkerSourcePlaybackId,
                        out _);
                ResolvedActionAnimationBinding target =
                    RequireProjectedBinding(
                        transaction,
                        plan.TargetActionPlaybackId,
                        out _);
                if (transaction.AddResolvedMarker(
                        plan.MarkerSourcePlaybackId))
                {
                    ResolveIndependentMarker(
                        transaction,
                        plan.MarkerSourcePlaybackId);
                }
                if (!transaction.AddResolvedMarker(
                ...
                {
                    throw new InvalidOperationException(
                        $"Action playback '{plan.TargetActionPlaybackId}' has more than one Slot marker route.");
                }
                if (source.Animation.MarkerSync.IsMarkerGroup &&
                    target.Animation.MarkerSync.IsMarkerGroup)
                {
                    ResolveRelatedMarker(
                        transaction,
                        plan.MarkerRelationId,
                        plan.MarkerSourcePlaybackId,
                        plan.TargetActionPlaybackId);
                }
                else
            ...
            {
                AnimationPlaybackId playbackId =
                    transaction.LifecycleAt(i).PlaybackId;
                if (!transaction.HasProjectedSample(playbackId) ||
                    !transaction.AddResolvedMarker(playbackId))
                {
                    continue;
                }
                ResolveIndependentMarker(transaction, playbackId);
            }
        }

        public void RebaseReleasedMarkerSource(
            ActionPresentationSamplingFrameTransaction transaction,
            AnimationPlaybackId playbackId)
        {
            RequireTransaction(transaction);
            m_MarkerState.RebaseReleasedSource(
                transaction.MarkerLease,
                playbackId);
        }
            ...
            for (int i = 0; i < transaction.LifecycleCount; i++)
            {
                ...
                ResolvedActionAnimationBinding binding =
                    RequireProjectedBinding(
                        ...
                        lifecycle.PlaybackId,
                        out ProjectedActionPresentationSample projected);
                if (!m_MarkerState.TryGet(
                        transaction.MarkerLease,
                        lifecycle.PlaybackId,
                        out ActionMarkerEffectiveSample effective))
                {
                    throw new InvalidOperationException(
                        $"Action playback '{lifecycle.PlaybackId}' has no Marker effective sample.");
                }
        ...
        internal void BuildCommittedMarkerSnapshots(
                ...
                relationDestination)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Action Marker committed diagnostics cannot read an active frame.");
            }
            m_MarkerState.BuildCommittedSnapshots(
                playbackDestination,
                relationDestination);
        }
            ...
            ActionPresentationSamplingFrameTransaction transaction)
        {
            ...
            m_SampleProjector.ValidateFrame(
                transaction.ProjectionLease);
            m_MarkerState.ValidateFrame(transaction.MarkerLease);
        }
                ...
                destination)
        {
            ...
            for (int i = 0; i < m_CommittedDiagnosticCount; i++)
            {
                ...
                if (!m_MarkerState.TryGetCommittedSample(
                        lifecycle.PlaybackId,
                        out ActionMarkerEffectiveSample effective))
                {
                    throw new InvalidOperationException(
                        $"Committed Action playback '{lifecycle.PlaybackId}' has no complete time diagnostics.");
                }
                ProjectedActionPresentationSample projected =
            ...
            ActionPresentationSamplingFrameTransaction transaction)
        {
            ...
            m_MarkerState.Commit(transaction.MarkerLease);
            ...
            ActionPresentationSamplingFrameTransaction transaction)
        {
            ...
            m_MarkerState.Discard(transaction.MarkerLease);
            m_SampleWorkspace?.DiscardFrame();
            Close(transaction);
        }
