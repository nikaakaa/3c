...
## Purpose
定义 `CharacterRuntimeCore`、Mono adapter、runtime ports 和正式 prefab 装配如何收敛到唯一角色帧管线，并退役旧 FullBody host/controller 入口。
## Requirements
### Requirement: Character Frame Runtime Port
系统 MUST 为角色帧管线提供明确的 runtime port Interface，使 `CharacterFramePipeline` 只依赖角色帧所需能力，而不是直接依赖具体 MonoBehaviour controller。该 Interface MUST 位于 `Assets/Scripts/Character/Pipeline/Contracts/...` 或等价角色级目录。该 Interface MUST 覆盖输入缓冲写入、CommittedAction submission 构建、输出应用、表现桥接、runtime facts 写入和诊断提交所需的最小能力。该 Interface MUST NOT 暴露 Unity scene object、Animancer runtime object、InputAction 或 `CharacterController`。

#### Scenario: Pipeline 通过端口推进 phase
- **WHEN** `CharacterFramePipeline` 推进一帧
- **THEN** 它 MUST 通过角色帧 runtime port 调用各 phase 所需能力
- **AND** MUST NOT 在方法签名或字段中直接依赖 旧 FullBody action controller
...
### Requirement: Action runtime 由 CharacterFrame 组合
系统 MUST 通过 `CharacterFrameRuntimeController` 或等价角色级 MonoBehaviour 组合 Action runtime、Locomotion runtime、输入缓冲、输出运行时和诊断端口。正式生产路径 MUST NOT 依赖 旧 FullBody action controller、旧 FullBody runtime port adapter 或等价旧 Host Adapter 作为角色帧 runtime port。
...
- **AND** 生产实现 MUST NOT 通过 旧 FullBody runtime port adapter 转接旧 controller
- **AND** pipeline 和 submitter MUST NOT 直接接收 旧 FullBody action controller
...
### Requirement: Committed Action Submission Runtime Port
系统 MUST 为 Committed Action submitter 提供窄 runtime port，使 submitter 只读取构建 action submission 和 frame submission 所需事实。该 port MUST 提供 runner、current snapshot、input buffer、Action Catalog、interrupt policy、action resistance 和 Locomotion runtime port 的访问能力，但 MUST NOT 允许 submitter 执行 motion、播放 animation、写 Unity scene object 或提交 snapshot/events。

#### Scenario: Submission builder 不依赖 controller 大类
- **WHEN** `CommittedActionFrameSubmitter` 构建 action submission 或 frame submission
- **THEN** 它 MUST 接收 action submission runtime port 或等价窄 Interface
- **AND** MUST NOT 在方法签名或字段中直接依赖 旧 FullBody action controller
- **AND** MUST NOT 直接依赖 `PlayerLocomotionController`

#### Scenario: Submission builder 只提交纯数据
- **WHEN** `CommittedActionFrameSubmitter` 完成 frame submission
...
系统 MUST 为 Locomotion 子职责提供窄 runtime port，使 Action submitter 和 Character output apply 可以通过 Interface 访问 Locomotion facts、frame 构建、motion 执行、动画提交和 runtime facts 写入。第一阶段 MUST 拆分 prepare/build 端口和 output/apply 端口，而不是用一个巨大端口复制旧 Locomotion controller Interface。Locomotion 正式状态 MUST 由 `CharacterRuntimeCore` 组合的 `LocomotionRuntimeModule` 或批准等价模块持有；Unity-facing adapter 只能提供场景依赖。

#### Scenario: Action 通过 Locomotion port 取 facts 和 frame
- **WHEN** Action submission 需要 Locomotion decision facts 或 motion frame
...
- **AND** 系统 MUST NOT 新增一个复制旧 Locomotion controller 调用面的单一巨大端口

#### Scenario: Locomotion facade 收口优先级
- **WHEN** 实施运行时端口化
- **THEN** 旧 Locomotion facade 的 direct tick、frame builder facade、snapshot、diagnostic、reference resolve、camera/facing resolve 职责 MUST 被识别为不同变化原因
- **AND** Character frame 主线 MUST 通过 Locomotion runtime port 访问必要职责
...
- **WHEN** 运行静态边界测试
- **THEN** 测试 MUST 确认 `CharacterFramePipeline` 不直接引用 旧 FullBody action controller
- **AND** MUST 确认 `CommittedActionFrameSubmitter` 不直接引用 旧 FullBody action controller
- **AND** MUST 确认 `CommittedActionFrameSubmitter` 不直接引用 `PlayerLocomotionController`
...
#### Scenario: 不产生分裂路径
- **WHEN** 完成运行时端口化迁移
- **THEN** Character gameplay MUST 仍通过 `CharacterFramePipeline` 推进
...
- **WHEN** 旧 action tick adapter 或等价 simulation tick adapter 推进 phase
...
- **AND** MUST NOT 直接创建旧 FullBody builder
- **AND** MUST NOT 直接依赖旧 FullBody 生产 submitter 具体类
...
- **AND** 该 port MUST NOT 要求调用方是旧 FullBody controller 才能作为正式目标架构成立
...
- **AND** 旧 FullBody action controller 或等价 FullBody host adapter MUST NOT 作为正式或迁移期 gameplay 入口存在
...
- **AND** 旧 FullBody tick adapter MUST 不再作为正式组件挂载、注册或推进 gameplay tick
...
#### Scenario: Runtime host 不由旧 FullBody controller 创建
- **WHEN** 生产路径创建 `CharacterFrameRuntimeHost`
- **THEN** 创建职责 MUST 位于 `CharacterFrameRuntimeController` 或等价角色级 owner
- **AND** 旧 FullBody action controller MUST NOT 直接 `new CharacterFrameRuntimeHost`
- **AND** 旧 FullBody action controller MUST NOT 决定正式 submitter graph 中有哪些 sibling submitter
...
角色级 runtime port MUST 能组合 Locomotion adapter、Action adapter、input buffer adapter、output runtime adapter 和 diagnostics adapter。该 port MUST NOT 通过旧 FullBody controller 的大操作面板表达所有能力，也 MUST NOT 要求新增身体域了解 FullBody-specific port。
...
- **AND** MUST NOT 读取 旧 FullBody action controller 私有字段
...
- **AND** 旧 FullBody controller 与 Locomotion controller 的 direct auto update MUST 不作为正式 gameplay driver
- **AND** 冲突 MUST 可通过自动测试或装配校验发现

### Requirement: Character runtime port 去 FullBody 化
系统 MUST 将正式 Character runtime port Interface 收敛为角色级 frame 能力合同。该 Interface MUST NOT 通过继承旧 FullBody submission/output port 或等价 FullBody port 来暴露旧 FullBody 内部操作面板。CommittedAction 端口必须以 Action/CommittedAction 窄接口表达，不得恢复 FullBody adapter。
...
#### Scenario: 不保留 FullBody adapter 领域端口
- **GIVEN** Action 仍需要 runner、snapshot、interrupt policy、action resistance 或 output runtime
- **WHEN** CommittedAction 构建自己的提交或输出
- **THEN** 它 MUST 使用 Action/CommittedAction narrow ports
- **AND** MUST NOT 通过 FullBody-specific narrow ports 成为 Character-level host 的正式 Interface
- **AND** 这些 ports MUST NOT 被新身体域当作上级 owner

### Requirement: 旧 FullBody host adapter 退役
旧 FullBody action controller 或等价 MonoBehaviour MUST 从正式角色帧 owner、Unity 装配、配置解析、调试 view 和旧 Tick 兼容 Adapter 中退役。正式生产路径 MUST 通过 `CharacterFrameRuntimeController` 组合 Character-level runtime host，不得保留旧 FullBody host 作为可调用 gameplay 入口。

#### Scenario: 旧 Tick 入口不存在正式转发
- **WHEN** 兼容代码、prefab 或 scene 被检查
- **THEN** 旧 FullBody action controller tick MUST NOT 作为正式或迁移期 gameplay 入口存在
...
- **AND** MUST NOT 由旧 FullBody controller 私有字段决定哪些身体域参与正式仲裁

### Requirement: 端口退役可测试
系统 MUST 提供自动测试证明旧 FullBody port 退役不会产生分裂路径。测试 MUST 覆盖 Character pipeline 不直接依赖旧 FullBody concrete adapter、新身体域不依赖旧 FullBody controller、旧 Tick 入口不存在且不注册或推进正式 gameplay tick。

#### Scenario: 静态测试阻止 FullBody 泄漏
- **WHEN** 运行 runtime port 静态测试
- **THEN** 测试 MUST 确认 Character-level port 不继承 FullBody ports
- **AND** MUST 确认 `CharacterFramePipeline` 不引用 旧 FullBody action controller
...
- **AND** MUST NOT 通过 `CharacterFrameRuntimeController` 再查找旧 Locomotion facade 或旧 旧 FullBody action runtime facade 来获得正式状态
...
正式角色 runtime 状态 MUST 从 MonoBehaviour 字段迁出。`CharacterFrameRuntimeController` 或批准的等价 Mono adapter MAY 保留序列化引用、Unity 生命周期入口和兼容 facade，但 MUST NOT 持有正式 `LocomotionRuntimeStateStore`、`CharacterRuntimeBlackboard`、`CharacterStateMachineRuntime`、`ActionLifecycleRuntime` 或 `CharacterFrameRuntimeHost` 作为 authoritative state。旧 Locomotion facade 和旧 旧 FullBody action runtime facade 不得作为正式运行时状态 owner 保留。
...
- **WHEN** 自动静态测试扫描正式 production runtime 代码
- **THEN** 旧 FullBody action runtime facade MUST NOT new 或持有正式 `CharacterStateMachineRuntime`
- **AND** MUST NOT new 或持有正式 `ActionLifecycleRuntime`
- **AND** Action state MUST 由 core-owned `CommittedActionRuntimeModule` 或批准等价 Action runtime module 持有
...
- **AND** 旧 FullBody action tick adapter 与 `LocomotionTickAdapter` 不作为可驱动组件挂载
...
正式 Action 运行时端口 MUST 不依赖 旧 FullBody action controller 或旧 Host Adapter。状态、请求、动画播放和诊断数据必须通过当前正式端口与视图暴露。

#### Scenario: Action 不需要旧 Host Adapter

- **GIVEN** Action runtime 被构建
- **WHEN** Action 请求、状态推进和动画状态被访问
- **THEN** 调用链不需要 旧 FullBody action controller
...
### Requirement: 旧 FullBody Host Adapter 退役
系统 MUST 删除 旧 FullBody action controller 或等价 FullBody MonoBehaviour host adapter。正式角色帧 runtime port MUST 由 `CharacterFrameRuntimeController` 或等价角色级 owner 组合状态机 runtime、Locomotion runtime、Action runtime、output runtime 和 diagnostics dependencies。生产路径 MUST NOT 使用 旧 FullBody runtime port adapter 包装 旧 FullBody action controller 暴露 pipeline 所需能力。

#### Scenario: 旧 controller 类型被删除
- **WHEN** 检查生产运行时代码、测试 fixture、prefab 和 scene
- **THEN** 旧 FullBody action controller 类型 MUST 不再作为正式组件、字段、属性、构造参数或端口依赖存在
...
- **AND** runner owner MUST NOT 通过旧 FullBody controller MonoBehaviour 表达
...
- **AND** MUST NOT 通过 旧 FullBody action controller 的公开属性或内部类访问
- **AND** MUST NOT 创建 fallback executor、fallback presenter 或隐藏默认配置

### Requirement: Behavior Submission 依赖窄端口
`CharacterBehaviorSubmissionRunner`、`LocomotionFrameSubmitter` 和 `CommittedActionFrameSubmitter` MUST 只依赖 Character、Locomotion、Action、StateMachine 和 Output 的窄端口。它们 MUST NOT 依赖 旧 FullBody action controller、旧 FullBody runtime port adapter 或单个 FullBody 集成端口来读取所有 runtime 状态。

#### Scenario: Behavior submission 不包装旧 FullBody controller
- **WHEN** 构建角色 runtime port 与 behavior submission runner
...
- **AND** behavior submission runner MUST NOT 通过 旧 FullBody action controller 或 旧 FullBody runtime port adapter 访问 runner、Dodge config、Locomotion snapshot、output runtime 或 diagnostics

### Requirement: 旧 FullBody action controller 删除验证
系统 MUST 提供自动测试验证 旧 FullBody action controller 已从正式 runtime 边界删除。测试 MUST 覆盖代码引用、prefab/scene 绑定、runtime port 组合和 rollback fixture 迁移。

#### Scenario: 静态边界验证无旧 controller
- **WHEN** 运行 runtime port 静态边界测试
- **THEN** 测试 MUST 确认生产 runtime 代码不定义 旧 FullBody action controller
- **AND** MUST 确认生产 runtime 代码不引用 旧 FullBody action controller
...
系统 MUST 让 Corin 正式 prefab 只挂载一个 gameplay runtime assembly adapter。该 adapter MUST 负责创建、持有或绑定 `CharacterRuntimeCore`，并将输入、运动、动画、facing、tick 和配置等 Unity-facing adapters 注入 core dependencies。`PlayerLocomotionController`、旧 FullBody action runtime facade 或等价迁移期 facade MUST NOT 作为正式 prefab 组件表达 Locomotion 或 Action owner。
...
- **AND** prefab MUST NOT 同时挂载 `PlayerLocomotionController` 和 旧 FullBody action runtime facade 作为正式 gameplay runtime facade
...
`PlayerLocomotionController` 和 旧 FullBody action runtime facade MUST 从正式代码面删除，旧测试、旧 fixture、旧 assembler 和旧 debug rig MUST NOT 继续依赖这些迁移期 facade。Locomotion 与 Action runtime state MUST 只由 `CharacterRuntimeCore` 组合的 module 持有，并通过窄 Unity-facing adapters 装配。
...
- **WHEN** 自动校验 Corin 正式 prefab 和正式 scene override
- **THEN** 旧 FullBody action runtime facade MUST NOT 作为正式 gameplay 组件存在
- **AND** Action runtime state MUST 仍由 `CharacterRuntimeCore` 组合的 `CommittedActionRuntimeModule` 或批准等价 Module 持有
...
- **THEN** `PlayerLocomotionController` 和 旧 FullBody action runtime facade 类型 MUST 不存在
- **AND** 旧测试 MUST 改为直接使用 `CharacterFrameRuntimeController`、`CharacterRuntimeCore`、`LocomotionRuntimeModule` 或 `CommittedActionRuntimeModule`
