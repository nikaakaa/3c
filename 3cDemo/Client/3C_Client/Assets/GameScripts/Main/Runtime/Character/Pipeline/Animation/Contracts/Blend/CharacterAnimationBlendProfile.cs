...
namespace ThirdPersonCharacter.Pipeline.Animation
{
    ...
    {
        public const string SchemaVersion = "character-animation-blend-profile/v2";
        ...
        public float[] BuildDense(CharacterAnimationRigDefinition rig)
        {
            ...
            var dense = new float[rig.PoseBoneCount];
            ...
            for (int i = 0; i < BoneOverrides.Count; i++)
            {
                ...
                dense[rig.RequirePoseBoneIndex(value.BoneId)] = value.Multiplier;
            }
            if (seen.Count != rig.PoseBoneCount)
                throw new InvalidOperationException($"Animation Blend Profile '{name}' does not explicitly cover every Pose Bone.");
            return dense;
        }
    }
}
