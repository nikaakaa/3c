...
{
    public sealed class AgentStateBehaviorMutationHandler : IAgentMutationHandler
    {
        readonly BtsmtlGraphAuthoringCapabilities m_Emitters;
        readonly AgentConditionRuleBuilder m_ConditionBuilder;

        public AgentStateBehaviorMutationHandler(
        ...
        bool PreflightActionExit(AgentMutationSession session, AgentEnsureActionExitLifecycleMutation command)
        {
            ...
            if (graph != null)
            {
                BaseNode lifecycleParent = AgentMutationGraphAuthoringUtility.ResolveLifecycleAnchor(graph, "OnExit");
                if (!lifecycleParent)
                {
        ...
        bool PreflightDelete(AgentMutationSession session, AgentDeleteStateBehaviorNodeMutation command)
        {
        ...
        bool PreflightNode(AgentMutationSession session, AgentEnsureStateBehaviorNodeMutation command)
        {
            if (!session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph))
                return false;
        ...
        bool PreflightTimeline(AgentMutationSession session, AgentEnsureTimelineNodeMutation command)
        {
            bool valid = session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph);
        ...
        bool PreflightActivation(AgentMutationSession session, AgentEnsureActionActivationMutation command)
        {
            bool valid = session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph);
        ...
        bool PreflightLifecycle(AgentMutationSession session, AgentEnsureActionLifecycleTransitionMutation command)
        {
            bool valid = session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph);
            if (!session.Resolver.TryResolveActionContext(command.ActionContext, out ActionContextSlot actionContext) || !actionContext)
        ...
        void ApplyActionExit(AgentMutationSession session, AgentEnsureActionExitLifecycleMutation command)
        {
            if (!session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph))
                return;
            if (!session.Resolver.TryResolveActionContext(command.ActionContext, out ActionContextSlot actionContext) || !actionContext)
            ...
            BaseNode onExit = AgentMutationGraphAuthoringUtility.ResolveLifecycleAnchor(graph, "OnExit");
            if (!onExit)
            {
                session.Report.Error(command.Path, "lifecycle_anchor_not_found", "Action exit lifecycle 无法解析 OnExit 入口。");
                return;
        ...
        void ApplyDelete(AgentMutationSession session, AgentDeleteStateBehaviorNodeMutation command)
        {
            if (!session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph) ||
            ...
            if (!session.RefreshIndex(command.Path))
                return;
            session.AddAppliedWithoutIdentity(command, graph, target, "deleted");
        }

        void ApplyNode(AgentMutationSession session, AgentEnsureStateBehaviorNodeMutation command)
        {
            if (!session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph))
                return;
            if (!session.TryResolveOptionalNode(graph, command.ExistingElement, command.Path, out BaseNode existing))
        ...
        void ApplyActivation(AgentMutationSession session, AgentEnsureActionActivationMutation command)
        {
            if (!session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph))
                return;
        ...
        void ApplyLifecycle(AgentMutationSession session, AgentEnsureActionLifecycleTransitionMutation command)
        {
            if (!session.TryResolveStateBehavior(command.Target, command.Path, out StateBehaviorSubTree graph))
                return;
