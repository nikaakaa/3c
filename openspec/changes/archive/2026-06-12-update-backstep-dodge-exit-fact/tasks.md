...
## 6. 手动验证
- [x] 6.1 打开当前演示场景，使用可琳角色。
- [x] 6.2 不输入方向只按 Shift，确认进入 Backstep 后闪。
- [x] 6.3 保持无输入，确认后闪蹲下到起身恢复过程不会在 0.35 秒被 Idle 打断。
- [x] 6.4 确认恢复完成后回到 Idle。
- [x] 6.5 后闪恢复段重新输入移动，确认可提前回到移动阶段。
- [x] 6.6 有方向按 Shift，确认 Directional Dodge 仍快速冲刺并接回移动。
- [x] 6.7 确认没有新增第二角色控制器、第二状态机或动画直接位移路径。
