...
每个可运行 `CharacterPipelineHost` MUST 配置唯一非空 ActorId，并传入 CharacterPipeline、GraphContext 和 GameplayEffectAdapter。Network Model binding MAY 读取该 ActorId 作为 subject identity，但 MUST NOT 保存第二份可独立配置的 SubjectActorId。GameplayEffectContext 的 Self source/target MUST 使用该 ActorId。
