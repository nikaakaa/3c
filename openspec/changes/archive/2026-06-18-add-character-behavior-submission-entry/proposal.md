...
- 新增最小 production runtime definition / config，使 runner 消费正式定义，不从隐藏 fallback 或代码默认树推导生产入口。
...
- 默认角色 runtime 的 behavior entry 来自正式 runtime definition / config，缺失时报配置错误，不生成隐藏 fallback 树。
...
- 如果需要用代码默认树或 Resources fallback 补齐缺失 behavior entry 配置，必须停止。
