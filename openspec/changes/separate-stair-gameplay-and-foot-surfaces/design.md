...
两套几何由同一个作者绑定关联，但每个消费者只看其中一套。分离是正式数据模型，不是运行时fallback。
...
所有标记为连续楼梯的作者对象必须拥有唯一Traversal Ramp。Fixed KCC只消费该Ramp在Collision Artifact中的量化表面，不再消费楼梯每一级的Box/Mesh Collider。Foot Placement继续消费真实踏面Collider，并完全排除Ramp。
...
Gameplay Lab中的LowStairs、HighStairs与连续OverLimitStairs全部迁移为Ramp Gameplay Collision。原0.14m、0.24m、0.40m Step能力边界迁移到独立`StepCapabilityCourse`：
...
3. 为每条连续楼梯建立持久化Traversal Ramp与Foot Surface根。
