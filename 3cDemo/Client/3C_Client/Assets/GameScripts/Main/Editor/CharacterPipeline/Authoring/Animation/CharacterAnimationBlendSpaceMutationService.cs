...
namespace ThirdPersonCharacter.Pipeline.Editor
{
    public static class CharacterAnimationBlendSpaceAuthoringService
    {
        ...
        public static void DeleteSample(CharacterAnimationBlendSpaceAsset asset, CharacterAnimationBlendSpaceSampleId sampleId)
        {
            ...
            RequireDeletable(asset, new[] { sampleId });
            ...
            IReadOnlyCollection<CharacterAnimationBlendSpaceSampleId> sampleIds)
        {
            ...
            foreach (CharacterAnimationBlendSpaceSampleId sampleId in removed)
                RequireSample(asset, sampleId);
            RequireDeletable(asset, removed);
        ...
        static void RequireDeletable(
            CharacterAnimationBlendSpaceAsset asset,
            IReadOnlyCollection<CharacterAnimationBlendSpaceSampleId> sampleIds)
        {
            if ((asset.PhasePolicy == CharacterAnimationBlendSpacePhasePolicy.LocomotionPhase ||
                 asset.PhasePolicy == CharacterAnimationBlendSpacePhasePolicy.LocomotionPhase) &&
                asset.PhaseReferenceSampleId.IsValid &&
                sampleIds.Contains(asset.PhaseReferenceSampleId))
            {
                throw new InvalidOperationException(
                    $"Sample '{asset.PhaseReferenceSampleId}' is the Marker Phase Reference. Select another reference or switch the phase policy before deleting it.");
            }
        }
        ...
        {
            RequireAsset(asset);
            if (policy == CharacterAnimationBlendSpacePhasePolicy.LocomotionPhase ||
