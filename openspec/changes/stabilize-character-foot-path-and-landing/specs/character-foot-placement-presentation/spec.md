...
纯`CharacterFootStateTargetResolver` MUST按Transition后的离散State生成Correction Target、Contact Reference、Goal与Ownership目标及typed Interpolation Policy Request。在既有正式Swing准入范围内，Swing与UnlockedSupport的可见高度 MUST使用同帧CurrentSupport零净空Sole参考与Formal Foot Height；Ground Path与Envelope MUST保留规划采样，不得同时作为可见高度或安全下限。Landing与Locked MUST只使用冻结的同Event Contact Anchor；Releasing MUST只回到同一份Swing Target。Resolver MUST不保存跨帧时间状态、不推进Residual、不改写State、不得执行World Query，也不得执行Post Constraint。

唯一typed `CharacterFootInterpolationRuntime` MUST拥有上一Target、从Swing连续到Plant的每脚Target Height History、上一Mixed World Target、上一实际Response Output Point、完整Vector Plant World Residual、标量Correction Response History、归一化Support Direction与Position Response Basis、唯一Swing/Release Residual与Completion。Swing Path换代、Landing Acquire和Release MUST只通过固定typed Policy Request连续化；迁移完成后 MUST删除分散的`SwingResidual`、`AcquireResidual`、`ReleaseResidual`、`ContactProgress`和重复Advance数学。

Swing MUST以`dot(CurrentSupport零净空Sole, ComponentUp) + Formal Foot Height`生成Raw Target Height，保持动画XZ与非负竖直修正。Target Height History MUST保存当前支撑参考沿ComponentUp的世界高度，输出 MUST为`Raw Target Height + Filtered Support Height - Current Support Height`。同一Event只有Formal Foot Height或未来规划点/Path身份变化时 MUST不因此捕获Swing残差；当前Support Surface换代 MUST发布`SupportSurfaceChanged`，同Event当前支撑高度变化超过`PathRevisionDistance` MUST发布`TargetTrackingApplied`。未来LandingPointDelta MUST只作为规划变化观测。Profile显式`TargetHeightAdoptionMode=Direct`时，当前支撑高度 MUST直接更新History；`RateLimited`时继续使用现有距离门、Force Refresh Distance与MaximumVerticalTargetSpeed，不新增参数。Plant取得Interpolation所有权后，Swing更新 MUST发布typed Held，不得改写Plant的高度History。CurrentSupport缺失 MUST发布typed unavailable，不得借用Envelope、上一命中或默认Up。已声明的GroundPath规划准入与供Pelvis使用的未来事件/点不在本步更改。

State Target MUST从Swing Ground/Current Support或Verified Anchor中选择一个同时携带Position、SupportNormal及分型来源lineage的正式Target；Swing Ground的Position MUST来自本帧CurrentSupport零净空Sole参考与Formal Foot Height，PositionSource MUST为SwingCurrentSupport且PositionPathIdentity为0；Normal继续来自本帧CurrentSupport，但Target MUST分别发布Position Source与Normal Source，不得伪装成同一Observation。两路来源 MUST属于同Frame、Side与World Revision。Support Normal MUST在有限检查后归一化为同Frame唯一Requested Support Direction。当前Final Component Pose已经完成Pose Graph动画混合；没有Pose Graph明确发布的同一StandardBlend source/target Foot骨骼实际贡献与双Support lineage时，Runtime MUST不再执行第二次Current/Target状态混合。Runtime在Target Event、Target Kind、Lock Response、Verification、Direct Follow、State/Response边沿、Target Point Revision或Target Height Force Refresh时，以持久上一实际Response Output Point为基准捕获完整WorldResidual。Approach Progress变化 MUST不Capture。稳定且Target Height delta为0的Locked帧 MUST发布`TargetHeightUpdateReason=None`。`DesiredOutputPoint` MUST等于`SelectedWorldTarget + ResidualAfterDecay`。
...
Target Height、Plant World Residual与Correction Response MUST分别发布Owner、Before、Target、Applied Delta、After和Reset Reason，不得合并、互相覆盖或由同一无类型Reset清空。旧单档`MaximumVerticalCorrectionSpeed`、稳定帧逐帧上一世界输出重表达、无条件全Plant限速、“World Residual取代Correction历史”的Disposition与“上一Animated Sole + Previous Correction”伪基准 MUST保持删除；173条所述正式换代一次性重基不属于旧链。Swing/UnlockedSupport MUST以同一SwingHeightReference中的CurrentSupport零净空Sole作为硬下界，Release MUST继续使用统一Residual。`AcquireByWeight`进入帧不得对Contact Anchor立即`RaiseToMinimum`，正式Weight达到1时也不得清除尚未收敛的Residual或Correction Response。Residual大于`SwingResidualTolerance`时，Interpolation Runtime MUST按正式Step Time计算Landing截止收敛；Releasing完成 MUST只读取独立`ReleaseCompletionTolerance`。Step Time只决定Residual衰减，不得改变Raw Target、重选State或掩盖同帧不连续。既有Foot Goal/Position Weight MUST只在Response Output之后与动画基线混合一次；FBBIK与Final Pose之后 MUST没有常驻Foot低通。

CurrentSupport安全下限、Contact Anchor与Reach MUST在Interpolation之后由唯一Post Constraint消费。Ground部分 MUST复用本帧Accepted Swing Motion携带的同一CurrentSupport零净空Sole参考或冻结的同Event Anchor，不得执行Raycast、SphereCast或读取另一Surface。Swing/UnlockedSupport的最低约束 MUST发布CurrentSupport owner、实际Surface identity和零Path identity，不得再由预测Envelope抬升；Landing/Locked的Contact Anchor部分 MUST只测量穿透、分类`GroundPenetrationTolerance`内外并发布Ground Catchup与Full Lock门控，不得立即Clamp、修改Effective Correction、触发Residual Revision或写回Interpolation历史。某次交接继承的超预算Contact穿透 MUST继续由同一Correction Response向Anchor收敛，期间 MUST禁止Full Lock。Reach部分 MAY硬夹紧已知不可达Goal，但 MUST不反向修改State、Transition Decision、Target或Residual。全部分型状态 MUST由同一根Bank统一Seal或Discard，不得形成第二状态机、第二生命周期或第二输出路径。
...
`TargetHeightAdoptionMode`、`MaximumVerticalTargetSpeed`、`TargetHeightForceRefreshDistance`、`SupportDirectionMaximumChangeDegrees`、`CorrectionResponseIncreaseSpeed`、`CorrectionResponseDecreaseSpeed`、`GroundPenetrationTolerance`与`LandingLockCompletionTolerance` MUST由Corin Profile显式序列化并进入Profile Revision；所有标量 MUST有限且为正，Direction角 MUST不超过180度，不得使用代码默认值或互相复用，也不得复用Landing接受、Path Revision、Swing Residual、Release完成与Lock准入距离。`TargetHeightForceRefreshDistance` MUST大于`PathRevisionDistance`。Corin MUST使用实测激活实例对应的`Direct` Target Height模式、每次`10°`Direction变化上限以及`1.8m/s`、`1.5m/s`两档标量速率。两档选择在项目中 MUST由米制PoseRoot-Y Desired Response相对历史的增减表达，不得复制或命名匿名`0x199`、D、`0x54`、`0x58`。`GroundPenetrationTolerance`与`LandingLockCompletionTolerance`首个候选均为`0.01m`，`TargetHeightForceRefreshDistance`为`0.30m`。旧`MaximumVerticalCorrectionSpeed`与`BasisTransferred` MUST不再出现在Profile、Runtime或诊断schema中；新Direction/Correction Response字段 MUST不得读取或迁移旧`0.6m/s`值。
...
#### Scenario: 同Event当前支撑换代

- **WHEN** 同一Swing Event的CurrentSupport Surface换代或支撑高度变化超过正式距离门
...
- **AND** Post Constraint MUST执行同一CurrentSupport零净空Sole的Swing最低约束并记录Clamp事实，不得把该约束扩展为Landing/Locked Contact Anchor的同帧抬升；仅未来Landing/Envelope规划变化不得捕获可见残差
...
#### Scenario: 原始正式脚高不能再次从接触捕获输出扣除

- **WHEN** 同Event从Swing进入Verified Anchor时Formal Foot Height下降，而上一可见输出已经经过Swing Residual、Correction Response和约束
- **THEN** World Residual捕获 MUST保留现有上一世界输出与Selected Target之差，不得把原始Formal Foot Height的变化当作仍完整存在于上一实际输出中的独立位移再次加减
- **AND** Runtime MUST保留完整XYZ连续性并在捕获后同帧Advance，不得用清Y、硬贴地、降低诊断阈值或最终Pose低通掩盖负Swing残差导致的面下目标；已否决候选只保留在历史实验与原始采样中
...
- **AND** 发布为Swing的Corrected Sole MUST立即遵守同一CurrentSupport零净空Sole最低约束并记录Clamp事实，不得沿用Landing/Locked的Contact竖直限速政策
...
本合同只声明本次坐标公式，MUST不把它表述为ZZZ原始desiredRaw的精确复刻，MUST不据此声明Owner任意TRS改变时无跳或完整脚掌旋转支持非均匀父级。g、k、writer W及owner-Y选速率 MUST不混入本单变量实验。

#### Scenario: 稳定Swing的Support Normal往返变化

- **WHEN** 同一PoseRoot basis、Source、Event及稳定Desired目标下，Support Direction在相邻帧往返且位置scalar尚未追到q
- **THEN** 位置响应欠账 MUST只沿WorldAxis作用，不能因Support Direction变化转成额外横向位移
- **AND** 支撑朝向仍 MUST按原角上限推进并进入唯一Foot Rotation，不能硬置Up或调整Slope/查询掩盖来源
...
#### Scenario: 当前支撑与预测包络诊断分责

- **WHEN** Accepted Ground Path Envelope高于连续Swing输出，但本帧CurrentSupport拥有当前脚高度
- **THEN** 诊断 MUST分别记录规划Envelope及其几何距离、实际CurrentSupport引用、Surface、最低修正与Safety Floor前后输出，不能把低于远处Envelope直接命名为实际Collider穿透
- **AND** 真实Contact间隙/穿透和Final Physical Heel/Toe质量 MUST继续按原正式平面与阈值判断，Runtime和Diagnostics不得从旧Envelope补写新的CurrentSupport高度
