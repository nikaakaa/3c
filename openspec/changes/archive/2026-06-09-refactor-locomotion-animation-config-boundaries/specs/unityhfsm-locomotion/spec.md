...
系统 MUST 使用项目已安装的 UnityHFSM 管理基础 Locomotion 的 `Idle / MoveStart / MoveLoop / MoveStop` 阶段，并 MUST 保留当前基础移动阶段语义。当前阶段机 MUST NOT 把 `Run`、`Walk` 或 Animancer alias 建模为逻辑状态。
...
- **WHEN** `MoveStop` 判断是否可以回到 `Idle`
- **THEN** 状态机 MUST 只读取纯数值的当前停止退出时长
