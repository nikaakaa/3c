    ...
    public class GrassRenderFeature : ScriptableRendererFeature
    {
        ...
        void OnEnable()
        {
            ...
            if (PipelineUtilities.RenderGraphEnabled())
            {
                ...
                Debug.LogError($"[{this.name}] Unsupported Unity version. Render feature and its functionality is not compatible with Unity 6.3+. Upgrade to the Unity 6 version of this asset.");
                #else
                Debug.LogError($"[{this.name}] Render Graph is enabled but is not supported. Enable \"Compatibility Mode\" in your project's Graphics Settings as a workaround. Upgrade to the Unity 6 version of this asset.");
                #endif
            }
            #endif
        }
