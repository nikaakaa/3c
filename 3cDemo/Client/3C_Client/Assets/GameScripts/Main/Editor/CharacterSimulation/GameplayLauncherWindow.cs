    ...
    public sealed class GameplayLauncherWindow : EditorWindow
    {
        ...
        void DrawGameplayLab()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button("Rebuild Assets"))
                        Execute(() =>
                        {
                            operations.RebuildAssets();
                            RefreshGameplayLab();
                        });
                }
            }
        ...
        static void DrawNetworkTests()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                EditorGUILayout.HelpBox("Build publishes a fixed product closure. Run only consumes the published artifacts.", MessageType.Info);
                using (new EditorGUI.DisabledScope(IsBusy))
                {
                    DrawNetworkRow(
                        "Deterministic Rollback",
                        DeterministicRollbackNetworkTestBuildAndRun.PrepareAssetsAndScenes,
        ...
        static void DrawNetworkRow(string label, Action prepare, Action build, Action run)
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.LabelField(label, GUILayout.Width(190f));
                if (prepare != null && GUILayout.Button("Prepare", GUILayout.Width(90f)))
                    Execute(prepare);
