...
支撑脚在Grounded、拥有LastLanding、不是当前摆动脚且未被有限Action占用时，MUST先按`plantHeight = max(0, dot(LastLanding - originalSole, ComponentUp))`把Sole和Ankle沿`Component Up`落到不低于LastLanding的高度，再按水平误差进入Locked、Sliding或Unlocked。Locked/Sliding的Sole水平位置 MUST使用LastLanding方向的水平偏移叠加到`plantedSole`，不得把`LastLanding + up * plantHeight`作为目标。Locked/Sliding的Ankle目标 MUST保留同一帧原生`originalAnkle - originalSole`偏移。Pelvis MUST使用现有`PelvisPreSolveTranslation`：步伐起点是支撑脚LastLanding，终点是revision后的摆动脚NextSwingLanding；Runtime MUST按Pose Root在步伐水平轴上的进度得到`sampledGround`，再以`dot(sampledGround - poseRootPosition, ComponentUp)`计算唯一地面表现修正，不得把Gameplay Body已经沿Traversal Ramp走过的`rise * progress`再次叠加。双脚位置目标 MUST先经过各自最终Goal换代与Position Weight，再把加权踝骨平移同步投影到Sole；净空抬升 MUST只等于更低加权Goal Sole相对更低原生Sole的非负高度差。地面修正与净空抬升 MUST共同成为唯一临界弹簧target；Pending弹簧 MUST从Committed slope、target、output和velocity初始化。支撑Event变化、坡向变化或新Target从上一Target所在方向跨过当前Output时 MUST形成typed handoff；Handoff MUST保留上一Output，仅当旧Velocity正在远离新Target时把输入Velocity清零。其它帧 MUST保留Committed Velocity。系统 MUST不按Target差绝对值建立阈值模式，不得按Stride Start重基旧output，不得把target差作为直通位移，也不得建立第二Pelvis Goal或额外交叉淡入淡出。首次有效步伐 MUST从零修正与零速度进入弹簧，`springOutput` MUST是唯一最终盆骨平移。
