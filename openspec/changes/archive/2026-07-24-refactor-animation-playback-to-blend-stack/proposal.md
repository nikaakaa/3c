...
本change交付的显式BlendStack继续只服务连接到自身的exact Selection历史，不重新变成每AnimationChannel隐藏Stack。后续`refactor-animation-control-boundaries`让PoseState Transition拥有自己的Blend Logic，让AnimationSlot拥有有限Action endpoint transition，并保留BlendStack作为并列显式节点。若本change晚于该后续change归档，归档时 MUST保留三类transition owner边界，不得把PoseState或Slot强制改回BlendStack包装。
