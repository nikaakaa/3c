...
namespace ThirdPersonCharacter.Pipeline.Editor
{
    ...
    sealed class BlendSpaceInspectorAdapter : IGraphAuthoringInspectorAdapter, IGraphAuthoringWorkspacePageAdapter
    {
        ...
        void DrawAsset()
        {
            ...
            AddReadOnly("BlendSpaceId", asset.BlendSpaceId.Value);
            AddReadOnly("Content Revision", asset.ContentRevision);
        ...
        void DrawSample(CharacterAnimationBlendSpaceSample sample)
        {
            ...
            DrawFootAnalysis(sample.SampleId);
        ...
        void DrawMarkers(CharacterAnimationBlendSpaceSample sample)
        {
            var foldout = new Foldout { text = "Marker Binding", value = false };
            var ids = new List<TextField>();
            var times = new List<FloatField>();
            for (int i = 0; i < sample.Markers.Count; i++)
            {
                int markerIndex = i;
                var row = new VisualElement { style = { flexDirection = FlexDirection.Row } };
                var id = new TextField { value = sample.Markers[i].MarkerId };
                id.style.flexGrow = 1f;
                var time = new FloatField { value = sample.Markers[i].NormalizedTime };
                time.style.width = 90f;
                ids.Add(id);
                times.Add(time);
                row.Add(id);
                row.Add(time);
                row.Add(new Button(() => m_Window.ApplyMutation(() =>
                    CharacterAnimationBlendSpaceAuthoringService.SetSampleMarkers(
                        m_Window.Asset,
                        sample.SampleId,
                        sample.Markers.Where((_, index) => index != markerIndex).ToArray()))) { text = "−" });
                foldout.Add(row);
            }
            foldout.Add(new Button(() => m_Window.ApplyMutation(() =>
            {
                float normalizedTime = sample.Markers.Count == 0
                    ? 0f
                    : (sample.Markers[sample.Markers.Count - 1].NormalizedTime + 1f) * 0.5f;
                CharacterAnimationBlendSpaceMarker[] markers = sample.Markers
                    .Concat(new[] { new CharacterAnimationBlendSpaceMarker($"marker.{Guid.NewGuid():N}", normalizedTime) })
                    .ToArray();
                CharacterAnimationBlendSpaceAuthoringService.SetSampleMarkers(m_Window.Asset, sample.SampleId, markers);
            })) { text = "Add Marker" });
            foldout.Add(new Button(() => m_Window.ApplyMutation(() =>
            {
                CharacterAnimationBlendSpaceMarker[] values = ids
                    .Select((id, index) => new CharacterAnimationBlendSpaceMarker(id.value, times[index].value))
                    .ToArray();
                CharacterAnimationBlendSpaceAuthoringService.SetSampleMarkers(m_Window.Asset, sample.SampleId, values);
            })) { text = "Apply Markers" });
            m_Content.Add(foldout);
        }
        ...
        void DrawReferences()
        {
            ...
            AddReadOnly("Projection Revision", m_Window.Projection ? m_Window.Projection.ProjectionRevision : "Unavailable");
            IReadOnlyList<AnimationProducerPresentationBinding> bindings = m_Window.ReferencingBindings;
            for (int i = 0; i < bindings.Count; i++)
                AddReadOnly($"Producer {i + 1}", bindings[i].ProducerId.ProgramProducerIdentity);
            ...
            if (m_Window.Definition && m_Window.Definition.SimulationProgram && m_Window.Projection)
            {
                try
                {
                    ...
                    for (int playerIndex = 0; playerIndex < projection.BlendSpacePlayers.Count; playerIndex++)
                    {
                        ...
                        bool matches = player.BlendSpacePlanIndices.Any(index =>
                            index >= 0 && index < projection.BlendSpaces.Count &&
                            projection.BlendSpaces[index].BlendSpaceId.Equals(m_Window.Asset.BlendSpaceId));
                        if (matches)
    ...
    sealed class BlendSpaceNavigatorAdapter : IGraphAuthoringWorkspaceRegionAdapter
    {
        ...
        public void Refresh()
        {
            ...
            if (!m_Window.Asset)
                return;
            m_Root.Add(new Label($"Asset · {m_Window.Asset.BlendSpaceId}"));
    ...
    sealed class BlendSpaceBottomDockAdapter : IGraphAuthoringWorkspaceRegionAdapter, IGraphAuthoringWorkspacePageAdapter
    {
        ...
        void DrawPreview()
        {
            ...
            m_Content.Add(time);
            var target = new ObjectField("Character Preview Target")
            {
                objectType = typeof(CharacterPipelineHost),
                allowSceneObjects = true,
                value = m_Window.PreviewTarget
            };
            target.RegisterValueChangedCallback(evt => m_Window.SetPreviewTarget(evt.newValue as CharacterPipelineHost));
            m_Content.Add(target);
            IReadOnlyList<string> producerIdentities = m_Window.PreviewProducerIdentities;
            if (producerIdentities.Count > 0)
            {
                int selected = Math.Max(0, producerIdentities.ToList().IndexOf(m_Window.PreviewProducerIdentity));
                var producer = new PopupField<string>("Producer", producerIdentities.ToList(), selected);
                producer.RegisterValueChangedCallback(evt => m_Window.SetPreviewProducer(evt.newValue));
                m_Content.Add(producer);
            }
            else
                m_Content.Add(new HelpBox("No formal producer binding references this Blend Space asset.", HelpBoxMessageType.Warning));
            ...
            {
                m_Window.ApplyPreview(Parameter(), time.value);
                m_Window.SeekCharacterPreview();
            }) { text = "Sample Character" });
            actions.Add(new Button(() =>
            {
                m_Window.ApplyPreview(Parameter(), time.value);
                m_Window.PlayCharacterPreview();
            }) { text = m_Window.PreviewPlaying ? "Playing" : "Play" });
            actions.Add(new Button(m_Window.PauseCharacterPreview) { text = "Pause" });
            actions.Add(new Button(m_Window.StepCharacterPreview) { text = "Step" });
            actions.Add(new Button(m_Window.StopCharacterPreview) { text = "Stop" });
            m_Content.Add(actions);
            if (m_Window.PreviewTarget && m_Window.PreviewTarget.HasPreviewAnimationRuntimeSnapshot)
            {
                AnimationPresentationRuntimeSnapshot runtime = m_Window.PreviewTarget.PreviewAnimationRuntimeSnapshot;
                m_Content.Add(new Label(
                    $"Character Runtime · completion {runtime.CompletionIdentity} · final {runtime.FinalAvailability}/{runtime.FinalInvalidReason}"));
            }
        ...
        void DrawReferences()
        {
            m_Content.Add(new Label($"Asset · {m_Window.Asset.BlendSpaceId}@{m_Window.Asset.ContentRevision}"));
            m_Content.Add(new Label($"Rig · {m_Window.Asset.Rig?.RigId ?? "Unavailable"}"));
            ...
            for (int i = 0; i < m_Window.ReferencingBindings.Count; i++)
                m_Content.Add(new Label($"Producer · {m_Window.ReferencingBindings[i].ProducerId.ProgramProducerIdentity}"));
    ...
    public sealed class CharacterAnimationBlendSpaceEditorWindow : GraphAuthoringEditorShell
    {
        ...
        [SerializeField] CharacterPipelineHost m_PreviewTarget;
        [SerializeField] string m_PreviewProducerIdentity = string.Empty;
        Vector2 m_PreviewParameter;
        float m_PreviewNormalizedTime;
        Guid m_PreviewSessionId;
        ulong m_PreviewTick;
        double m_LastPreviewUpdate;
        float m_LastPreviewTimeSeconds;
        AnimationProducerAuthoringEntry m_ActivePreviewProducer;
        CharacterAnimationBlendSpacePlan m_ActivePreviewPlan;
        ...
        internal CharacterPipelineHost PreviewTarget => m_PreviewTarget;
        internal Vector2 PreviewParameter => m_PreviewParameter;
        internal float PreviewNormalizedTime => m_PreviewNormalizedTime;
        internal bool PreviewPlaying => m_PreviewPlaying;
        internal string PreviewProducerIdentity => m_PreviewProducerIdentity ?? string.Empty;
        internal IReadOnlyList<string> PreviewProducerIdentities => ReferencingBindings
            .Select(binding => binding.ProducerId.ProgramProducerIdentity)
            .OrderBy(value => value, StringComparer.Ordinal)
            .ToArray();
        internal string RuntimeStatus => TryGetRuntimeSnapshot(out _, out string status) ? "Ready" : status;
        internal IReadOnlyList<AnimationProducerPresentationBinding> ReferencingBindings => m_Profile
            ? m_Profile.ProducerBindings.Where(binding => binding != null && binding.SourceKind == AnimationPoseSourceKind.BlendSpace && binding.BlendSpaceSource == m_Asset).ToArray()
        ...
        internal void SetDocument(CharacterAnimationBlendSpaceAsset asset, CharacterPipelineDefinition definition)
        {
            ...
            ResolvePreviewContextDefaults();
            RefreshWorkspace();
        }

        internal void SetPreviewTarget(CharacterPipelineHost target)
        {
            if (ReferenceEquals(m_PreviewTarget, target))
                return;
            StopCharacterPreview();
        ...
        internal void SetPreviewProducer(string producerIdentity)
        {
            string value = producerIdentity?.Trim() ?? string.Empty;
            if (string.Equals(m_PreviewProducerIdentity, value, StringComparison.Ordinal))
                return;
            StopCharacterPreview();
            m_PreviewProducerIdentity = value;
            m_BottomDock?.Refresh();
        }

        internal void PlayCharacterPreview()
        {
            if (!TryResolvePreviewContext(
                    out CharacterPipelineHost target,
                    out AnimationProducerAuthoringEntry producer,
                    out CharacterAnimationBlendSpacePlan plan,
                    out string status))
            {
                m_BottomDock?.Report(status);
                return;
            }
            if (m_PreviewSessionId == Guid.Empty)
                m_PreviewSessionId = Guid.NewGuid();
            m_ActivePreviewProducer = producer;
            m_ActivePreviewPlan = plan;
            m_PreviewPlaying = true;
            m_LastPreviewUpdate = EditorApplication.timeSinceStartup;
            EvaluateCharacterPreview(target, producer, plan, 0f, false);
            m_BottomDock?.Refresh();
        }

        internal void PauseCharacterPreview()
        {
            m_PreviewPlaying = false;
            m_BottomDock?.Refresh();
        }

        internal void StepCharacterPreview()
        {
            m_PreviewPlaying = false;
            if (!TryStartPreviewOperation(out string status))
            {
                m_BottomDock?.Report(status);
                return;
            }
            AdvanceCharacterPreview(1f / 60f);
            m_BottomDock?.Refresh();
        }

        internal void SeekCharacterPreview()
        {
            m_PreviewPlaying = false;
            if (!TryStartPreviewOperation(out string status))
            {
            ...
            EvaluateCharacterPreview(
                m_PreviewTarget,
                m_ActivePreviewProducer,
                m_ActivePreviewPlan,
                0f,
                true);
            m_BottomDock?.Refresh();
        ...
        internal void StopCharacterPreview()
        {
            m_PreviewPlaying = false;
            if (m_PreviewTarget && m_PreviewSessionId != Guid.Empty)
                m_PreviewTarget.ClearTimelinePreview(m_PreviewSessionId);
            m_PreviewSessionId = Guid.Empty;
            m_PreviewTick = 0;
            m_LastPreviewTimeSeconds = 0f;
            m_ActivePreviewProducer = null;
            m_ActivePreviewPlan = null;
        }

        bool TryStartPreviewOperation(out string status)
        {
            if (!TryResolvePreviewContext(
                    out _,
                    out AnimationProducerAuthoringEntry producer,
        ...
        void ResolvePreviewContextDefaults()
        {
            string[] producers = PreviewProducerIdentities.ToArray();
            if (producers.Length == 1)
                m_PreviewProducerIdentity = producers[0];
            else if (!producers.Contains(m_PreviewProducerIdentity, StringComparer.Ordinal))
                m_PreviewProducerIdentity = string.Empty;
            if (m_PreviewTarget && m_PreviewTarget.Definition == m_Definition)
                return;
            CharacterPipelineHost[] targets = Resources.FindObjectsOfTypeAll<CharacterPipelineHost>()
                .Where(target => target && target.gameObject.scene.IsValid() && target.Definition == m_Definition)
                .ToArray();
            m_PreviewTarget = targets.Length == 1 ? targets[0] : null;
        }

        bool TryResolvePreviewContext(
            out CharacterPipelineHost target,
            out AnimationProducerAuthoringEntry producer,
            out CharacterAnimationBlendSpacePlan plan,
            out string status)
        {
            ...
            producer = null;
            plan = null;
            if (!target || target.Definition != m_Definition || !target.CanPreviewTimeline)
            {
                status = "Character Preview requires one explicit scene CharacterPipelineHost using this Definition.";
                return false;
            }
            if (string.IsNullOrEmpty(m_PreviewProducerIdentity))
            {
                status = "Character Preview requires one explicit Blend Space producer binding.";
                return false;
            }
            if (!TryResolveCompiledPlan(out plan, out status))
                return false;
            try
            {
                IReadOnlyList<AnimationProducerAuthoringEntry> producers =
                    CharacterAnimationPresentationAuthoringService.DiscoverProducerTracks(m_Definition);
                producer = producers.SingleOrDefault(value => string.Equals(
                    value.ProgramProducerIdentity,
                    m_PreviewProducerIdentity,
            ...
            if (producer == null)
            {
                status = $"Producer '{m_PreviewProducerIdentity}' is absent from the Definition composition roots.";
                return false;
            }
            AnimationProducerPresentationBinding binding = m_Profile.FindProducerBinding(producer.ProducerId);
            if (binding == null || binding.SourceKind != AnimationPoseSourceKind.BlendSpace ||
                binding.BlendSpaceSource != m_Asset)
            {
                status = $"Producer '{m_PreviewProducerIdentity}' is not bound to this Blend Space asset.";
                return false;
            }
            status = "Ready";
            return true;
        ...
        void AdvanceCharacterPreview(float deltaSeconds)
        {
            if (!m_PreviewTarget ||
                m_PreviewTarget.Definition != m_Definition ||
                !m_PreviewTarget.CanPreviewTimeline ||
                m_PreviewSessionId == Guid.Empty ||
                m_ActivePreviewProducer == null ||
                m_ActivePreviewPlan == null)
            {
                m_PreviewPlaying = false;
                m_BottomDock?.Report("Character Preview session is unavailable. Press Play, Step, or Seek to start an explicit session.");
                return;
            }
            float duration = Mathf.Max(0.0001f, m_ActivePreviewPlan.ClockDurationSeconds);
            m_PreviewNormalizedTime = Mathf.Repeat(m_PreviewNormalizedTime + Mathf.Max(0f, deltaSeconds) / duration, 1f);
            EvaluateCharacterPreview(
                m_PreviewTarget,
                m_ActivePreviewProducer,
                m_ActivePreviewPlan,
        ...
        void EvaluateCharacterPreview(
            ...
            bool reset)
        {
            float currentTime = m_PreviewNormalizedTime * plan.ClockDurationSeconds;
            float previousTime = reset || m_PreviewTick == 0 ? currentTime : m_LastPreviewTimeSeconds;
            try
            {
                target.EvaluateBlendSpacePreview(
                    ...
                    reset,
                    m_PreviewParameter);
                m_LastPreviewTimeSeconds = currentTime;
                m_BottomDock?.Report(target.HasPreviewAnimationRuntimeSnapshot
                    ? $"Character Preview completion {target.PreviewAnimationRuntimeSnapshot.CompletionIdentity}."
                    : "Character Preview evaluated; runtime snapshot is unavailable.");
            }
            catch (Exception exception)
            {
                m_PreviewPlaying = false;
        ...
        void Update()
        {
            ...
            float delta = Mathf.Clamp((float)(now - m_LastPreviewUpdate), 0f, 0.1f);
            m_LastPreviewUpdate = now;
            AdvanceCharacterPreview(delta);
        }
        ...
        {
            m_PreviewWeights.Clear();
            if (m_ActivePreviewPlan == null ||
                !TryEvaluatePreview(m_ActivePreviewPlan, out BlendSpacePreviewEvaluation evaluation, out _))
            {
                return;
            }
            for (int i = 0; i < evaluation.Weights.Count; i++)
            ...
            out CharacterPresentationProjectionAsset projection)
        {
            ...
            for (int i = 0; i < guids.Length; i++)
            {
                ...
                if (!candidateProfile)
                    continue;
                bool referenced = candidateProfile.ProducerBindings.Any(binding =>
                    binding != null && binding.SourceKind == AnimationPoseSourceKind.BlendSpace && binding.BlendSpaceSource == asset);
                if (referenced)
    ...
    public sealed class CharacterAnimationBlendSpaceAssetEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField("BlendSpaceId", asset.BlendSpaceId.Value);
            EditorGUILayout.LabelField("Content Revision", asset.ContentRevision);
