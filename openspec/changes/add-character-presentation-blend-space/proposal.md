...
- 新增 `CharacterAnimationBlendSpaceAsset`，保存稳定资产身份、Rig、模式、轴、稳定 SampleId、直接AnimationClip引用、样本坐标、时间策略、显式Phase Reference Sample、Foot Analysis引用和显式Pose Parameter解析策略。
...
- Corin持续Locomotion只走`Presentation Fact -> PoseStateMachine -> ClipPlayer或正式BlendSpacePlayer -> Standard Blend`。Blend Space只在样本齐备时接入；素材不足时使用明确ClipPlayer State，不得恢复Gameplay BaseLocomotion Selection、Timeline locomotion producer、Locomotion Inertialization或Timeline/BlendSpace双写。
...
- 每个BlendSpacePlayer必须位于PoseState inline subgraph并解析唯一正式BlendSpace Pose source binding；节点位于根图、绑定Clip/MM资源或轴接口与Rig不一致时编译失败。
- LocomotionPhase缺少Phase Reference、共同Profile Group、合法per-clip Phase plan或样本绑定时编译失败，不回退到normalized time。
