...
Timeline视口已经收敛为固定工具栏/时间标尺与同步双栏纵向滚动。普通Wheel纵向浏览Track，Shift+Wheel横移，Ctrl+Wheel缩放，Middle Drag只横移；内联unbounded Curve仅用Alt+Wheel与Alt+Shift+Wheel调整纵向视图。Marker绘制线拥有18px透明命中区域并直接使用父Track坐标拖动；None轨道会先选中正式Inspector配置，不再暴露必然失败的新增入口。
