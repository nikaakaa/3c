...
namespace ThirdPersonCharacter.Pipeline.Simulation.DeterministicRollback
{
        ...
        IDisposable
    {
        ...
        public string AdapterIdentity => $"UnityInputSystem/FixedQ32.32/{m_Program.ProgramHash}";
        ...
        public FixedCharacterSimulationInput BuildInput(RollbackLocalInputBuildContext context)
        {
            RequireAlive();
        ...
        public IRollbackLocalInputAdapterCheckpoint CaptureCheckpoint()
        {
            RequireAlive();
            var pending = new PendingRequest[m_PendingRequests.Count];
            for (int i = 0; i < pending.Length; i++)
                pending[i] = m_PendingRequests[i].Clone();
            return new Checkpoint(this, m_RequestSequence, pending);
        }

        public void RestoreCheckpoint(IRollbackLocalInputAdapterCheckpoint checkpoint)
        {
            RequireAlive();
            if (checkpoint is not Checkpoint value || !ReferenceEquals(value.Owner, this))
                throw new ArgumentException("Unity Fixed Input Adapter checkpoint belongs to another adapter.", nameof(checkpoint));
            m_RequestSequence = value.RequestSequence;
            m_PendingRequests.Clear();
            for (int i = 0; i < value.PendingRequests.Length; i++)
                m_PendingRequests.Add(value.PendingRequests[i].Clone());
        }

        public RollbackLocalInputDiagnosticsSnapshot CaptureDiagnostics()
        {
            int count = 0;
            ...
            return new RollbackLocalInputDiagnosticsSnapshot(count, oldestCaptureTick, oldestEligibleTick);
        }
        ...
        sealed class PendingRequest
        {
            ...
            public PendingRequest Clone()
            {
                var result = new PendingRequest(
                    ...
                    TimingClass);
                if (CaptureTick != 0)
                    result.Schedule(CaptureTick, checked((int)(EligibleTick - CaptureTick)));
                return result;
            }
        }

        sealed class Checkpoint : IRollbackLocalInputAdapterCheckpoint
        {
            public Checkpoint(
                UnityFixedCharacterInputAdapter owner,
            ...
            public UnityFixedCharacterInputAdapter Owner { get; }
            public ulong RequestSequence { get; }
            public PendingRequest[] PendingRequests { get; }
        }
