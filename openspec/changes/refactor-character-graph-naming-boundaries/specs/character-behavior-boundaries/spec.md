...
### Requirement: Submitter 组合不得称为 Graph
角色帧提交器的顺序组合 MUST 使用 `CharacterFrameSubmitterChain` 或已经批准的等价 chain/composite 名称。submitter 组合 MUST NOT 被命名为 graph 并作为后续行为编辑器或行为 runtime 的扩展入口。

#### Scenario: Submitter chain 名称反映职责
- **WHEN** Locomotion submitter 和 Action submitter 仍以顺序组合参与 frame pipeline
- **THEN** 该组合 MUST 被命名为 chain、collection、composite 或等价职责名称
...
- **AND** MUST NOT 把旧 submitter graph 当成行为树 runtime
