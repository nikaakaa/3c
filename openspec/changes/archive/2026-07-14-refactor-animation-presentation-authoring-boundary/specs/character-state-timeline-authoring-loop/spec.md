...
### Requirement: Corin动画Driver必须配置在完整Tree Presentation投影

**Reason**: Corin 不再使用 Tree lifecycle Driver；CharacterPipelineDefinition Inspector 只按正式 producer identity 编辑 Animancer transition binding，不复制逻辑关系。

#### Scenario: 删除 Driver

- **WHEN** Corin 迁移完成
- **THEN** NodeEnter、NodeRelease 与 EdgeCommit MUST不保存或解析动画 Driver

### Requirement: Corin 必须由 Pipeline 解析 Base Previous 与 Desired

**Reason**: Previous/Desired、ExecutionLineage、Registry candidate 与 LayerPlan 仲裁已删除。Base 端点由 Current playback 与逻辑 selected playback 明确给出。

#### Scenario: Base 切换

- **WHEN** Base selection 从 Locomotion A 变为 Action B
- **THEN** AnimationPlaybackLifecycle MUST使用 Current A 与 selected B
- **AND** MUST不遍历普通 Tree 节点解析端点
