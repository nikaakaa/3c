...
{
    internal readonly struct TimelineAnimationMarkerSelection
    {
        public TimelineAnimationMarkerSelection(AnimationTrack track, string markerAuthoringId)
        {
            Track = track;
    ...
    internal sealed class TimelineAnimationMarkerView : VisualElement, ISelectable
    {
        ...
        public TimelineAnimationMarkerView(
        ...
        public AnimationTrack Track { get; }
        public AnimationSyncMarker Marker { get; }
        public TimelineAnimationMarkerSelection Selection => new TimelineAnimationMarkerSelection(Track, Marker.AuthoringId);
        ...
        void OnPointerDown(PointerDownEvent evt)
        {
            ...
            {
                if (!evt.actionKey)
                    m_TrackView.FieldView.ClearSelection();
                m_TrackView.FieldView.AddToSelection(this);
            }
            if (m_RuntimeReadOnly)
                return;
            m_OriginalFrame = Marker.Frame;
            m_TargetFrame = m_OriginalFrame;
            m_DragChanged = false;
            m_PointerId = evt.pointerId;
            m_Draft.Begin(m_OriginalFrame, unchecked((ulong)m_OriginalFrame));
        ...
        {
            if (m_RuntimeReadOnly || !m_Draft.IsActive || evt.pointerId != m_PointerId || !this.HasPointerCapture(evt.pointerId))
                return;
            TimelineFrameGeometry geometry = m_TrackView.FieldView.Geometry;
        ...
        {
            if (!m_Draft.IsActive || evt.pointerId != m_PointerId)
                return;
            FinishDrag(true);
            evt.StopImmediatePropagation();
        }
        ...
        void FinishDrag(bool commit)
        {
            ...
            int target = m_TargetFrame;
            bool changed = m_DragChanged;
            m_PointerId = -1;
            m_DragChanged = false;
            if (commit) m_Draft.Complete();
            else m_Draft.Cancel();
            ...
            if (commit && changed)
            {
                ...
                m_TrackView.FieldView.CommitAuthoringMutation(
                    () => Track.MoveMarker(markerAuthoringId, target),
                    "Move Animation Sync Marker");
            }
            else
            {
                Refresh();
        ...
        void ShowContextMenu()
        {
            ...
            menu.AddItem(new GUIContent("Rename Sync Marker..."), false, () => m_TrackView.ShowMarkerTextEntry(Track, Marker));
            menu.AddItem(new GUIContent("Delete Sync Marker"), false, () =>
                m_TrackView.FieldView.CommitAuthoringMutation(
                    () => Track.DeleteMarker(Marker.AuthoringId),
                    "Delete Animation Sync Marker"));
            menu.ShowAsContext();
        }

        public override bool Overlaps(Rect rectangle)
        {
            return worldBound.Overlaps(rectangle, true);
        }
