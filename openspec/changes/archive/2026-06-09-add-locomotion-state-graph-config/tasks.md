...
## 8. Animancer alias 映射边界
- [x] 8.1 不新增 `LocomotionAnimationSetSO` 或等价二次动画映射资产。
- [x] 8.2 presenter 使用 `Idle` alias。
- [x] 8.3 presenter 使用 `MoveStart + Walk` -> `WalkStart` alias。
- [x] 8.4 presenter 使用 `MoveStart + Run` -> `RunStart` alias。
- [x] 8.5 presenter 使用 `MoveLoop + Walk` -> `WalkLoop` alias。
- [x] 8.6 presenter 使用 `MoveLoop + Run` -> `RunLoop` alias。
- [x] 8.7 presenter 使用 `MoveStop + Walk` -> `WalkEnd` alias。
- [x] 8.8 presenter 使用 `MoveStop + Run` -> `RunEnd` alias。
- [x] 8.9 实际动画资源、淡入和 transition 参数由 Animancer TransitionLibrary alias 表管理。
- [x] 8.10 动画 presenter 不写入角色位置、速度或状态机。
- [x] 8.11 删除项目侧默认动画映射资产，避免和 Animancer alias 表形成重复权威。

## 9. 动画 presenter 接入
- [x] 9.1 `BasicLocomotionAnimancerPresenter` 不持有项目侧动画映射资产引用。
- [x] 9.2 presenter 根据移动上下文生成标准 Animancer alias key。
...
- [x] 9.9 迁移当前演示场景或 prefab，移除 `animationSet` 绑定。
...
## 12. EditMode 测试：动画 alias 边界
- [x] 12.1 测试 presenter 不暴露项目侧 AnimationSet 字段或属性。
- [x] 12.2 测试项目源码、场景、prefab 和配置资产不再引用 AnimationSet 类型或 `animationSet` 字段。
- [x] 12.3 测试 Presenter 仍不引用状态图 builder 或运动执行具体实现。
...
- [x] 14.2 删除基础移动默认动画集资产。
- [x] 14.3 将当前玩家 prefab 或演示场景绑定默认状态图资产。
- [x] 14.4 移除当前动画 presenter 的默认动画集资产绑定。
...
- [x] 15.3 运行项目侧 AnimationSet 移除相关 EditMode 测试。
...
- [x] 17.4 向用户说明 `RunEnd` 这类动画变体通过 Animancer alias 表配置。
