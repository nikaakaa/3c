...
#### Scenario: Tick 中出现未预期异常

- **WHEN** Begin、Apply 或 Remove 的内部不变量失败并抛出异常
- **THEN** Runtime MUST 恢复该 Tick 开始前的 CurrentTick、Attribute、Tag、ActiveEffect、PredictionJournal 和 ChangeSet
- **AND** 当前 Tick MUST 关闭，调用者 MAY 使用更大的新 Tick 重新 Begin
