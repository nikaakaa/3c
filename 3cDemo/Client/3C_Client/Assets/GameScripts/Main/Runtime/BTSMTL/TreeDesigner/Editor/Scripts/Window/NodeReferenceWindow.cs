...
namespace TreeDesigner.Editor
{
    public class NodeReferenceWindow : EditorWindow
    {
        ...
        List<BaseTreeAsset> m_TargetTrees = new List<BaseTreeAsset>();
        ...
        private void Update()
        {
            ...
            if (m_Started)
            {
                if (m_CurrentIndex < TreeModificationProcessor.TreeLocations.TreeInfos.Count)
                {
                    ...
                    BaseTreeAsset treeAsset = AssetDatabase.LoadAssetAtPath<BaseTreeAsset>(treeLocationInfo.path);
                    if (treeAsset && treeAsset.Tree.Nodes.Find(i => i.GetType() == m_TargetType) != null)
                        m_TargetTrees.Add(treeAsset);
                ...
                else
                {
                    ...
                    m_TreeContainer = new IMGUIContainer(() =>
                    {
                        ...
                        GUI.enabled = false;
                        m_TargetTrees.ForEach(i => EditorGUILayout.ObjectField(i, typeof(BaseTreeAsset), false));
                        GUI.enabled = true;
                    });
