...
系统 MUST 使用项目已安装的 UnityHFSM 管理基础 Locomotion 的 `Idle / MoveStart / MoveLoop / MoveStop` 阶段，并 MUST 保留当前基础移动阶段语义。当前阶段机 MUST NOT 把 `Run`、`Walk` 或 Animancer alias 建模为逻辑状态。时间型阶段退出 MUST 从纯数据 phase timing 判断，而不是从 Animancer 播放状态判断。
...
#### Scenario: 起步达到退出时长进入 MoveLoop
- **GIVEN** 当前阶段为 `MoveStart`
- **AND** 本帧持续存在移动意图
- **WHEN** `MoveStart` 阶段计时达到当前 phase timing 的退出时长
...
- **AND** 本帧没有移动意图
- **WHEN** `MoveStop` 阶段计时达到当前 phase timing 的退出时长
...
#### Scenario: Manual 阶段不因时间自动退出
- **GIVEN** 当前阶段的 phase timing 策略为 `Manual`
- **WHEN** 阶段计时持续增加
- **THEN** 时间型退出条件 MUST 保持 false
- **AND** 阶段机 MAY 继续通过其它条件切换状态

#### Scenario: alias 不进入逻辑状态
...
#### Scenario: 状态机不读取动画播放资产
- **WHEN** 状态机判断当前 phase 是否达到退出时间
- **THEN** 状态机 MUST 只读取纯 phase timing 数据
- **AND** MUST NOT 读取 Animancer TransitionLibrary
...
#### Scenario: 自动测试覆盖 phase timing
- **WHEN** phase timing 被配置
- **THEN** EditMode 测试 MUST 覆盖 `MoveStart` 未达到退出时长时保持 `MoveStart`
- **AND** MUST 覆盖 `MoveStart` 达到退出时长后进入 `MoveLoop`
- **AND** MUST 覆盖 `MoveStop` 未达到退出时长时保持 `MoveStop`
- **AND** MUST 覆盖 `MoveStop` 达到退出时长后回到 `Idle`
- **AND** MUST 覆盖 `Manual` 策略不触发时间型退出

#### Scenario: 自动测试覆盖停止期间打断
- **WHEN** `MoveStop` 正在等待 phase timing 退出时长
- **THEN** EditMode 测试 MUST 覆盖重新输入立即回到 `MoveStart`
- **AND** MUST 验证该切换不等待 `MoveStop` 退出时长完成
...
- **AND** MUST 能确认不引用 `AnimationClip`
- **AND** MUST 能确认不引用 `TransitionLibrary`
