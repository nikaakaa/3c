...
namespace ThirdPersonCharacter.Pipeline.Editor
{
    ...
    public sealed class CharacterPipelineHostEditor : UnityEditor.Editor
    {
        ...
        static void DrawSessionControls(RuntimeDebugSession session, RuntimeDebugViewModel view)
        {
            ...
            EditorGUILayout.LabelField("Revision", $"{view.Target.Revision.CompilationRevision} / {view.Target.Revision.SourceContentHash}");
        ...
        static void DrawMotion(RuntimeDebugViewModel view)
        {
            ...
            {
                RuntimeTracePayload payload = eventView.Event.Payload;
                return eventView.Event.Kind == RuntimeTraceEventKind.MotionContribution
                    ? $"{payload.Name} | {payload.Status} | P{payload.Priority} w{payload.Weight:0.###} | {payload.Value.DisplayValue()}"
                    : $"Resolved | {payload.Status} | {payload.Value.DisplayValue()} | yaw {payload.Time:0.###}/{payload.SecondaryTime:0.###}";
            });
        }
