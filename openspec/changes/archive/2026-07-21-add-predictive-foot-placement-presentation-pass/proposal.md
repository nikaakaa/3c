...
- 由Editor-only Foot Analysis按共享Rig Calibration采样每个AnimationClip的heel/toe轨迹，以统一校准地面、最低接触点和脚底垂直速度生成左右脚plant/landing事实；Runtime按Marker Sync后的连续视觉时间倍率采样这些事实，再与Body平移和yaw角速度合成世界接触点速度。最终混合姿势差分只用于非法跳变诊断，不新增 BTSMTL FootPhase、Timeline window、Blackboard variable 或第二份 locomotion gait phase 数据。
...
- 让每只脚只拥有 `Free`、`Locked`、`Sliding` 三种正式约束状态，并通过 plant/release 权重连续过渡。Current heel/toe必须独立查询并构造唯一virtual support plane；锁定位置保存为显式surface owner的Collider局部锚点。超过replant限制时旧约束必须先在Free中释放到零，随后才允许提交新锚点，不增加隐藏Replant状态或固定帧计时器。
- 根据当前支撑腿和两腿可达范围计算骨盆垂直偏移；使用临界阻尼、上下限和 ascent/descent 规则抑制楼梯上的身体跳动。计划中的骨盆偏移 MUST以VisualRoot组件空间竖直标量表达，solver写入具体骨骼前 MUST转换到pelvis父骨空间，不得把预旋转骨架的local Y当作角色竖直方向。Foot Placement 不旋转或移动 VisualRoot，不修改 Camera anchor 和 Gameplay Body。
...
- Animancer adapter 在完成本帧 Evaluate 后提供只读 visible playback contribution：producer identity、playback generation、sample time、cycle、Marker Sync后的连续视觉时间倍率与实际state weight。Foot Placement以该正式visual sample time复用同一Projection clip binding采样单一Foot Placement Weight和生成特征，再按最终视觉权重混合；暂停、重定位和rebase不得伪造脚速或未来落地。
...
- Current heel/toe保留独立合法support并形成唯一virtual support plane；Replant不得在释放旧锚点的同一表现帧满权重重新锁定。
...
- 同一Foot Placement Weight对脚位置、旋转、摆脚净空与Pelvis只应用一次；运行时世界接触速度包含动画局部脚速、Body可见线速度和yaw角速度。
