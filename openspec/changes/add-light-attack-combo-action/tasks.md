...
- [ ] 0.7 确认如需绕过 FullBody 主线、统一状态机、输入缓冲或 motion executor，立即停止并回到 OpenSpec。
...
- [ ] 1.5 搜索 `BuildDodgeRequestFact` 和 `FullBodyActionInterruptGate`，确认准入门当前形态。
...
## 5. Attack 请求构建和仲裁
- [ ] 5.1 新增 Attack 请求构建器或扩展现有 FullBody Action 请求构建器。
...
- [ ] 5.10 请求进入状态机事实前调用 `ActionInterruptArbiter`。
...
- [ ] 7.4 攻击 active 时 FullBody owner 为 Action。
- [ ] 7.5 攻击 active 时 Locomotion 不提交平面位移。
- [ ] 7.6 攻击 active 时 Locomotion 不提交 base layer 动画。
- [ ] 7.7 攻击可选位移通过 `IActionMovementExecutor` 或等价统一运动出口提交。
- [ ] 7.8 攻击可选转向通过现有运动/朝向输出边界提交。
- [ ] 7.9 动画 Presenter 只消费动作动画命令。
...
## 12. 手动验证
- [ ] 12.1 在 Unity Editor 打开 `Assets/Scenes/Sandbox.unity`。
- [ ] 12.2 绑定正式轻攻击三段配置和三段动作动画引用。
- [ ] 12.3 进入 Play Mode。
- [ ] 12.4 按一次 Attack，确认角色进入 Attack01 并播放第一段动作。
- [ ] 12.5 在 Attack01 连段窗口前按 Attack，确认不会立刻跳段。
- [ ] 12.6 在 Attack01 连段窗口内按 Attack，确认进入 Attack02。
- [ ] 12.7 在 Attack02 连段窗口内按 Attack，确认进入 Attack03。
- [ ] 12.8 Attack03 完成后确认回到 Locomotion。
- [ ] 12.9 攻击过程中输入 WASD，确认基础移动不会叠加平面位移。
- [ ] 12.10 攻击结束后输入 WASD，确认基础移动恢复。
- [ ] 12.11 攻击过程中移动鼠标或摇杆 Look，确认相机仍响应。
- [ ] 12.12 不绑定攻击动画引用时运行校验，确认报错而不是使用 fallback。
- [ ] 12.13 Console 中保留攻击准入和状态路径诊断 log。

## 13. OpenSpec 和构建验证
- [ ] 13.1 运行 `openspec validate add-light-attack-combo-action --strict --no-interactive`。
- [ ] 13.2 修复所有 OpenSpec validation 问题。
- [ ] 13.3 运行定向 EditMode 测试。
- [ ] 13.4 运行相关现有 EditMode 测试，至少包含输入缓冲、Action 仲裁、统一状态机、FullBody replay。
- [ ] 13.5 记录用户可复现的手动验证步骤和预期结果。
