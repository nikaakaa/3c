    ...
    public sealed class CharacterAnimationPresentationProfileEditor : UnityEditor.Editor
    {
            ...
            CharacterPresentationPoseSourceSlot slot)
        {
            ...
            using (new EditorGUI.DisabledScope(true))
            {
                ...
                EditorGUILayout.ObjectField("Rig", profile.RigDefinition, typeof(CharacterAnimationRigDefinition), false);
            }
            if (!binding)
                EditorGUILayout.HelpBox("This Source Slot has no Profile binding.", MessageType.Error);
            else if (binding is CharacterClipPoseSourceBinding clipBinding)
            {
            ...
            using (new EditorGUI.DisabledScope(!binding || !binding.SourceAsset))
            {
                ...
                if (GUILayout.Button("Open Animation Clip"))
                {
                    if (binding is CharacterClipPoseSourceBinding clipBinding && clipBinding.Clip)
            ...
            CharacterPresentationPoseSourceSlot slot)
        {
            ...
            if (cached.Count == 0)
                EditorGUILayout.HelpBox("No PoseState ClipPlayer currently consumes this source.", MessageType.Warning);
        }
