...
- 打开 Sandbox 场景，启用 Locomotion、Animation、Input 相关诊断日志开关。
- 前进移动后切后退，确认先出现统一管线 derived facts，再出现 TurnBack 状态。
- 横向 A/D 切换不应误触发前后 TurnBack。
- TurnBack 期间确认 `appliedInputRotation=False`。
- TurnBack 期间确认 `appliedInputPlanarMovement=False`。
- 动画结束后确认回到 MoveLoop 或 Idle。
