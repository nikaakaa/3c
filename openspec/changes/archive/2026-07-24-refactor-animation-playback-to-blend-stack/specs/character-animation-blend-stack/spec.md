...
每个Blend Stack节点 MUST引用唯一`CharacterAnimationBlendPolicy`。Policy MUST保存容量、Stored Pose策略、canonical curve、Blend Profile、authoring default rule与exact source-target override。Compiler MUST只枚举该节点可达Selection endpoint与Empty组合，并把default和override物化为完整CrossFade exact table；Runtime缺少pair MUST失败，MUST不fallback到默认时长、线性曲线、Inertial或Animancer fade。
