...
任何相同输入下的脚修正、状态时机映射、Anchor、Primary Support、Pelvis或Goal差异都属于重构回归，不能解释为优化。后续先由`build-character-foot-motion-data-foundation`建立AnimationClip脚步数据作者真相；Contact Plan、空间Swing、FootPath、Lock和Landing腿约束必须等数据归档后重新提案，不得在本change实施。
