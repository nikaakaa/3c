...
FutureLanding世界事实固定拆成`Raw Landing -> Prediction Input Gate -> canonical Landing Observation -> Acceptance`。Raw Landing仍从每帧不可变Frame Input重新投影；每脚Committed Observation同时保存最近一次实际查询使用的Raw Landing与Component Up输入快照。同Source/Cycle/Event/Profile/World Revision下，新Raw Landing相对该快照的累计位移不超过`PredictionInputUpdateDistance`且Component Up夹角不超过`PredictionInputUpAngleDegrees`时必须复用Committed Accepted或Rejected Observation，不得建立新Key或执行SphereCast。

Corin正式配置固定为5厘米输入累计位移与1度Component Up变化。超过任一阈值，或Source、Cycle、Landing Event、Profile Revision、World Revision发生变化时，Runtime才由Side、Landing Event、按1毫米量化的Raw Landing、按`1e-4`量化的Component Up、Profile Revision与World Revision构造新Observation Key。SphereCast必须使用Key反量化后的canonical几何，新Key恰好查询一次并只选择canonical最近合法Surface。Accepted与Rejected查询结果都属于不可变Observation；Pending根事务失败时不得提交新Page。

上一Committed Surface、Frame、Authority Tick、Trajectory Generation、Future Translation Source、Foot State、Residual与查询输出不得进入Observation Key或候选选择。上一Surface不得传入World Query或改变候选选择；它只允许在查询完成后参与正式换代分类。新Observation与同Surface NextSwingLanding距离小于`LandingPointAcceptanceDistance`时保留原Landing；Surface变化、Event变化或距离达到2厘米时正式换代。1毫米Key量化只定义实际查询的canonical几何，5厘米输入阈值只定义是否需要查询，2厘米Landing接受阈值只定义查询结果是否改变正式Path，三者不得合并。
...
Ground Path Input identity只表示查询输入lineage，不单独触发Residual重置。Interpolation Runtime只有在Event、Path可用性、正式Accepted Landing端点或正式Swing目标发生有效Revision时捕获`PreviousOutput - NewTarget`。Corin的`SwingRevisionDistance`固定为2厘米，`ResidualLandingTolerance`和`ReleaseCompletionDistance`各固定为5毫米；四项配置必须分别序列化，不得继续用一个`LandingUpdateDistance`同时决定Landing换点、Residual Revision、Landing截止与Release完成。原始Builder目标与State Target继续分列诊断，不得互相改名覆盖。
...
在上述Correction链已经连续后，普通Swing目标使用统一Interpolation State中的Residual。基础半衰期仍来自Profile；当Residual大于`ResidualLandingTolerance`时，Interpolation Runtime按剩余Step Time计算保证在Landing前收敛到5毫米容差所需的半衰期，并取它与基础半衰期的较小值。没有有效Step Time时不得猜测截止时间，只能发布明确输入不可用。Step Time只解决Landing前仍有Residual欠账，不负责改变Raw Target、重选State Output或修正同帧放大。
...
Contact Anchor同时保存Point与Normal。旋转目标以动画脚踝Forward投影到Contact平面后作为Yaw方向，以Contact Normal作为Up构造；投影退化时当前Foot帧typed invalid，不猜测Body Forward。Landing、Locked与Sliding的Rotation Weight等于Foot Placement Weight与正式Lock Weight的乘积；Releasing通过同一Interpolation连续回到动画脚踝旋转。FBBIK不得在Foot Placement之外补算第二旋转目标。
