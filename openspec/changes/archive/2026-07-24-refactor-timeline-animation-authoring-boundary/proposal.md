...
本change建立的Timeline Core边界、typed Marker/Curve编辑、按需Analysis和artifact Build仍然保留。后续`refactor-animation-control-boundaries`把纯Locomotion Clip、marker、Foot Placement Weight与Analysis identity迁到Profile Pose source；Timeline Editor只继续拥有有限Action及其它真实Gameplay Timeline内容。两类作者入口可以复用同一底层descriptor和artifact builder，但不得共享可写owner。若本change晚于该后续change归档，归档时 MUST按新current model收窄Timeline范围，不得恢复RunLoop Timeline作为持续Pose权威。
