...
- [x] 4.1 从现有Rig参考姿态与Profile准备过程取得精确typed方向初值及身份，非法准备显式失败，不新增默认轴或近似初始化
- [x] 4.2 在明确清空Solver历史的初始化、Reset与调参路径统一重建正式方向，每帧仅由正式输入设置Vendor工作字段
- [x] 4.3 删除空BendHistory时读取旧Vendor方向的路径，保护a40b71f可靠动画腿轴运输、Stable/Applied含义与既有退化分支，不恢复半球强翻或SmoothKnee
- [x] 4.4 通过既有正式初始化/Reset入口保存新建与完全重置的对照，以及普通历史帧回归证据；缺少覆盖时保持本项未完成

## 5. 配套诊断记录与采样映射统一

- [x] 5.1 在已完成的紧凑诊断链上按业务分组响应、Contact、Support、Reach/Pelvis证据，删除同义平铺拷贝，保留固定容量和同帧冻结
- [x] 5.2 建立唯一Editor typed列绑定，在现有初始化入口一次检查名称、类型、单位、有效性和读写覆盖，不增加逐层重复校验
- [x] 5.3 由同一绑定驱动Header、CSV写行、Analyzer读取和必需列校验，删除旧手工映射，保留单次解析、几何独立表和原始帧字节索引
- [x] 5.4 集中当前格式identity，纯映射整理保持原ABI；真实语义变化才升级，删除被实际替换的旧reader/别名，不恢复展开facts.json
- [x] 5.5 在独立输出目录对账字段、全部42项诊断、评分、紧凑明细及查询索引，保持规则与事件完整性，保留全部历史原包

## 6. 清理与收尾

- [x] 6.1 按权威读取迁移清单清除旧类型、非权威过程读取、重复决定、未消费字段与映射，确认没有第二生命周期、第二采样器或图外Goal后处理
