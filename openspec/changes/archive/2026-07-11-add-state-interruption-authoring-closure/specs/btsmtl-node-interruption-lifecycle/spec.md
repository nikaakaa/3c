...
BT ConditionRuleGraph 和 AbortPolicy MUST 只决定分支选择与结构停止。Tree edge、Selector 和通用 Runnable stop runtime MUST NOT 直接提交 Action lifecycle、取消特定业务 Timeline、写网络 packet 或决定动画 clip。具体节点和下层 runtime MUST 在自己的 stop callback 中处理所属资源。

#### Scenario: Selector 抢占 Attack SMNode

- **WHEN** Selector 发起 LowerPriority abort
- **THEN** Selector MUST 只传递 NodeStopContext 并等待 StopCompleted
- **AND** Action terminal lifecycle MUST 由 Attack State.OnExit 显式提交
