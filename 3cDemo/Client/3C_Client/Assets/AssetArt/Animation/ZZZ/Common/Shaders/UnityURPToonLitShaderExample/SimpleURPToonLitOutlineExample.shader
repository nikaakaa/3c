...
0.UniversalForwardOnly  pass    (toon lit surface rendered by URP's normal opaque pass)
1.ZZZToonOutlineSource  pass    (marker pass selected only by ZZZToonOutlineRendererFeature)
...
The outline is rendered as a second draw by ZZZToonOutlineRendererFeature with ZZZToonOutline.shader.
The second SubShader is an unlit surface used by Unity's built-in AnimationClip Inspector preview.
...
Shader "SimpleURPToonLitExample(With Outline)"
{
    ...
    {       
        ...
        Pass
        {
            Name "ZZZToonOutlineSource"
            Tags
            {
                "LightMode" = "ZZZToonOutlineSource"
            }
            ...
            HLSLPROGRAM
            #pragma target 2.0
            #pragma vertex ZZZToonOutlineSourceVertex
            #pragma fragment ZZZToonOutlineSourceFragment

            #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"

            struct ZZZToonOutlineSourceAttributes
            {
                float3 positionOS : POSITION;
            };

            struct ZZZToonOutlineSourceVaryings
            {
                float4 positionCS : SV_POSITION;
            };

            ZZZToonOutlineSourceVaryings ZZZToonOutlineSourceVertex(ZZZToonOutlineSourceAttributes input)
            {
                ZZZToonOutlineSourceVaryings output;
                output.positionCS = TransformObjectToHClip(input.positionOS);
                return output;
            }

            half4 ZZZToonOutlineSourceFragment() : SV_Target
            {
                return 0;
            }
            ENDHLSL
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CGPROGRAM
            #pragma target 2.0
            #pragma vertex EditorPreviewVertex
            #pragma fragment EditorPreviewFragment
            ...
            struct EditorPreviewAttributes
            {
                float4 positionOS : POSITION;
                float2 uv : TEXCOORD0;
            };

            struct EditorPreviewVaryings
            {
                float4 positionCS : SV_POSITION;
                float2 uv : TEXCOORD0;
            };

            EditorPreviewVaryings EditorPreviewVertex(EditorPreviewAttributes input)
            {
                EditorPreviewVaryings output;
                output.positionCS = UnityObjectToClipPos(input.positionOS);
                output.uv = TRANSFORM_TEX(input.uv, _BaseMap);
                return output;
            }

            fixed4 EditorPreviewFragment(EditorPreviewVaryings input) : SV_Target
            {
                fixed4 color = tex2D(_BaseMap, input.uv) * _BaseColor;
                color.a = 1;
                return color;
            }
            ENDCG
