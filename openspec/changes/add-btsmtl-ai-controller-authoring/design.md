...
- Session snapshot/restore包含AI State participant；不支持该participant的Composition在Active前拒绝AI Actor。
...
它不能立刻进入三个网络产品，但不会污染Network Model。明确拒绝不支持的Composition比在客户端临时运行AI或回退Neutral更安全。Authority与Fixed AI可以在同一AI Semantic IR上分别增加正式Target/Host。
