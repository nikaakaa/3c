...
| `0x171D6920`为每脚目标高度保存历史并执行`delta=clamp(target-history, ±rate×dt)`，同时由事件位控制更新、冻结与强制刷新 | 每脚唯一Target Height历史保存Accepted Landing沿Up高度；Swing输出为`正式Raw Height + Filtered Landing Height - Current Landing Height`，正常Phase直接通过，只有同Event Landing高度换代按`MaximumVerticalTargetSpeed`限速，Plant接管时冻结Swing更新并继续同一历史 | Interpolation Runtime Target Height Stage |
