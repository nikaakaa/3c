...
Compiler将参数降低为Target typed constants，并在ProgramExecutionLayout创建operation handle到Slot、Route、Equipment和Parameter的类型化索引。Graph通过`ReadEquipmentParameter`按Action Equipment Context或显式Slot/revision读取。Tick热路径不拼接字符串key、不扫描catalog，也不使用SerializedProperty或Unity asset查找。
...
Host不读取Feature asset、不回调C# handler、不复制控制流解释器。Unknown entry、重复entry或stale generation必须结构化失败。Persistent旧entry由唯一Runnable control递归ForceStop并清空operation state；Route continuation携带不可变Equipment revision，换装后继续读取参数或激活动作会因stale context失败，不存在可绕过generation检查的异步callback token。
...
Active Pending记录包含ChangeId、SlotId、from/to Equipment、source ActionInstance与begin tick；Commit或Cancel后active记录必须移除，并把同一ChangeId、最终state与resolved tick保存为last resolved record，供snapshot、restore和diagnostics确认事务结果。
