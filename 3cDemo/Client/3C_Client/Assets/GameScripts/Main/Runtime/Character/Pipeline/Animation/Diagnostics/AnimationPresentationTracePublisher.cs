...
namespace ThirdPersonCharacter.Pipeline.Animation.Diagnostics
{
    internal static class AnimationPresentationTracePublisher
    {
            ...
            Vector3 poseRootWorldScale)
        {
            ...
            if (hasModifier && modifier.Rewritten && modifier.ClearanceEvaluated &&
                modifier.CurrentPathSupport.HasHit)
            {
                finalPhysicalSupportKind = "PredictiveClearancePath";
                finalPhysicalSupportSurfaceIdentity = modifier.CurrentPathSupport.SurfaceIdentity;
                finalPhysicalSupportPoint = modifier.CurrentPathSupport.Point;
                finalPhysicalSupportNormal = modifier.CurrentPathSupport.Normal;
            }
            else if (grounding.SoleSupport.HasHit)
            {
