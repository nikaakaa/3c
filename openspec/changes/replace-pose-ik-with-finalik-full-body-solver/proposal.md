...
In-place AnimationClip + Rig/Calibration
  ...
  -> FootGrounding current-support facts
  -> optional PredictiveFootPlacementModifier
  -> Stance / Anchor / Support Leg / Pelvis / Body Pivot
  -> one Final Goal Set
...
这里的`FootGrounding`只发布当前支撑、接触和鞋底安全事实，不先生成一套响应式Swing Goal供预测覆盖。Predictive直接消费原始Component Pose、权威步事件、未来身体轨迹和唯一查询结果；没有Executable计划时，Swing保持原动画并明确报告Unavailable或Rejected，不把响应式结果伪装成预测成功。
...
1. 新Artifact在平地逐相位重建原动画Foot/Sole/Ankle/Knee/Hip位置与旋转，并通过固定误差门禁；
2. Start、Loop、Stop、MovingTurn左右脚Action Step身份、cycle和phase连续；
3. 平地Runtime预测Foot Route与同相位Native Pose匹配，前向距离和侧向轮廓不再出现约两倍或横向漂移；
4. A/D连续转向由committed Future Body Transform轨迹解释，Revision交接的位置、线速度和角速度连续；
5. 楼梯Executable Plan完整显示Virtual Ground、Capsule查询、合法支撑、Reachability、Ground Envelope和实际消费点；
6. 同一Plan内Path、Landing和Goal没有未解释跳变，Rejected不会切换到伪预测结果；
7. 上楼支撑腿与Hip高度连续，下楼Heel/Toe没有未解释下陷或浮空；
8. Locked脚世界Goal稳定，Sliding只在支撑面内移动，Landing与后继Plan共享同一支撑事务；
9. Foot Orientation和Body Pivot对上坡、下坡、跑步与转向使用明确策略；
10. Final Goal连续安全时，FBBIK solver与physical residual保持正式容差；
11. OpenSpec strict validate、单一路径静态搜索、Runtime/Editor编译和精确Float32/Fixed Character Build通过；
12. 编译、Build或Console 0 Error不得代替IK效果与数据验收。
