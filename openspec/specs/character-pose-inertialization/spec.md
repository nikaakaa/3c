# character-pose-inertialization Specification

## Purpose

定义Pose Graph局部分支惯性化的正式节点、typed discontinuity输入、Pose输出、策略覆盖、每骨骼残差、原子rebase、阶段边界与只读诊断，并禁止隐藏全局连续化路径。
## Requirements
...
#### Scenario: PoseState分支局部惯性化

- **WHEN** Locomotion State的SelectedPosePlayer连接Inertialization而Action Slot分支不连接
- **THEN** state-local source jump MUST只更新该节点残差
...
- **WHEN** Inertialization节点直接消费PoseStateMachine且一条edge触发Inertialization
- **THEN** 节点 MUST使用该edge编译后的duration、curve index与profile index
- **AND** 节点Policy MUST不覆盖该时间数学
