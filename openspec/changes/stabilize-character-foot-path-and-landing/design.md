...
| `0x171D7910`为每脚修正标量保存另一份历史并执行第二次`±rate×dt`限制 | `EffectiveCorrection`必须在状态权重混合后形成独立历史并受正式Correction响应限制；不得用Plant Target历史替代这一层。两档速率与一次性旁路在ZZZ中存在，但业务选择位未证实，项目不得直接命名照搬 | Interpolation Runtime Correction Stage |
