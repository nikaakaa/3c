...
namespace ThirdPersonCharacter.Pipeline.Animation.MotionMatching
{
    public readonly struct MotionMatchingSelectionDecision
    {
        ...
        {
            if (kind == MotionMatchingSelectionDecisionKind.Invalid || !generation.IsValid || !plan.IsValid || sampleIndex < 0)
        ...
        public bool IsValid => Kind != MotionMatchingSelectionDecisionKind.Invalid && Generation.IsValid && Plan.IsValid && SampleIndex >= 0;
    }
    ...
    public sealed class CharacterMotionMatchingSelectionRuntime
    {
        ...
        public MotionMatchingSelectionDecision SearchAndSelect(MotionMatchingQuery query, MotionMatchingSearchTriggerReason triggerReason)
        {
            ...
            MotionMatchingSelectionDecisionKind kind;
            if (!m_Generation.IsValid || query.Initialization)
            {
            ...
            m_PlanCursor = 0;
            m_SampleAccumulator = 0f;
            LastDecision = new MotionMatchingSelectionDecision(kind, m_Generation, plan, m_CurrentSampleIndex, triggerReason);
            return LastDecision;
        }
        ...
        {
            if (!HasSelection)
                return new MotionMatchingSelectionDecision(MotionMatchingInvalidReason.NoValidPlan, MotionMatchingSearchTriggerReason.PlanInvalidated);
