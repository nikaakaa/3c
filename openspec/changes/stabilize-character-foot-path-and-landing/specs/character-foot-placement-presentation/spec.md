...
唯一typed `CharacterFootInterpolationRuntime` MUST拥有上一Target、从Swing连续到Plant的每脚Target Height History、上一Mixed World Target、上一实际Response Output Point、完整Vector Plant World Residual、Component Up Correction Response History、唯一Swing/Release Residual与Completion。Swing Path换代、Landing Acquire和Release MUST只通过固定typed Policy Request连续化；迁移完成后 MUST删除分散的`SwingResidual`、`AcquireResidual`、`ReleaseResidual`、`ContactProgress`和重复Advance数学。
...
Target Height、Plant World Residual与Correction Response MUST分别发布Owner、Before、Target、Applied Delta、After和Reset Reason，不得合并、互相覆盖或由同一无类型Reset清空。旧单档`MaximumVerticalCorrectionSpeed`、上一世界输出重表达、无条件全Plant限速、“World Residual取代Correction历史”的Disposition与“上一Animated Sole + Previous Correction”伪基准 MUST保持删除。Swing/UnlockedSupport MUST继续以Accepted Ground Envelope作为硬下界，Release MUST继续使用统一Residual。`AcquireByWeight`进入帧不得对Contact Anchor立即`RaiseToMinimum`，正式Weight达到1时也不得清除尚未收敛的Residual或Correction Response。Residual大于`SwingResidualTolerance`时，Interpolation Runtime MUST按正式Step Time计算Landing截止收敛；Releasing完成 MUST只读取独立`ReleaseCompletionTolerance`。Step Time只决定Residual衰减，不得改变Raw Target、重选State或掩盖同帧不连续。既有Foot Goal/Position Weight MUST只在Response Output之后与动画基线混合一次；FBBIK与Final Pose之后 MUST没有常驻Foot低通。
...
`TargetHeightAdoptionMode`、`MaximumVerticalTargetSpeed`、`TargetHeightForceRefreshDistance`、`CorrectionResponseIncreaseSpeed`、`CorrectionResponseDecreaseSpeed`、`GroundPenetrationTolerance`与`LandingLockCompletionTolerance` MUST由Corin Profile显式序列化并进入Profile Revision；所有标量 MUST有限且为正，不得使用代码默认值或互相复用，也不得复用Landing接受、Path Revision、Swing Residual、Release完成与Lock准入距离。`TargetHeightForceRefreshDistance` MUST大于`PathRevisionDistance`。Corin MUST首先使用实测激活实例对应的`Direct` Target Height模式；Correction Response两档速率的首个项目候选 MUST分别为`1.8m/s`与`1.5m/s`。两档选择在项目中 MUST由Desired Response沿Component Up的增减方向表达，不得复制或命名匿名`0x199`、D、`0x54`、`0x58`。`GroundPenetrationTolerance`与`LandingLockCompletionTolerance`首个候选均为`0.01m`，`TargetHeightForceRefreshDistance`首个候选为`0.30m`。旧`MaximumVerticalCorrectionSpeed` MUST不再出现在Profile、Runtime或诊断schema中；新Correction Response字段 MUST不得读取或迁移其旧`0.6m/s`值。
...
每个表现帧 MUST从与Foot Placement相同的`FinalAnimationPoseFrame`和Rig Calibration取得每脚Heel、Toe接触点、Foot Rotation、Sole Forward、Component Up与脚掌尺寸，并在现有World Query Backend内建立固定容量`CurrentSupportObservation`。Heel与Toe MUST分别使用同一Profile声明的查询距离、半径、坡度、Layer、自身Collider排除、有限值与World Revision合同；查询形状 MUST使用项目Backend已经正式声明的Support Probe，不得从ZZZ未恢复名字的外部函数猜测Unity重载、硬编码六次查询或复制对象偏移。

每个Probe MUST分别保存Accepted/Rejected、命中位置、命中法线、距离、Surface identity、World Revision和拒绝原因。任一Probe没有合法命中或容量溢出时，Current Support MUST发布typed unavailable；不得用上一结果、Animated Up、单点降级、默认地面或另一脚结果冒充本帧成功。两个Probe都合法时，Runtime MUST计算Heel与Toe相对动画脚掌沿Component Up的所需位移，以较大所需位移决定不会让任一点穿入支撑面的唯一Foot Target Position；所需位移相等时 MUST按稳定Surface identity和固定Heel/Toe顺序决定，不得依赖物理查询返回顺序。Support Normal MUST由被选支撑命中的有限法线、另一命中法线和Heel到Toe脚掌方向形成，归一化后与Position、Surface identity及Observation lineage一起发布为唯一`CharacterFootSupportTarget`。

Current与Target `CharacterFootSupportTarget` MUST在State Target阶段使用同一typed状态/过渡权重同时混合Position与Support Normal；Normal MUST在构造Rotation前重新归一化。Runtime MUST以Rig Calibration的Sole Forward和唯一Support Normal生成同一个Foot Rotation Goal；Position、Rotation、Goal Weight、Observation lineage与Writer MUST属于同一Resolved Foot。Toe MUST没有独立Goal、Correction History、IK、Writer或Pose后Rotation低通。Pelvis、Primary Support与下游Goal Assembler MUST只消费Resolved Foot Pair，不得读取Heel/Toe原始命中或执行第二次查询。

#### Scenario: Heel与Toe落在台阶边缘的不同高度

- **WHEN** Heel与Toe Probe都合法且沿Component Up要求不同的脚掌位移
- **THEN** Runtime MUST以较大所需位移生成唯一Foot Target Position，并以稳定Surface identity和固定Probe顺序处理等价情况
- **AND** MUST保留两个Probe事实、生成一个归一化Support Normal且只写一个Foot Goal

#### Scenario: 任一脚掌Probe不可用

- **WHEN** Heel或Toe任一Probe没有合法命中、World Revision不匹配或固定容量溢出
- **THEN** Current Support MUST发布typed unavailable并保持根Pending事务不可提交该结果
- **AND** MUST不退化为单点支撑、上一帧法线、默认Up或独立Toe Goal

#### Scenario: 攻击动画产生大幅脚目标变化

- **WHEN** 正式攻击动画使Foot/Toe Pose输入或Desired Foot Target单帧大幅变化
- **THEN** Current Support MUST继续进入与Locomotion相同的State Blend、Target Height、World Residual、Correction Response与Goal链
