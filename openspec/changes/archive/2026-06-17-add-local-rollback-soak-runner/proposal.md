...
## Verification Notes
- Play Mode 触发 F6 后，Console、过滤脚本、断言脚本或 HITL 脚本应可搜索并验证 `[rollback-synctest] PASS`。
- Play Mode 触发 F8 soak 后，应可搜索并验证 `ROLLBACK_SOAK_RESULT`。
- 默认 hidden F6/F8 后，角色画面和相机视角应保持触发前状态。
