...
{
    public interface IFixedSimulationPipelineDefinition
    {
    ...
    {
        int MaximumBodySamplesPerActor { get; }
        FixedSimulationSourceRuntimeBinding BindRuntime(FixedSimulationSourceRuntimeBindingRequest request);
    }

    public sealed class FixedSimulationSourceRuntimeBindingRequest
    {
        public FixedSimulationSourceRuntimeBindingRequest(
            ...
            ThirdPersonSimulation.Fixed.ISimulationDiagnosticsSink diagnostics,
            SimulationComponentIdentity committerIdentity,
            IReadOnlyList<IFixedSimulationActorRegistration> registrations)
        {
            PipelineDefinition = pipelineDefinition ? pipelineDefinition : throw new ArgumentNullException(nameof(pipelineDefinition));
            SnapshotCodec = snapshotCodec ?? throw new ArgumentNullException(nameof(snapshotCodec));
            Output = output ?? throw new ArgumentNullException(nameof(output));
            Diagnostics = diagnostics ?? throw new ArgumentNullException(nameof(diagnostics));
            if (!committerIdentity.IsValid || committerIdentity.Role != SimulationComponentRole.Committer)
                throw new ArgumentException("Fixed Committer identity is invalid.", nameof(committerIdentity));
            CommitterIdentity = committerIdentity;
            Registrations = registrations ?? throw new ArgumentNullException(nameof(registrations));
        }

        public SimulationPipelineDefinition PipelineDefinition { get; }
        public IFixedSimulationSessionSnapshotCodec SnapshotCodec { get; }
        public IFixedSimulationResultOutputPort Output { get; }
        public ThirdPersonSimulation.Fixed.ISimulationDiagnosticsSink Diagnostics { get; }
        public SimulationComponentIdentity CommitterIdentity { get; }
        public IReadOnlyList<IFixedSimulationActorRegistration> Registrations { get; }
    }

    public sealed class FixedSimulationSourceRuntimeBinding
    {
        public FixedSimulationSourceRuntimeBinding(
            IFixedSimulationSessionRuntimeLauncher runtimeLauncher,
            ...
            ThirdPersonSimulation.Fixed.SimulationPipelineInitialStateSource pipelineInitialState)
        {
        ...
        public ThirdPersonSimulation.Fixed.SimulationPipelineInitialStateSource PipelineInitialState { get; }
    }

    internal sealed class FixedSimulationOutputAggregate :
        IFixedSimulationResultOutputPort,
