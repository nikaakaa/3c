...
- 让动画表现通过 Animancer TransitionLibrary alias 表映射，不让逻辑层知道 `RunEnd` 这类具体动画 key。
- 保持逻辑层可复用：同一套状态图可以用于不同角色、不同 Animancer transition alias 表和未来不同 motion executor。
...
- 不新增全局动画 catalog，也不新增项目侧动画映射资产。当前基础移动动画资源由 Animancer TransitionLibrary alias 表管理。
...
### Decision: 动画映射使用 Animancer TransitionLibrary alias 表
不新增 `LocomotionAnimationSetSO` 或等价资产。`BasicLocomotionAnimancerPresenter` 只根据逻辑阶段、输入强度和上一次移动步态生成标准 alias key：
...
实际动画资源、淡入和 transition 参数由 Animancer TransitionLibrary 管理。

Reason: 项目已经有 Animancer alias 表。再新增一张基础移动动画映射表会形成重复权威，导致 Inspector 需要维护两份别名关系。`RunEnd` 仍是 `MoveStop + LastMovingGait=Run` 的动画变体，不是逻辑状态；但具体 transition 应由 Animancer alias 表而不是项目侧二次映射资产决定。
...
- 从初始状态能到达所有启用状态
- 四阶段默认图必要转移齐全
- Animancer TransitionLibrary 缺少当前四阶段必要 alias 时由 presenter 报错
...
BasicLocomotionAnimancerPresenter
  ...
  - 根据 MovementAnimationContext 生成标准 Animancer alias key
  - 通过 Animancer TransitionLibrary 解析实际 transition
  - 不修改逻辑状态
  - 不调用运动 API
```
...
- Risk: 标准 alias key 目前写在 presenter 中。
  - Mitigation: 这些 key 是 Animancer TransitionLibrary 的公开约定，测试覆盖 prefab/场景不再绑定二次动画映射资产；如果未来需要角色差异，优先在 Animancer alias 表中处理。
...
- EditMode 测试 presenter 不暴露项目侧 AnimationSet，并确认项目资产不再引用 `animationSet` 字段。
