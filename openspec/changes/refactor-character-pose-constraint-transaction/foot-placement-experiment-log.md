...
1. Swing FootPath只使用非负动画高度增量：

```text
max(0, Envelope - Baseline)
```
...
1. PlantConfidence只用于接触事件判断，不能作为连续插值Alpha。
2. 实时Path Target不得拥有最终硬约束权限。
3. Prediction Point、Contact Patch、Contact Anchor必须分型。
4. 平滑结果是否Settled不能决定Plant事件是否成立。
5. Locked Anchor必须只有一个位置Owner，且锁定后不可被Path更新。
6. Landing、Acquiring、Locked之间不得重复捕获起点或叠加多次平滑。
...
4. 下一方案引用某个动画权重作为进度前，必须先用CSV证明它在同一事件内真实单调。
