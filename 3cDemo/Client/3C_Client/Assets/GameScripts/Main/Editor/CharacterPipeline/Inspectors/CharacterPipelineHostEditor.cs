    ...
    static class CharacterRuntimeDiagnosticsInspector
    {
        ...
        static void AppendFootIkLegHeader(List<string> values, string prefix)
        {
            string[] beforePath =
            {
                ...
                "maximum_anchor_distance", "anchor_blend_speed",
                "stance_ownership_state", "anchor_transaction_state", "anchor_plan_sequence", "anchor_landing_event_identity",
                "animation_constraint_event_identity", "animation_constraint_weight", "animation_support_weight",
                "anchor_raw_blend", "anchor_target_blend", "pelvis_support_raw_weight", "pelvis_support_target", "committed_anchor_goal_available",
                "has_anchor", "anchor_local_x", "anchor_local_y", "anchor_local_z",
                ...
                "plan_build_candidate_kind", "plan_build_decision_reason", "plan_build_landing_event_identity",
                "plan_build_origin_kind", "plan_build_origin_plan_sequence", "plan_build_origin_landing_event_identity", "plan_build_origin_support_surface",
                "plan_build_motion_generation", "plan_build_motion_authority_tick", "plan_build_attempted",
                "plan_build_current_planning_candidate", "plan_build_incoming_planning_candidate", "plan_build_current_plan_matches",
                "plan_build_active_event_replaced", "plan_build_needs_initial_plan", "plan_build_intent_revision_requested",
                "plan_build_can_prepare_event_successor", "plan_build_motion_within_commit_tolerance", "plan_build_can_begin_transition",
                "plan_build_future_body_available", "plan_build_current_plan_has_executable_path", "plan_build_plan_fading_out",
                ...
                "prediction_lead_seconds", "planned_landing_elapsed_seconds", "world_projection_frozen",
                "world_projection_expected_root_x", "world_projection_expected_root_y", "world_projection_expected_root_z",
                "world_projection_current_root_x", "world_projection_current_root_y", "world_projection_current_root_z",
                "expected_presented_body_x", "expected_presented_body_y", "expected_presented_body_z",
                "projected_expected_presented_body_x", "projected_expected_presented_body_y", "projected_expected_presented_body_z",
            ...
            string[] afterPath =
            {
                ...
                "landing_handoff_available", "landing_handoff_plan_sequence", "landing_handoff_event_identity", "landing_handoff_blend",
                "landing_handoff_origin_world_x", "landing_handoff_origin_world_y", "landing_handoff_origin_world_z",
                "landing_handoff_target_world_x", "landing_handoff_target_world_y", "landing_handoff_target_world_z",
                "active_plan_geometry_goal_available", "active_plan_geometry_goal_world_x", "active_plan_geometry_goal_world_y", "active_plan_geometry_goal_world_z",
                "active_plan_goal_available", "active_plan_goal_world_x", "active_plan_goal_world_y", "active_plan_goal_world_z",
                "active_plan_goal_reject_reason",
                "revision_plan_geometry_goal_available", "revision_plan_geometry_goal_world_x", "revision_plan_geometry_goal_world_y", "revision_plan_geometry_goal_world_z",
                "revision_plan_goal_available", "revision_plan_goal_world_x", "revision_plan_goal_world_y", "revision_plan_goal_world_z",
                "revision_plan_goal_reject_reason",
        ...
        internal static void AppendFootIkLegValues(List<string> values, RuntimeFootIkLegTraceSnapshot leg)
        {
            values.AddRange(new[]
            {
                ...
                Number(leg.MaximumAnchorDistance), Number(leg.AnchorBlendSpeed),
                leg.StanceOwnershipState, leg.AnchorTransactionState, Number(leg.AnchorPlanSequence), Number(leg.AnchorLandingEventIdentity),
                Number(leg.AnimationConstraintEventIdentity), Number(leg.AnimationConstraintWeight), Number(leg.AnimationSupportWeight),
                Number(leg.RawAnchorBlendWeight), Number(leg.AnchorBlendTarget), Number(leg.RawPelvisSupportWeight), Number(leg.PelvisSupportTarget), Bool(leg.CommittedAnchorGoalAvailable),
                Bool(leg.HasSurfaceAnchor),
                ...
                leg.PlanBuildCandidateKind, leg.PlanBuildDecisionReason, Number(leg.PlanBuildLandingEventIdentity),
                leg.PlanBuildOriginKind, Number(leg.PlanBuildOriginPlanSequence), Number(leg.PlanBuildOriginLandingEventIdentity), Number(leg.PlanBuildOriginSupportSurfaceIdentity),
                Number(leg.PlanBuildMotionGeneration), Number(leg.PlanBuildMotionAuthorityTick), Bool(leg.PlanBuildAttempted),
                Bool(leg.PlanBuildCurrentPlanningCandidate), Bool(leg.PlanBuildIncomingPlanningCandidate), Bool(leg.PlanBuildCurrentPlanMatches),
                Bool(leg.PlanBuildActiveEventReplaced), Bool(leg.PlanBuildNeedsInitialPlan), Bool(leg.PlanBuildIntentRevisionRequested),
                Bool(leg.PlanBuildCanPrepareEventSuccessor), Bool(leg.PlanBuildMotionWithinCommitTolerance), Bool(leg.PlanBuildCanBeginTransition),
                Bool(leg.PlanBuildFutureBodyAvailable), Bool(leg.PlanBuildCurrentPlanHasExecutablePath), Bool(leg.PlanBuildPlanFadingOut),
                ...
                Number(leg.PredictionLeadSeconds), Number(leg.PlannedLandingElapsedSeconds), Bool(leg.WorldProjectionFrozen),
                Number(leg.WorldProjectionExpectedRoot.x), Number(leg.WorldProjectionExpectedRoot.y), Number(leg.WorldProjectionExpectedRoot.z),
                Number(leg.WorldProjectionCurrentRoot.x), Number(leg.WorldProjectionCurrentRoot.y), Number(leg.WorldProjectionCurrentRoot.z),
                Number(leg.ExpectedPresentedBodyPosition.x), Number(leg.ExpectedPresentedBodyPosition.y), Number(leg.ExpectedPresentedBodyPosition.z),
                Number(leg.ProjectedExpectedPresentedBodyPosition.x), Number(leg.ProjectedExpectedPresentedBodyPosition.y), Number(leg.ProjectedExpectedPresentedBodyPosition.z),
            ...
            values.AddRange(new[]
            {
                ...
                Bool(leg.LandingHandoffAvailable), Number(leg.LandingHandoffPlanSequence),
                Number(leg.LandingHandoffEventIdentity), Number(leg.LandingHandoffBlend),
                Number(leg.LandingHandoffOriginWorldPosition.x), Number(leg.LandingHandoffOriginWorldPosition.y), Number(leg.LandingHandoffOriginWorldPosition.z),
                Number(leg.LandingHandoffTargetWorldPosition.x), Number(leg.LandingHandoffTargetWorldPosition.y), Number(leg.LandingHandoffTargetWorldPosition.z),
                Bool(leg.ActivePlanGeometryGoalAvailable),
                Number(leg.ActivePlanGeometryGoalWorldPosition.x), Number(leg.ActivePlanGeometryGoalWorldPosition.y), Number(leg.ActivePlanGeometryGoalWorldPosition.z),
                ...
                leg.ActivePlanGoalRejectReason,
                Bool(leg.RevisionPlanGeometryGoalAvailable),
                Number(leg.RevisionPlanGeometryGoalWorldPosition.x), Number(leg.RevisionPlanGeometryGoalWorldPosition.y), Number(leg.RevisionPlanGeometryGoalWorldPosition.z),
                ...
                leg.RevisionPlanGoalRejectReason,
