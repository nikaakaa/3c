    ...
    public sealed class AgentPatchAuthoringService
    {
        ...
        AgentAuthoringResponse ExportSnapshot(AgentAuthoringRequest request, CharacterPipelineDefinition definition)
        {
            return new AgentAuthoringResponse
            {
                ...
                success = true,
                snapshot = new AgentGraphSnapshotExporter().Export(definition)
            };
        }
            ...
            bool apply)
        {
            ...
            AgentCompileReport dryRun = new AgentPatchCompiler().Compile(definition, snapshot, patch, false);
            if (!apply || dryRun.HasErrors())
                return FromReport(request, dryRun, false);
            ...
            {
                dryRun.Error("transaction", code, message);
                return FromReport(request, dryRun, false, code, message);
            }
            ...
            try
            {
                ...
                applied = new AgentPatchCompiler().Compile(definition, snapshot, patch, true);
                CopyPlannedDiff(dryRun, applied);
        ...
        static void CopyPlannedDiff(AgentCompileReport source, AgentCompileReport target)
        {
