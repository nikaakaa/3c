...
- [ ] 4A.22 `CharacterFootCompletedOutput`必须原子保存Final Goal Owner；Output Continuity identity必须同时包含Plan Sequence与Goal Owner。Runtime Trace与CSV schema v103必须逐脚发布Landing Handoff可用性、Plan/Event identity、冻结Origin、Committed Target和Blend，使`Previous Final -> Frozen Landing Origin -> Committed Anchor -> Final Goal -> FBBIK`可逐帧对账；Landing仍有效时不得提前消费其释放连续性。
