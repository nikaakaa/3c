...
- 同一`LandingEventIdentity`在摆动阶段接受实时成功预测；小于更新死区时复用路径，累计误差超过软阈值后逐步降低Foot Goal约束，硬阈值时释放约束。
...
- Foot Goal以现有Foot Placement Weight为上限，并使用当前Step从`LiftOffPhase`到`LandingPhase`的显式无状态相位权重连续进入；不增加Landing Confidence、输入方向、隐藏阶段权重、跨帧弹簧或跨帧平滑层。
