...
该Requirement被Document v4原子替代v3取代。

#### Scenario: 旧v3合同被请求

- **WHEN** 调用方要求继续写入v3 package
- **THEN** service MUST拒绝并要求重新checkout

### Requirement: Document v3失败恢复必须同时覆盖Unity owner与正式package

该Requirement由v4失败恢复合同取代，并把AnimationClip owner纳入同一事务。

#### Scenario: v3 apply请求进入服务

- **WHEN** apply携带v3 document hash
- **THEN** schema门禁 MUST在Mutation前失败
