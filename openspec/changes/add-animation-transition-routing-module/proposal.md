...
后续`integrate-animation-transition-routing-pipeline`只能复用该模块的正式合同和状态机，把现有Player、BlendStack与Inertialization接入；不得复制另一份路由算法。
...
- 新增模块帧输出，显式报告Standard Blend命令、request准备、request提交、等待、拒绝、release许可和结构化错误。
- 新增连续打断状态机：
...
- 本change完成并由用户跑通、归档后，`integrate-animation-transition-routing-pipeline`才可开始实施。
