...
{
    internal sealed partial class FixedGameplayEffectTarget : IGameplayEffectControlPort<
            ...
            FixedScalar>
    {
            ...
            FixedGameplayEffectExecutionScratch scratch)
        {
            ...
            m_Admission = new GameplayEffectApplicationAdmissionRuntime<
                SimulationGameplayEffectApplication,
                PortableEffectSpecState,
                FixedScalar>(this, m_Scratch.SuppliedSourceAttributes);
        }
        ...
        bool EvaluateAttributeRequirement(PortableEffectSpecState spec, PortableAttributeRequirementsComponent requirement)
        {
            ...
            if (!TryResolveMagnitude(spec, requirement.Threshold, out FixedScalar threshold, out _))
                return false;
        ...
        {
            if (!TryResolveMagnitude(active.Spec, modifier.Magnitude, out FixedScalar value, out string liveAttribute))
                throw new InvalidOperationException($"Unable to resolve Gameplay Attribute modifier magnitude for '{modifier.AttributeId}'.");
            FixedScalar coefficient = modifier.Magnitude.Coefficient;
            FixedScalar postAdd = modifier.Magnitude.PostAdd;
            if (modifier.ScaleWithStack)
            {
                FixedScalar stack = FixedScalar.FromInt64(active.StackCount);
            ...
            PortableEffectComponent component)
        {
            switch (component)
            {
                ...
                {
                    if (!TryResolveMagnitude(spec, modifier.Magnitude, out FixedScalar magnitude, out _))
                        throw new InvalidOperationException($"Unable to resolve Gameplay Modifier magnitude for '{modifier.AttributeId}'.");
                    if (modifier.ScaleWithStack)
                        magnitude *= FixedScalar.FromInt64(stackCount);
                ...
                case PortableExecutionComponent execution:
                {
                    for (int mutationIndex = 0; mutationIndex < execution.Mutations.Length; mutationIndex++)
                    {
                        ...
                        if (!TryResolveMagnitude(spec, mutation.Magnitude, out FixedScalar magnitude, out _))
        ...
        bool TryResolveMagnitude(PortableEffectSpecState spec, PortableMagnitude magnitude, out FixedScalar value, out string liveAttribute)
        {
