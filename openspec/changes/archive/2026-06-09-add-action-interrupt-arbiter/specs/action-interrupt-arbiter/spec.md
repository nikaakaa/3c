...
系统 MUST 保持当前基础移动 `Idle / MoveStart / MoveLoop / MoveStop` 状态图的局部流转职责。动作打断仲裁模块 MUST NOT 接管当前 `MoveStop -> MoveStart` 或 `MoveStop -> Idle` 路径。

#### Scenario: MoveStop 重新输入仍由状态图处理
- **GIVEN** 当前基础移动阶段为 `MoveStop`
- **WHEN** 本帧重新出现移动输入
- **THEN** `MoveStop -> MoveStart` MUST 继续由 `LocomotionStateGraphTransitionConfig` 或等价 Locomotion 状态图规则处理
