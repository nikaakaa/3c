...
缺口不在KCC。当前Gameplay Lab只有Ramp连续楼梯和孤立`StepCapabilityCourse`，没有一段可与Ramp直接比较的连续真实台阶内容；同时`CharacterVisualTrajectoryFollower`在Grounded时强制visible Y直接等于target Y，真实台阶产生的逐级Body高度变化会直接传给VisualRoot和默认Camera。`PredictiveFootPlacement`已经从最终Body Frame、原始Component Pose与同一world query backend生成Current/Future Support和Foot Goals，不需要读取KCC Step phase或新增楼梯专用IK。
...
- Predictive Foot Placement继续使用现有`Ground | FootPlacementSurface`查询Mask、FinalIK Grounding、Ground Envelope、锁脚与moving anchor；它只消费最终Body Frame位移，不读取新阶段的内部状态来选择Surface。
...
- 只需审计Predictive Foot Placement与Camera现有消费者继续读取最终Body Frame；不重写FinalIK Grounding、Predictive Extension、FullBodyIK或Camera resolver。
...
- current `character-foot-placement-presentation`已经明确`Ground | FootPlacementSurface`查询、排除`CharacterTraversal`，并由FinalIK Grounding-backed Predictive Extension生成跨离散台阶的pelvis pre-solve与Foot Goals。本change复用这些合同，只补充离散楼梯的`Ground`表面场景角色。
...
- active `replace-pose-ik-with-finalik-full-body-solver`继续拥有只读PredictiveFootPlacement Goal Source与唯一FullBodyIK Pose solver的职责分离；本change不改变其Pose Graph拓扑、typed Goal Sets或骨骼写入边界。
