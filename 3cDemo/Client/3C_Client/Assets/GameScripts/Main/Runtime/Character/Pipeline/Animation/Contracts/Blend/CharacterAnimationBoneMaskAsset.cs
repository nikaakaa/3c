    ...
    public sealed class CharacterAnimationBoneMaskAsset : ScriptableObject
    {
        ...
        public float[] BuildDense(CharacterAnimationRigDefinition rig)
        {
            ...
            for (int i = 0; i < Weights.Count; i++)
            {
                ...
                dense[rig.RequirePoseBoneIndex(weight.BoneId)] = weight.Weight;
            }
            if (seen.Count != rig.PoseBoneCount)
                throw new InvalidOperationException($"Bone Mask '{name}' does not explicitly cover every Pose Bone.");
            return dense;
        }
    }
}
