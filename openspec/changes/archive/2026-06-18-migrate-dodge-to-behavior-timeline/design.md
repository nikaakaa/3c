...
### Decision: Frame 是时间权威
Dodge timeline runtime MUST 使用 frame index、duration frame 和 window frame range 作为唯一权威。Editor、inspector 或 importer MAY 显示 seconds，但 seconds MUST 在进入 runtime definition 前转换为 frame。
...
- Runtime duration/frame windows use frame authority.
