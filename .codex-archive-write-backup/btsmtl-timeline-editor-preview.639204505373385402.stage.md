...
Authoring Preview MUST把当前Timeline/Track/Clip时间降低为含raw visual time的正式Animation Selection与Parameter page，并执行匹配Projection的`CharacterPresentationPosePlan`。Preview MUST只在图中存在MarkerSync时显示和应用effective time，并按图中显式节点决定直接Player硬切、局部Inertialization残差或Blend Stack多source历史，按同一Layered/Additive/ModifyBone拓扑生成Composed Pose；具备正式Body与PhysicsScene上下文时 MAY执行FootPlacement节点，否则 MUST把world-aware阶段标记为Unavailable。图上正式Inertialization节点 MUST执行history、capture、rebase、HardCut与Reset语义；连续播放 MAY产生Discontinuity并惯性化，非连续scrub/seek MUST重置节点。Preview MUST不创建隐藏Marker Sync、固定per-slot Stack、隐藏Inertialization、简化PoseGraph、Animancer direct Play、假Foot Physics或自动全局平滑。
