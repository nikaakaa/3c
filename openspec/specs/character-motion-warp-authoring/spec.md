# character-motion-warp-authoring Specification
...
Timeline Inspector、Semantic Compiler与Agent Validator MUST复用同一套MotionWarp校验。source、owner、window、mode、offset、weight、clamp、progress curve、Action Context与Action target requirement任一无效时，artifact发布 MUST失败。系统 MUST不写默认目标、不缩短窗口或创建fallback配置。

MotionWarp所属动作 MAY声明`OptionalSnapshot`或`SnapshotRequired`。`None`与MotionWarp的组合 MUST在发布前拒绝。`OptionalSnapshot`动作无目标时，runtime MUST根据已编译业务策略保留resolved source MotionCurve，并报告`NoTargetByOptionalPolicy`或等价typed结果；该结果 MUST不被描述为配置失败或静默禁用。
...
MotionWarp Clip的Position Progress与Yaw Progress MUST继续由MotionWarp Clip唯一拥有并由既有MotionWarp validator、Compiler与Program runtime消费。Timeline Editor MAY通过显式registered ChannelId编辑当前mode启用的进度曲线，但 MUST复用正式单调性、端点与最小key校验，不得创建第二份Warp curve、反射写入口或Generic Curve runtime。

#### Scenario: 编辑Position Progress

- **WHEN** 作者在CURVES分组修改启用position warp的Position Progress channel
- **THEN** Mutation MUST原子替换同一MotionWarp Clip曲线并通过正式MotionWarp validator
- **AND** 后续Compiler MUST沿既有MotionWarp Program字段生成结果
