...
- [ ] 16.1 删除剩余Primary骨盆硬夹紧、边界速度／完成／强制权重分支及末端夹脚，清理正式API／CSV／诊断，保留逐腿观测和原Foot输入
- [ ] 16.2 完整编译、正式加载并同输入自动回放，对141106与152254比较实际骨盆／脚／膝，不改评分、不擅自恢复删除策略
- [ ] 16.3 提交实现与简短MD，原采样原位保留，明确仍存在的问题与未覆盖范围
