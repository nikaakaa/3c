...
namespace ThirdPersonCharacter.Pipeline.Editor.AgentAuthoring
{
    public sealed class AgentPatchCompileSession
    {
        ...
        public bool Initialize()
        {
            ...
            {
                Report.Error("snapshot.domain", "domain_root_mismatch", "Snapshot、Patch 与加载的 root domain 不一致。");
                return false;
            }
            BaseTree rootTree;
            if (Definition)
            ...
            {
                Report.Error("snapshot", "snapshot_source_changed", "Snapshot/Patch 的 root identity 或 source revision 与当前 Definition 不一致，请重新导出 Snapshot。");
                return false;
            }
            ...
            out BaseEdge edge)
        {
            ...
            if (!IsApply)
                return true;
            if (m_ActualOutputs.TryGetValue(value.OperationOutput.OperationId, out AgentPatchActualOutput output) &&
                output.Edge != null && graph != null && graph.Edges.Contains(output.Edge))
            {
                edge = output.Edge;
                return true;
            }
            Report.Error(path, "flow_edge_operation_not_found", $"Flow edge operation output 无法解析：{value.OperationOutput.Value}");
            return false;
        }
        ...
        public bool TryResolveDeclaration(BaseGraph graph, AgentAuthoringReference reference, string path, out BaseExposedProperty declaration)
        {
            ...
            if (!IsApply)
                return true;
            if (m_ActualOutputs.TryGetValue(reference.OperationOutput.OperationId, out AgentPatchActualOutput output) && output.AuthoringValue is BaseExposedProperty created)
            {
                declaration = created;
                return true;
            }
            Report.Error(path, "blackboard_declaration_operation_not_found", $"Blackboard declaration operation output 无法解析：{reference.OperationOutput.Value}");
            return false;
        }
    ...
    public sealed class AgentPatchBoundaryIdentity
    {
        ...
        public bool Validate(UnityEngine.Object definition, AgentPatchCompileSession session, AgentCompileReport report)
        {
            string definitionPath = AssetDatabase.GetAssetPath(definition);
