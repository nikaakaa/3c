...
PresentationFrame MUST消费committed Body/Intent、构造typed Presentation Fact，并消费完整有限Action Selection batch与Parameter page；随后按Projection编译的ordered Pose/Value DAG执行PoseState selection、State source demand/capture、Action playback、Marker time resolve、AnimationSlot、Transition Routing、Local Pose composition、显式Local/Component转换、Component Pose控制、从同一Component Pose分支执行Lyra Foot Plant等价普通FootGrounding Goal Source、可选Swing脚PredictiveFootPlacementModifier与PoseBoneIKGoals、把全部最终Goals汇聚到唯一pure pose FullBodyIK、执行后续Pose stage与FinalPublication。只有唯一OutputPose及全部必需stage完成后才可由唯一final writer发布`FinalAnimationPoseFrame`并推进Camera；Goal Source调度 MUST不产生中间骨骼Pose。任一Fact、source、MarkerSync、Player、Slot、转换、Pose operation、Lyra Sphere Trace、Predictive Extension、Goal validation、FinalIK binding或FullBodyIK solver失败 MUST阻止部分最终结果发布，不得沿用上一帧、只发布pelvis pre-solve结果或绕过节点。
...
- **WHEN** FootGrounding、可选Modifier与PoseBoneIKGoals发布合法最终Goals且FullBodyIK完成一次全身求解
...
Input、ingress、Program operation、StateMachine、Timeline、Blackboard、WorldRequest/Result、Action、Effect、commit、Animation、Lyra Current Grounding、Stance Stabilization、Pelvis Resolve、Predictive Extension、FullBodyIK和Camera diagnostics MUST进入统一structured Trace/view model。Inspector MUST不遍历旧stage、FinalIK组件、FinalIK Grounding、TwoBoneIK/LegIK私有结果或runtime service私有集合形成平行调试链。FootGrounding、Predictive Extension与FullBodyIK trace MUST只读取正式Presentation snapshot，不得重新执行地面查询或solver。
