...
缺口不在KCC。当前Gameplay Lab只有Ramp连续楼梯和孤立`StepCapabilityCourse`，没有一段可与Ramp直接比较的连续真实台阶内容；同时`CharacterVisualTrajectoryFollower`在Grounded时强制visible Y直接等于target Y，真实台阶产生的逐级Body高度变化会直接传给VisualRoot和默认Camera。普通`FootGrounding`从最终Body Frame、原始Component Pose与同一world query backend为每脚执行一次Lyra current Sphere Trace，再完成contact/anchor稳定与pelvis resolve并生成Baseline Foot Goals；可选`PredictiveFootPlacementModifier`只为未被anchor拥有的Swing脚生成Future Support/Envelope，不需要读取KCC Step phase或新增楼梯专用IK。
...
- 普通FootGrounding继续使用现有`Ground | FootPlacementSurface`查询Mask和Lyra单次Sphere Trace，并允许合法stance脚只基于该current hit建立surface-local anchor；可选Modifier继续使用同一Mask为未被anchor拥有的Swing脚生成Future Support与Ground Envelope。两者只消费最终Body Frame，不读取新阶段的内部状态来选择Surface。
...
- 只需审计FootGrounding、Predictive Extension与Camera现有消费者继续读取最终Body Frame；不重写Lyra Current Grounding、Stance Stabilization、Pelvis Resolve、FullBodyIK或Camera resolver。
...
- current `character-foot-placement-presentation`已经明确`Ground | FootPlacementSurface`查询并排除`CharacterTraversal`；active IK change进一步规定普通FootGrounding按`Lyra Current Grounding -> Stance Stabilization -> Pelvis Resolve`生成Baseline Goals，可选Modifier只拥有未被anchor拥有的Swing脚Future Support/Envelope。本change复用这些合同，只补充离散楼梯的`Ground`表面场景角色。
...
- active `replace-pose-ik-with-finalik-full-body-solver`继续拥有只读FootGrounding、可选Swing脚PredictiveFootPlacementModifier与唯一FullBodyIK Pose solver的职责分离；本change不改变其Pose Graph拓扑、contact/anchor所有权、typed Goal Sets或骨骼写入边界。
