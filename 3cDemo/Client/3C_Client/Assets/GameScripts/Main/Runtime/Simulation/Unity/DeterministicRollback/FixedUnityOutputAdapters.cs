...
{
    public sealed class FixedUnityPresentationOutputAdapter : IRollbackPresentationOutputPort
    {
        ...
        bool TryBuildStateKey(CharacterPresentationCommand command, out PresentationStateKey key)
        {
            ...
            switch (command.Kind)
            {
                case CharacterPresentationCommandKind.SelectProducer:
                    key = new PresentationStateKey("animation-selection", producer.LayerId, 0);
                    return true;
