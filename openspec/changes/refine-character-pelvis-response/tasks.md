...
- [x] 11.4 a0aea66恢复四资产到20毫米；协调后101451/facts66对085223的1197个非身份CSV列、16个非身份geometry列逐值相同，37规则／计数／score及1044完整Proof帧恢复。候选12文件与恢复14文件ZIP、原字节Proof及清单核哈希保留；官方对10毫米前驱的7个产品身份差异false不改写。之后并行Ground Path提交另行验证，193957固定效果对照不替换

## 12. 历史经验与失败证据

- [x] 12.1 统一索引Foot前置失败与骨盆9步实验，明确提交、采样时间、保留／撤销／恢复关系及未覆盖边界，不将局部保留称为整体通过
- [x] 12.2 补存085223上的同脚MIN、混合限速、小误差分支及ZZZ普通核只读筛选，记录公式、初值、冻结输入、模型结果与限制；均未实施或新Replay
- [x] 12.3 更新过期恢复状态，保存101451完整原包及原字节Proof，逐项核对SHA和原始行为，保留内存暂停快照与最终1044帧封口的区别；不改算法、质量规则或原始包

## 13. 当前组合自动回归

- [x] 13.1 对721b69f组合显式Refresh并通过正式入口运行原Record，112611完整发布1043帧／2086行；当前表面Ready1787、无空缓存Unavailable，正式summary／frame查询成功，结束后退出本任务Play
- [x] 13.2 独立核对与101451的1044完整Proof数组和原始输出；37项规则／计数／score保持，骨盆世界大步仍33、420仍下降80.210毫米，明确质量未通过；保存14文件ZIP与原字节Proof，不把baseline-created冒充官方A/B

## 14. 几何共同候选的项目适配实验

- [x] 14.1 闭合112611同帧Ankle/Pelvis输入与2F40/97B0候选数学，保存完整选择器与仅MIN控制模型；减少主要突降但平地加深的代价单列，不当真实Replay
- [x] 14.2 c3166ff完成唯一几何候选与正式Corin半径0.2米，Runtime27既有警告0错误、52.79秒并shutdown；132307后因真实代价不接纳，当前已精确恢复本步四路径，几何候选不作为活代码
- [x] 14.3 676fd5c独立迁移67/36、CSV1233及完整候选重算，Editor57既有警告0错误、112.67秒并shutdown，37质量规则不变；132307后ccb14e1仅撤销此六文件，恢复66/35，原67证据不改
- [x] 14.4 9a2db28匹配产品完成132307同Record Replay：1044完整输入帧一致、官方false仅7产品身份；455候选公式、37质量项及Foot保护对账完成，突降33→24但移动平地更低、膝峰移位代价单列；14原文件及原Proof逐项ZIP保留，不以总分不变宣布通过
- [x] 14.5 9eef754保存132307失败证据；1b10e81／ccb14e1精确恢复Runtime／产品／诊断及活跃合同，完整Editor57既有警告0错误并shutdown、显式加载后141106回放完成：1200个非身份列、37规则／计数／score、1044完整Proof及Runtime Identity均与112611相同。恢复14原文件及原字节Proof逐项ZIP保留；这仅确认撤销，不表示整体骨盆达标
...
- [x] 15.3 152254原始数据与141106完成对比，旧非负余量诊断已由5dfa6d1修正；该分支未发布正式analysis，用户随后明确要求第16步撤掉全部硬执行，原恢复计划取消，原始证据保留

## 16. 按用户要求撤掉Reach硬执行

- [x] 16.1 9da24a5删除全部骨盆Reach硬执行与末端夹脚，1790663迁移69/38及观察合同；无旧执行API或恒值兼容路径，原Foot输入／权重不改
- [x] 16.2 完整Editor57既有／依赖警告0错误并shutdown；4934fb9修正表头后重新Unity编译，160901正式同输入回放matched1044并发布69/38；对141106和152254完成实际比较，37规则不变，删除保留且整体质量未达标
- [x] 16.3 原实验MD追加真实改善、Foot／Knee代价与覆盖边界；原采样原位保留，只保护Temp必要Proof，不制作ZIP或提交数据
