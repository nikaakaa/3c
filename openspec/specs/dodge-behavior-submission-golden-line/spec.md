# dodge-behavior-submission-golden-line Specification

## Purpose
定义 Dodge 迁移到 Behavior Submission 后的金线验证，确保 Directional、Backstep、rejected Dodge 和生命周期输出与旧路径等价且不形成第二生产路径。
## Requirements
...
- **THEN** baseline output 与 mapped behavior submission MUST 表达同一 current tick、motion 和 animation intent
...
### Requirement: Golden Line 字段级对比
Dodge golden line MUST 使用共享 comparison schema 或批准的等价断言方式，对 request、claim、motion、animation、input consume、window facts、cue、diagnostics、Run latch 和 restore timing 进行字段级对比。缺失字段 MUST 失败并报告具体字段。

#### Scenario: 缺失 Motion 字段失败
- **GIVEN** baseline Directional Dodge 包含 motion duration ticks 和 distance
- **WHEN** mapped behavior submission 缺失其中任一字段
- **THEN** golden line 测试 MUST 失败
