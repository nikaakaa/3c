...
- FROM: `### Requirement: Corin animation producer 必须绑定正式source与Pose Slot transition`
- TO: `### Requirement: Corin animation producer 必须绑定正式source与显式Player路径`
...
### Requirement: Corin animation producer 必须绑定正式source与显式Player路径

Corin每个正式Timeline animation producer MUST拥有稳定presentation key和显式AnimationChannelId，并通过Profile/Projection绑定到唯一Selection Input与source resource。Profile Inspector MUST按稳定identity显示Locomotion、Action、Attack1..5与Dodge的Channel、Selection Input、Player路径和binding。BaseLocomotion MUST使用`SelectedPosePlayer -> Inertialization`，FullBodyAction MUST使用显式`BlendStack`；Pose Graph继续保存后续composition。BTSMTL Graph/State edge MUST不保存transition technique、duration、curve、Bone Mask或Driver。

#### Scenario: 配置Attack1至Attack5

- **WHEN** 作者在Corin Definition context检查Attack producer
- **THEN** Inspector MUST显示它们属于FullBodyAction Selection Input、Action BlendStack及其source binding
- **AND** Action Blend Policy MUST覆盖可达CrossFade pair
- **AND** Pose Graph MUST不复制Attack combo逻辑edge

#### Scenario: 缺失producer binding

- **WHEN** selected producer没有合法source、AnimationChannelId、Selection Input或所需Policy entry
