...
namespace ThirdPersonCharacter.Pipeline.Editor.AgentAuthoring
{
    public sealed class AgentPatchIdentityBinder
    {
        ...
        readonly Dictionary<string, IdentityReference> m_GraphReferences = new Dictionary<string, IdentityReference>(StringComparer.Ordinal);
        ...
        void BindEnsureStateMachine(AgentPatchOperation operation, string path)
        {
            ...
            if (existing.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(operation.graphAuthoringId) &&
                    m_Graphs.TryGetValue(operation.graphAuthoringId, out AgentSnapshotGraph parent))
                {
            ...
            RegisterGraphReference(selector, existing.IsValid
                ? new IdentityReference(existing.AuthoringId, string.Empty)
                : new IdentityReference(string.Empty, operation.id));
        }
        ...
        void BindEnsureState(AgentPatchOperation operation, string path)
        {
            ...
            if (existing.IsValid)
            {
                ...
                AgentSnapshotStateSummary state = m_Snapshot.stateMachines
                    .SelectMany(value => value.states)
                    .FirstOrDefault(value => value.stateAuthoringId == existing.AuthoringId);
                RegisterGraphReference($"{selector} State Body", new IdentityReference(state?.behaviorGraphAuthoringId, string.Empty));
            }
            else
            {
                ...
                RegisterGraphReference($"{selector} State Body", new IdentityReference(string.Empty, operation.id));
            }
        }
        ...
        void BindGraph(AgentPatchOperation operation, string path, bool required = false)
        {
            ...
            if (m_GraphReferences.TryGetValue(operation.graph, out IdentityReference reference))
            {
                operation.graphAuthoringId = reference.AuthoringId;
                operation.graphOperationId = reference.OperationId;
                return;
            }
        ...
        void BindTargetGraph(AgentPatchOperation operation, string path)
        {
            if (!string.IsNullOrEmpty(operation.targetGraphAuthoringId) || !string.IsNullOrEmpty(operation.targetGraphOperationId) || string.IsNullOrEmpty(operation.targetGraph))
                return;
            if (m_GraphReferences.TryGetValue(operation.targetGraph, out IdentityReference reference))
            {
                operation.targetGraphAuthoringId = reference.AuthoringId;
                operation.targetGraphOperationId = reference.OperationId;
            }
            else
        ...
        void BindExistingEnsureTarget(AgentPatchOperation operation, string path)
        {
            ...
            bool supported = operation.op is "ensure_action_exit_lifecycle" or "ensure_state_behavior_node" or "ensure_timeline_node" or "ensure_action_activation" or "ensure_action_lifecycle_transition" or "ensure_input_node" or "ensure_blackboard_write";
            if (!supported)
                return;
            string typeSuffix = operation.op switch
            {
                "ensure_action_exit_lifecycle" => "SelectorNode",
                ...
                "ensure_blackboard_write" => "ExposedPropertyNode",
                _ => string.Empty
            };
            ...
            string displayName = operation.op == "ensure_action_exit_lifecycle"
                ? "Action Exit"
                : !string.IsNullOrEmpty(operation.displayName) ? operation.displayName : operation.inputId;
            List<AgentSnapshotNode> matches = operation.op == "ensure_blackboard_write"
                ? graph.nodes.Where(i =>
                    i?.blackboardWrite != null &&
                    i.typeName.EndsWith(typeSuffix, StringComparison.Ordinal) &&
                    string.Equals(i.blackboardWrite.declarationAuthoringId, operation.declarationAuthoringId, StringComparison.Ordinal) &&
                    i.blackboardWrite.boolValue == operation.blackboardBoolValue).ToList()
                : graph.nodes.Where(i =>
                    i != null &&
        ...
        void RegisterGraphReference(string selector, IdentityReference reference)
        {
            if (!string.IsNullOrEmpty(selector) && reference.IsValid)
                m_GraphReferences[selector] = reference;
        }
