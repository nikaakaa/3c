...
namespace ThirdPersonCharacter.Pipeline.Editor.AgentAuthoring
{
    internal static class AgentAuthoringPresentationPackageCodec
    {
        ...
        const string ClipPrefix = "editable/animation-clips/";
            ...
            AgentCompileReport report)
        {
            ...
            foreach (KeyValuePair<string, JToken> pair in files)
            {
                ...
                if (!pair.Key.StartsWith(
                        ...
                        StringComparison.Ordinal) &&
                    !pair.Key.StartsWith(ClipPrefix, StringComparison.Ordinal))
                    continue;
                if (!string.Equals(
                        pair.Key,
                        ProfilePath,
        ...
        static string ClipPath(string id) =>
            ClipPrefix + AgentAuthoringPackageMapper.Segment(id) + "/curves.json";

        static bool IsClipFile(string path) =>
