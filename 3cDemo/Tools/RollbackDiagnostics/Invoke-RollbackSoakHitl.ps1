...
function Invoke-Step([string]$Name, [scriptblock]$Step) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Output "ROLLBACK_SOAK_HITL result=FAIL stage=$Name"
        exit 1
    }
}
...
if (-not (Test-Path -LiteralPath $LogPath)) {
    Write-Output "ROLLBACK_SOAK_HITL result=FAIL stage=log reason=log-not-found path=$LogPath"
    exit 1
}
...
Write-Output "ROLLBACK_SOAK_HITL waiting=True timeoutSec=$TimeoutSec action=enter-play-mode-and-press-F8"
...
if (-not $seen) {
    Write-Output "ROLLBACK_SOAK_HITL result=FAIL stage=wait reason=missing-ROLLBACK_SOAK_RESULT timeoutSec=$TimeoutSec"
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Output "ROLLBACK_SOAK_HITL result=FAIL stage=assert"
    exit 1
}

Write-Output "ROLLBACK_SOAK_HITL result=PASS"
exit 0
