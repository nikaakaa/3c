...
| Foot与Toe世界姿态生成脚掌多点查询，先解析唯一`FootTargetPosition + SupportNormal`；只有FootL/FootR与Pelvis writer，没有Toe writer。Corin实例已闭合`FootHeights=.08`、Heel/Toe Offset各`.01`、`FootWidthRadius=.005`、`RayCastHeight=.5`，QueryFrame数组为空；五个固定点加条件Foot Pivot第六点均沿World Down查询1米 | Rig Calibration正式保存五个通用Foot/Toe局部查询点，Profile独立保存Current Support Ray查询高度与距离；现有World Query Backend后续只消费这些typed数据。条件第六点不需要额外校准值。结果记录如何组合Position+Direction仍未闭合，当前数据迁移不提前替换双Probe行为 | Rig Calibration、Current Support Query Profile、Observation、State Target、Resolved Foot |
...
| Corin五点布局、条件第六点、World Down方向与查询距离 | 字段名、Corin实例绑定、五点/第六点构造和单记录Ray查询P0/P1闭合；最终六记录Position+Direction聚合仍未知 | 五点布局进入Rig Calibration，Ray范围进入Profile；第六点使用实时Foot Pivot。未闭合聚合前不得以最大Up、平均Normal或nearest结果冒充完整迁移 |
