...
Rollback Product Build的角色产物阶段只向公共Character Simulation Build Request安装Fixed Target Adapter。该请求从当前Definition执行一次Frontend、建立Presentation Semantic Contract、生成唯一target-neutral Projection与Fixed Program，并在ContractHash一致后原子发布。它不调用Float32 Target Compiler，也不写Float32 Program wrapper；Run阶段仍只读取已经发布的Fixed Program、Projection和产品manifest，不触发编译或修复。
