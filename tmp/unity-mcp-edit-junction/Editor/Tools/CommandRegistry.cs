...
namespace MCPForUnity.Editor.Tools
{
    ...
    class HandlerInfo
    {
        ...
        public bool BackgroundPollingStatus { get; }
        public string PollAction { get; }

        public bool IsAsync => AsyncHandler != null;

        public HandlerInfo(
            string commandName,
            Func<JObject, object> syncHandler,
            Func<JObject, Task<object>> asyncHandler,
            bool backgroundPollingStatus,
            string pollAction)
        {
            CommandName = commandName;
            SyncHandler = syncHandler;
            AsyncHandler = asyncHandler;
            BackgroundPollingStatus = backgroundPollingStatus;
    ...
    public static class CommandRegistry
    {
        ...
        private static bool RegisterCommandType(Type type, bool isResource)
        {
            ...
            bool backgroundPollingStatus = false;
            string pollAction = "status";
            ...
            else
            {
                ...
                backgroundPollingStatus = toolAttr.BackgroundPollingStatus;
                pollAction = string.IsNullOrWhiteSpace(toolAttr.PollAction)
                    ? "status"
                    : toolAttr.PollAction;
            }
        ...
        internal static bool TryExecuteBackgroundPollingStatus(
            string commandName,
            JObject parameters,
            out object result)
        {
            result = null;
            if (!_handlers.TryGetValue(commandName, out HandlerInfo handlerInfo) ||
                !handlerInfo.BackgroundPollingStatus ||
                handlerInfo.IsAsync ||
                handlerInfo.SyncHandler == null)
                return false;
            string action = parameters?.Value<string>("action");
            if (!string.Equals(
                    action?.Trim(),
                    handlerInfo.PollAction,
                    StringComparison.OrdinalIgnoreCase))
                return false;
            result = handlerInfo.SyncHandler(parameters ?? new JObject());
            return true;
        }
