...
### Requirement: Request生命周期必须原子等待capture completion

Inertialization transition MUST依次经过AwaitingTarget、Prepared、AwaitingCaptureCompletion与Committed状态。模块 MUST只在target identity、selection generation、request generation和capture completion全部匹配后输出旧source release permission。任何reset、过期completion或identity不匹配 MUST使旧completion失效，并且 MUST不输出release permission。
