...
同一脚的`NextSwingLanding`在同一Landing Event identity下保留首次Accepted点作为误差基准；摆动阶段新的成功预测超过正式更新死区时更新落点并重建Ground Path，累计误差从软阈值开始用SmoothStep降低约束，达到硬阈值时位置权重为零。这样实时误差能被观察和退出，而毫米级变化不会制造抖动。
...
PositionWeight = verticalCorrection > geometryTolerance
    ? animation.foot-placement-weight * SmoothStep(0, 1, swingPhase)
    : 0
RotationWeight = 0
```
...
本轮计算是单帧纯函数，不新增跨帧Foot Motion缓存、Plan、Revision、Spring或Goal history；Landing事实只在同一Landing Event内保持首次Accepted落点。失败帧直接发布typed rejection和零权重Goal，不能用上一帧结果补洞。
...
3. 同时启用现有FootPlacement Goal Target Watch与FullBodyIK Pose Watch，确认二者Frame、Completion和Rig lineage一致，并以FullBodyIK left/right foot solved position确认最终骨骼Pose已经消费非零Goal；Scene Gizmo和Foot Landing CSV只证明Goal事实，不能单独证明骨骼结果。
