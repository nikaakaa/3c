...
- [ ] 6.21 从同一`FinalAnimationPoseFrame`和Rig Calibration读取每脚Heel/Toe接触点、Foot Rotation、Sole Forward/Right、Component Up与显式Rear/Lateral/Toe Probe尺度，作为Current Support唯一Pose输入；Corin使用`0.03m / 0.04m / 0.01m`且尺度进入Calibration Revision，不得读取Foot Motion Toe曲线、另一Source或LateUpdate骨骼
- [ ] 6.22 在现有World Query Backend内按固定`Base/Rear/PositiveLateral/NegativeLateral/Toe`五点建立Current Support Observation，复用正式距离、半径、坡度、Layer、自身Collider排除、有限值与World Revision合同，并分别保存Accepted/Rejected/NotExecuted、SphereCastExecuted、Hit Capacity、位置、法线、距离、Surface identity和拒绝原因；Base/Toe为必需记录，三个扩展点为始终执行的正式候选
- [ ] 6.23 删除022607失败的`OriginalSole + ComponentUp × max displacement`与另一条selected SphereCast raw Normal组合；每个Accepted Probe以自身Normal和`dot(HitPoint-ProbePosition, Normal)`生成完整XYZ Sole候选，按沿Component Up最高、Surface identity和固定Probe顺序选择同一记录的Position+Requested Direction。不得把HitPoint直接当Position，不猜ZZZ外部重载或匿名第六点，不得以调低Slope、偏好Up、多法线平均、旧结果或默认Up掩盖
